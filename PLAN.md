...
- **Verified 2026-09-21:** all 13 FQDNs serve HTTP 200 over TLS; WebFinger confirms each Iris instance advertises its own FQDN (e.g. `alice@dev1-iris-a.luit.ink` → `acct:alice@dev1-iris-a.luit.ink`). The FQDNs are mapped to the per-env published ports by an **external (operator) reverse proxy** — there is **no proxy stack in this repo** and none to configure. The per-env compose only publishes the host ports (e.g. `dev1-iris-a` → 10081); the external proxy terminates TLS and routes each FQDN to that port. **All communication with the project goes over the deployed containers via the public FQDNs** — never a local port, `localhost`, or `docker exec`.
  - **Dev1 stack:** `dev1-iris-a` + `dev1-iris-b` healthy (fresh build from `2ca23c22`, 2026-09-22 — carries the **S21 residual server `/following` stale-cache fix `1f941cfb`** (creator's `following` + feed cache invalidated on community auto-follow) + S24-D2/S36 home-feed own-outbox content filter `61c328fa` + S24-D2 outbox **Follow-only** filter + S24-D4 remote-actor collection proxy + directory paging + S17 fix + all merged main work deployed; green-gate re-run 2026-09-22: build 0/0, fast suite 2265 passed / 0 failed).
...
- **S40** — orphaned auto-follow `Follow`(0) edge on community delete (dev2 `296e3cdd`, deployed to the dev2 stack; QA must rebuild the **qa** stack to `296e3cdd` to re-verify): create a community → delete it → the deleted community no longer appears in the Communities **Following** tab (no re-fetch 404), the signed `GET /ap/v1/u/{creator}/following` no longer lists it, and the DB has **no** `Follow`(Kind=0) edge targeting the community IRI. 0 console errors on the delete + reload.
