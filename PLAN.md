...
1. **General UI/UX review** (recurring) — Next pass after improvements land. Pass 8 (2026-09-19): post-139.2-s5a/s5b verification — authless pass (all 8 authenticated routes redirect to `/login`, 404 + register accessible, 0 new console errors) + signed-in spot-check as uxreview8 (newly registered) (Home, Communities, Community detail hard-refresh stable, Profile, Compose deep-link hard-refresh stable, Directory, Settings hard-refresh stable, Search hard-refresh stable, Notifications hard-refresh stable), 0 new defects (only console errors were remote-instance availability — 401s from `/ap/v1/proxy/`, CORS from direct remote fetches — not Iris bugs). [change doc](docs/changes/997-ui-ux-review.md)
2. **139.2 Scenario 5 (audience/visibility) — ALL surfaces done.** The S1 read-path gap is **closed** for the public feed, global search, the follow feed, the object-document endpoint (local + federated-in), and the federation read path. The **follow-feed owner gate** (S5c) is done (403 for non-owners). The **object-document visibility gate** (S5a/s5b) is now done for **all** non-tombstone content objects — local AND federated-in (the S5b decision supersedes the original S5a "remote objects are exempt" carve-out). The **federation visibility policy** (S5b) is done: non-public federated content is stored on receipt with `to`/`cc` intact (no suppression) and hidden on the read path by the S5 filter. Remaining (deferred product decision, not a defect): a `cc`-to-follower-set model for followers-only posts (scalability optimization, not a privacy fix — the current per-follower `cc` model is correct; should be its own phase). See [change doc](docs/changes/1392-5a5b-object-document-federated-gate.md).
