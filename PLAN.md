...
Phases 0–9 are complete. Phase 8 delivered the server-side Docker foundation; Phase 9 (Real-World Deployment Preparation) is complete — all five deliverables done and grounded in the real config/client/server/test surface. **Phase 10 (Project & Test Review) is in progress** — Slice 1.1 (test audit + dead-code sweep + IRI-resolution dedup) and Slice 1.2 (page-flatten + collection-IRI dedup) are done: removed the fully-dead `CommunityIris.cs` + `LazyActorDocumentFetcher.cs` (both 0 callers), consolidated the triple-duplicated `IObjectOrLink`→`Iri` resolution into `IriExtensions.ResolveObjectIri`, moved `CollectionPage` to `Iris.Core`, added `CollectionPageFactory.FromOrderedCollectionPage` + `IriExtensions.ResolveCollectionIri` to eliminate the duplicated page-flatten + `TryGetIri` in both `ActivityPubClient` and `IrisRemoteCollectionFetcher` (444→460, 11 new tests). Per-slice detail (what was built, key types, test counts, resolved decisions) lives in [docs/CHANGELOG.md](docs/CHANGELOG.md); the phased plan and open questions are in [docs/ROADMAP.md](docs/ROADMAP.md).
