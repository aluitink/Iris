...
- **Note:** ⑤ Lemmy interop unblocked: (1) actor documents now carry `published` (Lemmy `objects::instance` requires it — was 400 "missing field `published`"); (2) a person's Follow to a *cached* remote community now delivers over the wire (host-based local/remote split — was silently skipped). **⑤ live re-verify (2026-09-20): a person's Follow to Lemmy `c/interop` reached Lemmy end-to-end** (Lemmy `community_follower` gained a row, `pending=f`). The shared-inbox route (`POST /ap/v1/shared-inbox`) is now implemented (was advertised but unhandled — Mastodon dropped its deliveries) — [change doc 1598](docs/changes/1598-shared-inbox-route.md). The Lemmy **Undo (unfollow) 400** is fixed in code (the outbound Undo now embeds the original Follow — [change doc 14814](docs/changes/14814-lemmy-undo-follow-embedded.md)); live re-verify pending redeploy. S2/S14 proxy seam still 401s unsigned GETs.

## Active Slice

- **Lemmy Undo (unfollow) 400 — code fix done + regression test (14814); live re-verify pending.** The outbound Undo now embeds the original Follow (Lemmy's untagged-enum parser rejects a bare IRI link → 400 → dead-letter). Person + community outbox paths both fixed. Full fast suite green. Next: redeploy + live Follow→Undo→200 + `community_follower` row removed. [change doc 14814](docs/changes/14814-lemmy-undo-follow-embedded.md)
...
- *(empty — Lemmy Undo 400 fixed, moved to Recently Completed; live re-verify is the Active Slice)*
...
- **⑤ Live Lemmy interop — Follow COMPLETE + Undo fixed (2026-09-20).** Interop blockers fixed + deployed: actor `published` field, follow delivery for cached remote communities ([change doc 1597](docs/changes/1597-lemmy-follow-delivery-and-actor-published.md)), and the Undo (unfollow) 400 ([change doc 14814](docs/changes/14814-lemmy-undo-follow-embedded.md)). **Live-verified:** a person's Follow to Lemmy `c/interop` reached Lemmy end-to-end (`community_follower` row, `pending=f`). **Remaining:** (a) live re-verify the Undo (Active Slice); (b) **Mastodon unfollow-fanout** observation: user still sees messages from an unfollowed user — investigate whether the Undo-fanout to followers' feeds is dropped (now partly addressed by the shared-inbox route, [change doc 1598](docs/changes/1598-shared-inbox-route.md)).
...
- **Lemmy Undo (unfollow) 400 fix (2026-09-20):** the outbound Undo of a Follow now **embeds the original Follow** (Lemmy's untagged-enum ActivityStreams parser rejects a bare IRI link → 400 → the un-follow was dead-lettered). Person + community outbox paths both fixed; the stored activity keeps the bare link (only the outbound delivery materializes the embedded form — backward-compatible, Iris inbound `UndoActivityHandler` accepts either). New regression test asserts the embedded shape. Full fast suite green: 1401 passed, 0 failed. Live re-verify (Follow→Undo→200 + `community_follower` row removed) is the Active Slice. [change doc 14814](docs/changes/14814-lemmy-undo-follow-embedded.md)
