...
- **S2-sev:** **S36** [home-feed omits posts](docs/qa/s36-home-feed-omits-posts-and-is-polluted-with-actor-document-activity.md) — **FIXED `14eb0db1` + `61c328fa`** (IsFollowReply cc=[followers] fix + own-outbox content filter for foreign-boost/actor-doc noise), awaiting QA re-verify. **S17** [profile tabs overfetch outbox](docs/qa/s17-profile-tabs-overfetch-outbox.md) — **FIXED `645b3660`, awaiting QA re-verify** (Likes tab scoped to `/liked`; Your posts/Replies bounded to 4 pages). **S4** [communities following drops remote](docs/qa/s04-communities-following-remote.md) — **FIXED `83eb22aa`+`cca01fa7`, awaiting QA re-verify** (client fetch-by-IRI fallback + server Accept/Reject recipient re-resolution). **S24-D2** [foreign activities in local outbox](docs/qa/s24-cross-instance-follow-state-inconsistent.md) — **feed-surface facet FIXED `61c328fa`** (home feed no longer surfaces foreign boost / actor-doc noise); **outbox-integrity facet (foreign activities physically in the local outbox) still OPEN — dev2 investigating the write path**. **S24-D4** [remote-actor collection routes 404](docs/qa/s24-cross-instance-follow-state-inconsistent.md) — **FIXED `2804fb55`, live-verified** (collection proxy to the remote instance). **S2, S3, S14, S19, S20, S21, S35** — open.
- **S3-sev:** **S38** — open.
...
- **S24-D2 / S36 home-feed pollution:** **FIXED `61c328fa`.** The home feed's own-outbox branch now keeps only owner-authored content (`Create` of Note/Article/Question, or own `Announce`), excluding foreign boost fan-out (actor=other) and actor-doc noise that dominated the feed and pushed real posts out of the cap. Feed endpoint is owner-gated so no cross-instance impact; the F-15 boost fan-out is preserved. 1438 Server + 113 Web tests pass. (Outbox-integrity facet — foreign activities physically in the local outbox — remains open, dev2 on the write path.)
