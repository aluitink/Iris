...
**45.5: Notifications & moderation pass** (next up). See [docs/plans/wasm-stabilization.md](docs/plans/wasm-stabilization.md) for scope.
...
1. **45.5: Notifications & moderation pass** — notifications list + unread badge + mark-all-read; block/mute/report + undo (posts + actor detail); admin user list.
2. **45.6: Edge states pass** — empty/loading/error states, deep links + refresh on every route, responsive 375px/1024px, console errors on every page.
3. **45.7: Triage closeout** — review all defects from 45.1–45.6; unfixed → Up Next; change doc summarizing findings + fixes.
