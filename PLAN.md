...
**Phase 31 — explorer UI & server namespace polish.** 31.1–31.8 complete (WebFinger RFC 7033 + retry, `PagedCollection` initial-load re-render, carla fake-remote seed gated off, directory actors-only + rename, actor-detail profile header + tabs, community profile header + tabs, the shared `CollectionBrowser` component, and the server `iris:` namespace derived from the public base URI + hosted namespace document). Next: 31.9 (followed-feed `/feed` drops Create + Accept activities — server `FollowFeedHandler` union bug). Phase 30 (30.2 health check + readiness probe) resumes after the Phase 31 Inbox is drained.
