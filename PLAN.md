...
- **138.6 flaky test (BLOCKED):** `MutualFollow_BothFollowersCollections_ListTheOther` passes in isolation (329 ms) but flakily times out under full-suite load (60 s timeout exceeded). Root cause unknown — the delivery round trip (A→B Follow → B Accept → A) does not complete within 60 s when the full suite runs, likely due to GC pressure or thread pool starvation affecting the in-process `TestServer`. The implementation is correct (proven by the test passing in isolation). **Next step:** investigate whether the `DeliveryWorker`'s retry budget (5 attempts, 31 s total) is exhausted under load, or whether the circuit breaker is open from a prior test. If the retry budget is the issue, consider increasing it for this test or making the test deliver synchronously (bypassing the `DeliveryWorker`).
...
- **138.6 (mutual peering handshake) — both /following and /followers populate on both sides:** `AcceptActivityHandler` community branch now records both `AddFollowAsync` and `AddFollowerAsync` (symmetric with `FollowActivityHandler`), so the inbound Accept finalizes the followers edge. `RemoteActorCache`/`RemoteKeyCache` gained `Clear()`; `SharedTwoHostFixture.Reset()` is virtual and the 138.6 fixture override clears both hosts' remote caches. 3 new integration tests (`MutualPeeringHandshakeIntegrationTests`) on a cross-wired two-host fixture verify both /following, both /followers (the auto-accept round trip), and the peer Group persisted on both sides. Root cause of the delivery `TransportError`: `SeedForFixture` in `InitializeAsync` re-seeded community keys, disposing the RSA material in the `IdentityKeys` key store via `InMemoryKeyStore.PutKey`'s replace-and-dispose; fixed by re-seeding via `SeedCommunityWithExistingKey`. [Plan](docs/plans/phase-138-lemmy-community-integration.md).
- **138.5 (Iris → Lemmy community discovery) — cross-instance remote `Group` follow + persist:** an integration test (`CommunityFollowRemoteGroupDiscoveryIntegrationTests`, 3 tests) proves the existing `POST /local/v1/c/{name}/follow/{targetIri}` endpoint drives a genuine cross-instance discovery — an Iris community follows a remote `Group` on a non-Iris instance, and the delivery's inbox-resolution fetch persists the remote `Group` to Iris's durable community store via the 135.1 `RemoteCommunityPersister` path, served back by `GET /ap/v1/actor?iri=…`. Test-only (no source change). [Plan](docs/plans/phase-138-lemmy-community-integration.md).
