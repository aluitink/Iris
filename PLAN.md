...
- **138.7 (peering trust/identity checks) — HTTP Signature wire-compatibility with real Lemmy key:** 4 tests in `PeeringTrustIdentityTests` fetch the real Lemmy interop community's `publicKeyPem` (live, gated on the local Docker container) and verify: (1) the key loads as public-only (can verify, cannot sign — the trust boundary); (2) signature base construction is deterministic and algorithm-compatible (RSA-PKCS1v15 + SHA-256); (3) Iris-signed requests are well-formed and parseable; (4) the lowercase `sha-256=` digest convention is confirmed. The live wire test (actual Iris→Lemmy delivery accepted with 2xx) is deferred to 138.10. [Plan](docs/plans/phase-138-lemmy-community-integration.md).
