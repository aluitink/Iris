...
- **Mastodon interop tests server (in progress)** Docker scaffold committed. Instance API + WebFinger + ActivityPub actor all respond. Test user `mstest` (pw: `Password1`) created via direct SQL. **Verified:** Iris discovers `mstest@mastodon.luit.ink` via fediverse search; actor profile renders in Iris UI. **BUG FOUND + FIXED:** Iris records the Follow edge locally but does NOT deliver the outbound Follow activity to Mastodon's inbox. **Root cause:** `DefaultLocalActorResolver.IsLocalActorAsync` returned `true` for any actor in the `Actors` table, but `RemoteActorPersister` caches remote actors in that same table (for the directory's "All known" scope). So the Mastodon actor was classified as local, and the outbox handler wrote the follow to the local inbox instead of delivering it to the remote shared inbox. **Fix:** `DefaultLocalActorResolver` now checks the instance base prefix before consulting the store (commit `22a09ae`). 1330 tests green. **Remaining blocker:** the Iris Docker container cannot reach `mastodon.luit.ink` via HTTPS (the proxy only serves HTTP on port 8082; the external TLS-terminating nginx is on the host and unreachable from inside the container's Docker network). Need to add HTTPS to the Mastodon proxy or reconfigure networking. **Next:** (1) fix the container→Mastodon HTTPS path, (2) re-test follow delivery end-to-end, (3) test posting from Iris, (4) test following from Mastodon → Iris.
