...
- [ ] Phase 2 — Client Library (in progress)
  - Done: `IKeyProvider`/`InMemoryKeyProvider` (session actor→key resolution), `SigningHandler` (`DelegatingHandler`; `ClientToServer` for bodyless GETs, `ServerToServer` for body POSTs; host derived from the request URI), `WebFingerClient` + `IDiscoveryService`/`WebFingerDiscoveryService` (RFC 8410 lookup on the account's own host), `ActivityPubClient` + `IActivityPubClient`/`IActivityPubClientFactory` (signing pipeline into an owned `HttpClient`; `GetActorAsync`), and `IClientAuthenticator`/`BasicAuthClientAuthenticator` (Basic-auth → owner actor doc + loaded `KeyPair`; algorithm carried in a `keyAlgorithm` doc extension). 38 client unit tests.
  - `dotnet build Iris.slnx` 0 warnings; `dotnet test Iris.slnx` 173/173; `dotnet format` clean.
  - Next: client caching (`ActorCache`, `CollectionPageCache`, `WebFingerCache`, `KeyCache` on `ICache<T>`); rich paged collections; `JsonLdHandler`/`RetryHandler`; client ↔ `TestServer` integration tests.
