...
**Phase 156 - New Post/Compose enhancements** — *Slice A done* (client composes Mastodon-faithful `content` HTML so `@mention`/`#hashtag` tokens render as tappable `<a>` links, plus the structured `tag` array). **Remaining slices (next turns):** (Slice B) display-side linkify pass in `ObjectView` for incoming remote notes that carry plain-text mentions/hashtags but no inline HTML (so remote notes get tappable links too); (Slice C) backend resolution of `@handle@domain` / `#hashtag` tokens in incoming notes that lack matching `tag`/`cc` references, so new mentions/hashtags resolve. Also an open item: the `Mention` tag currently serializes as a compact IRI string in the stored document (server ingestion normalization, pre-existing) rather than the full `{type:"Mention",href}` object — worth confirming interop parity with remote Mastodon. See docs/changes/1561-….
