...
None — Phase 28 is closed. The protocol edge-case grind (Phases 19–28) is largely done; the live priorities are now **sample-UI review** (the one user-facing surface never given a full functional + visual pass) and **test-suite speed** (partially addressed: a `Category=Slow` trait + fast/full run convention; the structural host-reuse follow-up is noted in [TESTING.md](docs/reference/TESTING.md#running-the-suite-fast-vs-full)).
