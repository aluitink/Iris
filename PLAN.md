...
1. **General UI/UX review** (recurring) — Next pass after improvements land. Pass 7 (2026-09-19): post-S5b verification — authless pass (all 8 authenticated routes redirect to `/login`, 404 + register accessible, 0 console errors) + signed-in spot-check (Home, Communities, Community detail hard-refresh stable, Profile, Compose deep-link hard-refresh stable), 0 new defects (only console errors were remote-instance availability — lemmy.ml 500, lemmy.luit.ink 403 — not Iris bugs). [change doc](docs/changes/997-ui-ux-review.md)
2. **139.2 Scenario 5 (audience/visibility) — remaining deferred surfaces** — the S1 read-path gap is now **closed for the public feed, global search, the follow feed, the object-document endpoint, and the federation read path**. The **follow-feed owner gate** (S5c) is done (403 for non-owners). The **object-document visibility gate** (S5a) is done (404 for non-recipients on non-public local content). The **federation visibility policy** (S5b) is now **done**: non-public federated content is stored on receipt with `to`/`cc` intact (no suppression) and hidden on the read path by the S5 filter — pinned by cross-instance integration tests. Remaining (deferred product decisions, not defects): (1) object-document visibility for **federated-in** non-public content (cross-instance deep links still serve the full object — interacts with federation's legitimate need to fetch non-public content); (2) a `cc`-to-follower-set model for followers-only posts. See [change doc](docs/changes/1392-5b-federation-visibility-policy.md).
...
*(Pass 2 of the recurring UI/UX review, the 139.4 UI/UX & accessibility review, plus Scenarios 2, 3, 4, 6, 7, 8 of 139.3, the s6-F1/s6-F2 follow-ups, and 140.1, are in [docs/ROADMAP.md](docs/ROADMAP.md) — see the one-liner entries there.)*
