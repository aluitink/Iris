...
- **73 — Compose feature completion (COMPLETE).** All 6 slices done: 73.1 (poll creation), 73.2 (multi-media), 73.3 (audience selector), 73.4 (community media+CW), 73.5 (cross-instance mentions), 73.6 (poll voting). Change docs: [731](docs/changes/731-poll-creation.md) · [732](docs/changes/732-multiple-media-attachments.md) · [733](docs/changes/733-audience-selector.md) · [734](docs/changes/734-community-media-cw.md) · [735](docs/changes/735-cross-instance-mention-resolution.md) · [736](docs/changes/736-poll-voting.md). Build 0 warn/0 err; full suite: 1665 passed, 0 failed, 17 skipped.
...
- 74.1 — Sensitive content blur should also include the image (blur both text and image when CW is present)
- 74 — Better Mastodon compatibility: standardize activity minting to match Mastodon's wire format. See `docs/references/ExampleWithTagAndMentionAndAttachedImage.json`. Walk random outboxes, explore RayvenMX@mastodon.world followings, use the UI to drive this effort.
