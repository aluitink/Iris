...
- **S5 — drop the stale orphaned `localhost` actor from Search (dev, 2026-09-20).** Implemented + live-verified; **awaiting commit.** `GlobalSearchService.IsSameInstanceActor` now drops a local actor (carries a `preferredUsername`) whose IRI is not under the instance base IRI — the stale `localhost:8088` ghost is gone from the mixed (Search) path, the canonical public-base row remains, and remote actors are unaffected. [change doc](docs/changes/1574-search-drop-stale-local-actor-foreign-base.md)
...
- **17 open. No blockers (all S2/S3-sev).** (Pass 27: S4 local-community following, S6 remote join, S8 "All on this instance", and S16 poll-vote persistence **fixed + live-verified**; S4's *remote* community display caveat remains — [s04](docs/qa/s04-communities-following-remote.md).)
- **Top priority:** S18 (local follow → empty Home timeline; data-integrity, breaks the core follow loop), S19 (community Requests tab dead-end — moderation entry point broken), S2/S14 (signed-out proxy bypass), then the S2-sev bugs S5, S7, S9, S11, S12, S17; S3-sev S3, S10, S13, S15.
- **Last pass:** 30 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — next pass re-verifies S2/S14 once dev's anonymous-proxy rate-limiter WIP is committed+rebuilt, then fresh areas (Search page, "Edit profile" flow, media attachments, Communities "Create a community" re-verify).
