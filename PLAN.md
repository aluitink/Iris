...
- **115.9 — (candidate)** — Continue D-column matrix reconciliation: pick the next cluster of implemented-but-unverified D items — the **Compose & content** rows (text post, reply, mentions, media, CW, delete, edit, rich attachment) or the **Timeline/feed** rows (home feed, actor outbox, community feed). Live-verify via Playwright + reconcile. Replenish from `docs/plans/production-app-feature-matrix.md` ☐ rows (~24 remain).
- **115.10 — (candidate)** — If a D item turns out to be a *genuine* web/server gap (not just unreconciled), implement + fix it as a vertical slice (impl + tests + live-verify). (115.5 was exactly this: a live search 500.)
