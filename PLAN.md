...
- **Dev2 stack:** `dev2-iris-a` + `dev2-iris-b` healthy (fresh build from `98399b73`, 2026-09-22 — S24-D2 outbox filter + S38 webfinger proxy + S36 home-feed fix + S17 Likes tab scoping + S22/S23 follow-notification + S39 notification delivery all deployed).
...
- **S24-D2 (foreign activities in local outbox):** **FIXED (dev2, 2026-09-22).** `GetOutboxAsync` in `EfActivityStore`, `InMemoryActivityStore`, and `FileBackedActivityStore` now filters by `activity.Actor` — only activities authored by the actor are returned; foreign (inbound) activities are excluded. 4 follow-request-queue tests updated to use the dedicated `ILocalModerationClient.GetFollowRequestsAsync` API (the production surface) instead of reading the outbox directly. 113 tests pass.
