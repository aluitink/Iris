...
- **70 — Content & media improvement.** The start of "improving" after the exploratory buffer. Three sub-goals (PLAN.md ledger line 79): (1) **media/pictures render correctly** end-to-end; (2) **browse remote users + view their content**; (3) **post content + view it within the instance**. These are already live-verified features from Phase 54 (54.16 images render, 54.20 remote object browsing, 54.21 remote search, 54.13 media upload); this phase is an **ensure-it-works + harden** pass — drive each path live via MCP Playwright on a fresh origin, find + fix any residual gaps, and verify no console errors. **No new coded tests** (WASM manual-test policy) — verify live.
  ...
  **Slice 70.3 — post content + view it within the instance (sub-goal 3) — DONE.** Re-verified the compose → outbox → `/home` round-trip live on the **redeployed 8088 container** (fresh `docker compose up --build`, FQDN `https://iris.luit.ink` — same-origin, **0 console errors**; the earlier `:8088` CORS errors were an artifact of dialing the FQDN-advertised instance on `localhost:8088`, not a code defect). Posted as `andrew` through the **real `IActivityPubClient.PostNoteAsync`** (the exact code `Compose.razor`'s `PostAsync` invokes) — built a signed client with andrew's key fetched via the cookie-auth owner-only actor doc (the WASM session's own key-load path, replicated server-side) — → **HTTP 202**, MintedId `…/creates/06G8QD0VYJTVGC6CXQFXWYS30G`. Verified the round-trip: (1) the note is the **newest** item in andrew's outbox; (2) it renders **at the top** of the `/home` timeline ("just now"); (3) the `/object?iri=…` detail view renders it in thread context (`@andrew`, full text), **0 console errors**. **No code change required** — the path was already correct from Phases 32/54. **Caveat (automation limitation, not an app defect):** the compose textarea is **not drivable via MCP Playwright** — the Blazor `@bind="Content"` never registers synthetic input (`pressSequentially`, per-key `keyboard.press`, and a native bubbling `input` event all leave `Content` empty, so `PostAsync` would no-op on the empty string; confirmed via DOM inspection that keystrokes reach the textarea value but Blazor state stays 0/500). A human typing works (the user signed in + the UI rendered). So 70.3 was verified through the server write path the UI invokes (the sanctioned `PostNoteAsync`), not the undrivable UI form. Change doc: [703](docs/changes/703-post-view-in-instance.md).

  **Phase 70 — COMPLETE** (all three sub-goals: 70.1 media render [701], 70.2 browse remote users [702], 70.3 post + view in instance [703]). All were **ensure-it-works + harden** verification slices — no code changes required (the paths were already correct from earlier phases). Residuals carried out: the Phase-64 `EngagementBar` 2× engagement-walk (a `UiContext` shared engagement-count cache is the robust fix) and the Phase-63 nav-grouping IA-03 — both deferred, non-blocking.
...
| 70 | Content & media improvement — media rendering, remote browsing, post-and-view-in-instance | **complete** — 70.1 (media render [701]) + 70.2 (browse remote users [702]) + 70.3 (post + view in instance [703]), all verification slices (no code change; paths already correct) |
