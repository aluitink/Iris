...
1. 29.1: **delivery-worker retry budget is configurable per-endpoint** — the `DeliveryWorker` uses a fixed `DeliveryRetryOptions` (5 attempts, 1s base) resolved via `IOptions<T>`; a later `AddSingleton<T>` does not override the cached instance (a known footgun from 26.5). Investigate whether per-endpoint or per-actor retry budgets are needed for operational hardening, or whether the single global budget is sufficient (and document the decision).
