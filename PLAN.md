...
Phases 0–9 are complete. Phase 8 delivered the server-side Docker foundation; Phase 9 (Real-World Deployment Preparation) is complete — all five deliverables done and grounded in the real config/client/server/test surface. **Phase 10 (Project & Test Review) is in progress** — Slices 1.1, 1.2, and 1.3 are done: removed the fully-dead `CommunityIris.cs` + `LazyActorDocumentFetcher.cs` (both 0 callers), consolidated the triple-duplicated `IObjectOrLink`→`Iri` resolution into `IriExtensions.ResolveObjectIri`, moved `CollectionPage` to `Iris.Core` + added `CollectionPageFactory` + `IriExtensions.ResolveCollectionIri` (eliminated duplicated page-flatten + `TryGetIri` in client & server), and consolidated the parallel `CachingClientCache<T>`/`CachingServerCache<T>` into a single `CachingReadThrough<T>` in `Iris.Core` (migrated all 10 typed façades) (444→466, 22 new tests). Per-slice detail (what was built, key types, test counts, resolved decisions) lives in [docs/CHANGELOG.md](docs/CHANGELOG.md); the phased plan and open questions are in [docs/ROADMAP.md](docs/ROADMAP.md).
...
| 10 — Project & Test Review | 🚧 in progress | Slice 1.1 done (test audit + dead-code sweep + IRI-resolution dedup: removed dead `CommunityIris.cs` + `LazyActorDocumentFetcher.cs`, consolidated triple-duplicated IRI resolution into `IriExtensions.ResolveObjectIri`, 5 new tests, 444→449). Slice 1.2 done (page-flatten + collection-IRI dedup: moved `CollectionPage` to `Iris.Core`, added `CollectionPageFactory` + `IriExtensions.ResolveCollectionIri`, eliminated duplicated `TryGetIri` + page-flatten in client & server, 11 new tests, 449→460). Slice 1.3 done (parallel cache engine consolidation: `CachingClientCache<T>` + `CachingServerCache<T>` → single `CachingReadThrough<T>` in `Iris.Core`, migrated 10 typed façades, 6 new tests, 460→466). Remaining: near-identical handler stacks, harness consolidation, API surface pass, doc sync. |
