...
**54.9: Possible duplicate home-feed fetch** — investigated. `Iris.Web.Client` is **pure Blazor WASM** (no server prerender), so the "prerender + circuit double-mount" hypothesis is ruled out. Found a real flaw in [PagedCollection.razor](apps/Iris.Web.Client/Components/PagedCollection.razor): `OnParametersSet` reset `_loadedFor = null` whenever `Client` was null, and the fire-and-forget initial load had no in-flight guard — so a transient null-`Client` re-render between dispatch and fetch completion re-fired a duplicate `GET /feed`. **Fix (implemented, build clean, `Iris.Web.Tests` green, Docker redeployed):** removed the harmful reset + added a `_loadingInitial` in-flight guard (set on dispatch, cleared in `finally`), so exactly one initial fetch per distinct collection. [changes/382](docs/changes/382-54.9-duplicate-home-feed-fetch.md)

`remaining:` **live Playwright verification PENDING** — the MCP Playwright browser crashed during a prior timed-out `run_code_unsafe` and was not restarted (the harness-managed MCP server process is gone). On the next turn, load `/home` in a fresh browser context (cache disabled) and confirm the network tab shows **exactly one** `GET /ap/v1/u/{actor}/feed` for the first-page load (and that navigating to a different actor still fetches its feed once). Once verified, move to Recently Completed.
