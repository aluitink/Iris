...
- **6 open. No blockers (all S2/S3-sev).** (Pass 71: S4 re-confirmed — remote interop missing (11th pass). S21 /c/{handle} still broken — redirect not active (8th pass).)
- **Top priority:** S21 (/c/{handle} route still broken — redirect page not active; auto-follow FIXED), S19 (requests endpoint missing + no Accept/Decline in notifications + Edit community Save silent no-op — requireApproval NULL in DB), S20 (Home feed Communities tab non-functional — visual-only toggle, 0 new API requests — 5th pass), S4 (remote communities missing from Following tab — 11th pass), S17 (profile tabs over-fetch — 32 outbox requests for 3 tabs), S16-UX (poll "You voted" badge missing on object detail page — vote count consistent). S2 (proxy 401 — 2 errors, no change) / S14 (proxy 401 + CSP on actor detail — identical to Pass 32) still blocked on dev.
- **Last pass:** 71 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S21 PARTIALLY FIXED (auto-follow works, /c/{handle} redirect not active); S2 partially improved (11→2 errors); S16-UX partially improved (vote count consistent, badge visible in profile). Remaining open: S2, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21 (/c/{handle} only).
