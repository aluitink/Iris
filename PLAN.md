...
Phases 0–9 are complete. Phase 8 (Sample Docker Composition) delivered the server-side Docker foundation (SampleServer Dockerfile + a two-instance compose stack + smoke test), verified. Phase 9 (Real-World Deployment Preparation) is complete — all five deliverables are done and grounded in the real config/client/server/test surface: FQDN & TLS plan + bootstrap runbook (`docs/DEPLOYMENT_PREP.md`), real-user enumeration design (`docs/ENUMERATION_DESIGN.md`), compatibility matrix (`docs/COMPATIBILITY_MATRIX.md`), test-harness extension design (`docs/INTEROP_TEST_HARNESS.md`), and risk & gap register (`docs/RISK_GAP_REGISTER.md`). **Phase 10 (Project & Test Review) is next.** Per-slice detail (what was built, key types, test counts, resolved decisions) lives in [docs/CHANGELOG.md](docs/CHANGELOG.md); the phased plan and open questions are in [docs/ROADMAP.md](docs/ROADMAP.md).
...
| 10 — Project & Test Review | 🚧 next | Full audit: remove redundant/useless tests, consolidate code (incl. hoisting the ~9 copy-pasted per-test federation helpers into `Iris.Testing` per the Phase 9 harness design), dead-code sweep, API surface pass. |
