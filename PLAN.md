...
- **84.1 — Durable dead-letter store (Active Slice).** The dead-letter queue (deliveries that exhausted their retry budget, surfaced by 83.3's `GET /ap/v1/dead-letters`) is **in-memory only** (`InMemoryDeliveryDeadLetterStore`) — a server restart loses every dead-lettered delivery, so an operator cannot inspect what failed to federate across a restart. Add a **durable** `IDeliveryDeadLetterStore` (file-backed +/or EF-backed, mirroring the existing `IKeyStore`/persistence-store pattern) that survives restarts: the same `AddAsync`/`Count`/`ListAsync` contract, bounded capacity (evict-oldest), newest-first listing. Wire it into `AddActivityPubServer`'s delivery DI (config-driven: file-backed when a path is set, EF when a connection is set, in-memory default — mirroring `Iris:ConnectionString`'s persistence selection). Coded tests (server + data): a delivery that dead-letters is persisted; a fresh provider over the same backing store reads it back; capacity eviction + newest-first survive a "restart" (new store instance).
...
- **84 — Production hardening & scale-out readiness (NEW).** Phase 83 made the instance identity-correct + observable + degrade gracefully; Phase 84 makes it **deploy at scale** — the remaining operational gaps: durability across restarts, distributed state for multi-instance, and the key-rotation lifecycle. Coded tests for server/data-side behavior.
  - **84.1 — Durable dead-letter store (seed).** The dead-letter queue (83.3's `GET /ap/v1/dead-letters`) is in-memory only — a restart loses every dead-lettered delivery. Add a durable `IDeliveryDeadLetterStore` (file-backed +/or EF-backed, mirroring the `IKeyStore`/persistence-store pattern): same `AddAsync`/`Count`/`ListAsync` contract, bounded capacity (evict-oldest), newest-first. Config-driven DI (file when a path is set, EF when a connection is set, in-memory default). Tests: a dead-lettered delivery is persisted; a fresh provider over the same backing store reads it back; capacity eviction + newest-first survive a "restart".
  - **84.2 — Local key-rotation lifecycle (seed).** 82.3 documented local key-rotation as a follow-up. Add the lifecycle: rotate the local actor's signing key (mint a new key, advertise it via the actor document's `publicKey` + `Move`/key-rotation so peers learn the new key, retire the old key once the rotation is confirmed) + the `#key-N` addressing the fragment-aware `Iri` (83.1) + key stores (82.3) now support. Tests: rotation produces a resolvable new key; the actor document advertises it; old-key signatures still verify during the overlap window.
