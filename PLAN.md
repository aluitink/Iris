...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` + `SharedTwoHostFixture` + per-method reset + `ServerRefFor` are in place. **Thirteen RISKY single-instance classes converted** (all single-instance RISKY classes done) + **fourteen two-instance federation classes converted**: UpdatePropagation (3), LikeAnnounceUndoPropagation (2), MuteUndoPropagation (1), CrossInstanceAcceptPropagation (2), CrossInstanceRejectPropagation (2), ModerationUndoPropagation (2), RelayLifecycle (1), PersonFollowsPersonUnfollowPropagation (2), CommunityFollowsPersonUnfollowPropagation (2), CommunityFollowsCommunityUnfollowPropagation (2), MoveExtensionRoundTrip (1), DuplicateInboundDeliveryIdempotency (2), RelayFanOut (3), PostFederation (2). Full fast suite green (899/899, 5m6s). **remaining:** ~8 two-instance federation classes (Outbox*FanOut, OutboxAudience*, UpdateDeleteRelayFanOut, AnnouncePropagation with DeliveryCounter, OutboxCreateFanOut with 3-instance topology). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/258](docs/changes/258-29.3-signature-validation-risky-classes.md).
...
- 29.3 (follow-up, partial 8): **2 two-instance federation classes** — converted `MoveExtensionRoundTripIntegrationTests` (1, B fetcher reaches A, bob→alice follow edge) + `DuplicateInboundDeliveryIdempotencyIntegrationTests` (2, A routing fetcher + identity, B identity + fetcher + delivery to A). **All 13 single-instance RISKY + 12 two-instance federation classes converted.** Remaining: ~10 two-instance federation classes (Outbox*FanOut, Relay*FanOut, PostFederation, etc.). Full fast suite green (899/899, 5m38s). → [docs/changes/262-29.3-move-ext-dup-delivery-classes.md](docs/changes/262-29.3-move-ext-dup-delivery-classes.md)
