...
- **Directory Paging (dev1, in progress).** Add infinite scroll to the WASM Directory page so remote actors beyond the first server page load on demand. Server already pages (`GET /ap/v1/search` → `next` link + `totalItems`), so this is client + UI. Client: `SearchPagedAsync`/`GetSearchPageAsync`/`BuildSearchIri` + extension link/totalItems readers in `Iris.Client` (the `next` link and `totalItems` are read from `ExtensionData`/the `Collection` base since `OrderedCollection` exposes neither). UI: `Directory.razor` holds one `IAsyncEnumerator<SearchPage>` across renders; initial `LoadActorsAsync` + `LoadMoreAsync` (sentinel + "load more" fallback, matching `ActorListPanel.razor`). Also fixed `CookieAuthenticationStateProvider` to re-resolve auth on every read (it permanently cached the first result, so a circuit that first rendered signed-out stayed signed-out). Tests: `SearchPagedAsyncTests` (client, 4/4) + `ClientSearchPagedAsync_WalksAllPages_AgainstLiveEndpoint` (server integration). Build green; fast suite green except pre-existing `Iris.LiveInterop.Tests` failures (verified failing on clean `main`). Awaiting live UI verification over `https://dev1-iris-a.luit.ink`.
