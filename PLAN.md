...
- **Phase 81 — Federation hardening & real-world interop (ACTIVE; 81.1 DONE).** The app is feature-complete + polished; the next value is making it federate robustly with real-world AP servers. 81.1 (Mastodon) done — 4 real-fixture round-trip tests, no code changes needed (Iris handled real Mastodon wire format correctly). Next: 81.2 (Misskey + Pleroma), 81.3 (pagination/outbox conformance). Coded tests for server-side wire-format behavior (fixture-capture + round-trip pattern, per 79.2/81.1).

## Active Slice

- **81.2 — Live-server interop sweep: Misskey + Pleroma (next).** Capture real wire-format documents from a permissive live Misskey + Pleroma instance (same fixture-capture pattern as 81.1 — the host can reach them; the container's egress is unreliable per 79.2). Misskey: a note (with `reactions`/`renote`/`antennas`/`emoji` extensions), a renote (Announce with Misskey fields). Pleroma: a toot + an announce (Pleroma-specific extensions in `ExtensionData`). Add coded round-trip tests in `Iris.Core.Tests` (deserialize → `IsType` → extractors → serialize round-trip). Fix any wire-format gap found. **Note:** `mastodon.online` was permissive in 81.1; find equivalent permissive Misskey/Pleroma instances (or use their public timelines / a permissive mirror).
...
- **81 — Federation hardening & real-world interop (NEW).** The app is feature-complete + polished; make it federate robustly with real-world AP servers. Test against live servers and fix wire-format/interop gaps. Playwright-driven where UI-facing; coded tests for server-side wire-format behavior (the established interop-verification pattern).
  ...
  - **81.2** — Live-server interop sweep: **Misskey + Pleroma** (inbound: Misskey notes/renotes/antennas, Pleroma toots/announces; outbound: Iris content renders correctly on those clients). Same fixture-capture + round-trip pattern as 81.1 (capture real wire-format documents from a permissive live instance, add coded tests in `Iris.Core.Tests`).
  - **81.2** — Live-server interop sweep: **Misskey + Pleroma** (inbound: Misskey notes/renotes/antennas, Pleroma toots/announces; outbound: Iris content renders correctly on those clients).
...
- **80.1 — Phase 80 full-route defect hunt — DONE** ([800](docs/changes/800-phase80-bug-hunt-tracker.md)): all 18 routes visited (signed-in `andrew` + authless), console-checked. 5 findings logged, no S1 blockers.
