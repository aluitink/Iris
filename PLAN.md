...
- **8 open. No blockers (all S2/S3-sev).** (Pass 62: S4 re-confirmed — remote `interop` community missing from Following tab (10th consecutive pass). S16-UX re-confirmed — poll "You voted" badge missing on object detail page (vote count consistent at 1).)
...
- **Last pass:** 62 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8 fixed; S18 partially fixed; S21 partially resolved (caching); S2 partially improved (11→2 errors); S16-UX partially improved (vote count consistent, badge visible in profile). Remaining open: S2, S3, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21.
