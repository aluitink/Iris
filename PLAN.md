...
- [x] Phase 3 — Server Foundation (complete)
  ...
  - Next: Phase 4 — Inbox & Delivery (inbound signature validation `SignatureValidationMiddleware`+`ISignatureValidator`, `IInboxProcessor`/`IActivityHandler<T>`, `IDeliveryService`/`IDeliveryQueue`, follow/accept/reject end-to-end, first true instance-to-instance federation test). The four remote object caches built in Phase 3 are the seam Phase 4's outbound federation paths (inbound signature validation, object delivery) will read through.
