...
- **8 open. No blockers (all S2/S3-sev).** (Pass 63: S19 re-confirmed — requests endpoint still 404s on `technology`; UI error without API call; no retry button. Now tested on 3 communities.)
...
- **Last pass:** 63 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8 fixed; S18 partially fixed; S21 partially resolved (caching); S2 partially improved (11→2 errors); S16-UX partially improved (vote count consistent, badge visible in profile). Remaining open: S2, S3, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21.
