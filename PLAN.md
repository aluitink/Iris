...
- **90 — Improve the Feed (reply context + recipient line + boosts) (DONE)** — reworked the shared `ObjectView` post card: a reply fetches its "in reply to" parent and renders it as a muted context card above the body (one-line link fallback); the `To`/`Cc` audience now renders as a `To:` line at the top (email-style, moved from the bottom); a boost (Announce) renders the boosted post inline with its own author header under a small `Boosted by @booster` tag (bare-IRI remote targets fall back to a `View boosted post →` link). UI-only (`ObjectView.razor` + `app.css`), 0 new coded web tests. Live-verified via Playwright (reply context card, To: line, boost-by), 0 unexpected console errors. [changes/901](docs/changes/901-phase90-improve-the-feed.md)
