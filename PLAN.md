...
- **71 — Note card + compose polish (in progress).** Five of six slices implemented (71.1–71.5); 71.6 (review remote user content with attachments) is a live-verification pass remaining. **No new coded tests** (WASM manual-test policy) — build 0 warn/0 err; `Iris.Web.Tests` 63/63. Change doc: [711](docs/changes/711-note-card-compose-polish.md).
  ...
  **Slice 71.6 — Review remote user content with attachments — REMAINING.** Live-verification pass (no code change expected — the render path is the same `ObjectView` component). Drive a remote post with attached documents/media and confirm the object-detail view + actor-outbox render the attachments correctly.

  **remaining:** 71.6 live verification + 71.2 residual (richer inline document rendering).
...
| Phase | Role | Status |
|---|---|---|
| 71 | Note card + compose polish — card built from all content, inline attachments, interaction bar at bottom, sensitive-blur, mention dropdown | **in progress** — 71.1/71.3/71.4/71.5 done; 71.6 remaining |
...
- 71.6 — Review **external/remote user content with attachments** (object-detail + actor-outbox render of remote attached documents/media; example: RayvenMX status `…/statuses/116182691592848717`). **Unblocked** — login now works normally (was stale Playwright context state).
- 71.2 (residual) — Richer inline document rendering (the document card is a link-card; a fully inline document preview is deferred).
