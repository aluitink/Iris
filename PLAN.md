...
- **Still open:** **S24-D2 (outbox-integrity facet)** — the *feed-surface* facet is closed (S36), but `ii-a1`'s raw local outbox still physically contains **17 foreign `ii-b1` items** (9 Like + 8 Create, pages 1–5). The write path still records foreign Create/Like into the followed actor's local outbox. Dev-owned; QA documents in [s24](docs/qa/s24-cross-instance-follow-state-inconsistent.md). **S3** (object-detail 404) FIXED; its residual **Like/Boost-button count UI facet is CLOSED** (live-re-verified 2026-09-22 on dev1 — the buttons now read the denormalized `likedCount`/`sharedCount` and update live). **S21** — fixed in code, awaiting QA re-verify.
...
- **CLOSED:** S2, S4, S14, S17, S19, S20, S24-D4, S26, S27, S28, S29, S30, S31, S32, S33, S34, S36, S37, S38, S39.
