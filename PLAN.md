...
- **8 open. No blockers (all S2/S3-sev).** (Pass 54: S2 partially improved — error count reduced 11→2, no more CORS/direct-fallback noise; proxy 401 for unsigned Mastodon GETs persists. S14 re-confirmed — proxy 401 + CSP violations on actor detail, identical to Pass 32.)
- **Top priority:** S3 (Create-IRI 404 — Create activities not persisted to DB — 5th pass), S21 (no auto-follow on community creation + /c/{handle} route missing), S19 (requests endpoint missing + no Accept/Decline in notifications + Edit community Save silent no-op), S20 (Home feed Communities tab non-functional — server 403s `?source=` param), S4 (remote communities missing from Following tab + Directory shows "Join" for followed remote), S17 (profile tabs over-fetch — 34 outbox requests for 3 tabs), S16-UX (poll vote count inconsistent + badge missing everywhere). S2 (proxy 401 — partially improved, 11→2 errors) / S14 (proxy 401 + CSP on actor detail) still blocked on dev.
- **Last pass:** 54 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8 fixed; S18 partially fixed; S21 partially resolved (caching); S2 partially improved (11→2 errors). Remaining open: S2, S3, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21.
