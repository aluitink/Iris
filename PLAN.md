...
**Phase 19.6.1 (change 161k): client community-maintenance one-call methods** — added `AddMemberAsync`/`RemoveMemberAsync` (an `Add`/`Remove` with `actor` = the community, delivered directly to the community's own inbox through the signed pipeline; create-community deferred — no federated activity type/route exists). Three new integration tests pin the round-trips. Suite 1,261, 0 failed.
