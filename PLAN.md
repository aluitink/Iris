...
- **Phase 81 — Federation hardening & real-world interop (NEW, ACTIVE).** The app is feature-complete + polished; the next value is making it federate robustly with real-world AP servers. Test against live servers (Mastodon.social, Misskey, Pleroma, PeerTube, Lemmy) and fix any wire-format/interop gaps found. Playwright-driven where UI-facing; coded tests for server-side wire-format behavior. See Up Next for slices 81.1–81.3.

## Active Slice

- **81.1 — Live-server interop sweep: Mastodon.social (in progress next).** Test Iris against a live Mastodon instance: (a) inbound — fetch + render a real Mastodon toot (with media, CW, mentions, hashtags, emojis, edited posts, polls); (b) outbound — Iris posts/boosts/replies visible + correct on Mastodon; (c) WebFinger/Web login interop. Log every wire-format/interop gap as a finding (class + severity) in a new Phase 81 tracker; fix what's in scope; re-verify live. **Note:** live cross-instance testing may be constrained by Docker network isolation (as in 79.2) — if so, fall back to capturing real Mastodon wire-format fixtures (public toot JSON) and round-tripping them through Iris's deserializer (coded tests in `Iris.Core.Tests`), which is the established pattern for interop verification.
...
- **81 — Federation hardening & real-world interop (NEW).** The app is feature-complete + polished; make it federate robustly with real-world AP servers. Test against live servers and fix wire-format/interop gaps. Playwright-driven where UI-facing; coded tests for server-side wire-format behavior (the established interop-verification pattern).
  - **81.1** — Live-server interop sweep: **Mastodon.social** (inbound render of a real toot with media/CW/mentions/hashtags/emojis/edited/polls; outbound post/boost/reply visible + correct on Mastodon; WebFinger/login interop). If Docker network isolation blocks live testing (as in 79.2), fall back to capturing real Mastodon wire-format fixtures and round-tripping them through Iris's deserializer (coded tests in `Iris.Core.Tests`).
...
- **Phase 80 — UI/UX polish + defect hunt — COMPLETE** ([800](docs/changes/800-phase80-bug-hunt-tracker.md) · [801](docs/changes/801-phase80-defect-hunt-fixes.md) · [803](docs/changes/803-phase80-p004-object-detail-parent-preview.md) · [804](docs/changes/804-phase80-broader-ux-polish-verification.md)): fresh full-app Playwright defect hunt over all 18 routes; 5 findings (P-001…P-005) all resolved — P-001 `ObjectView` `?? parent.Value` literal, P-002 `Compose` `?? "community"` literal, P-005 admin-page login redirect, P-004 object-detail parent preview; P-003 wontfix-by-design. Broader UX polish (empty/loading/error states + a11y) verified in excellent shape — no code changes (0 unnamed buttons, 0 textless links, 1 decorative avatar with correct `alt=""`). Build 0 warn/0 err; suite 0 failed. **Next: Phase 81 (federation hardening).**
- **80.3 (defects) — P-004 object-detail parent preview — DONE** ([803](docs/changes/803-phase80-p004-object-detail-parent-preview.md)): the object-detail "in reply to" link now shows the parent's content preview (not the raw IRI). Verified from a clean entry; build 0 warn/0 err; suite 0 failed.
- **80.2 — Phase 80 bug-class fixes — DONE** ([801](docs/changes/801-phase80-defect-hunt-fixes.md)): fixed + verified P-001 (`ObjectView` `?? parent.Value`), P-002 (`Compose` `?? "community"`), P-005 (admin-page `[Authorize(Policy="Admin")]` removed → friendly "not an admin" message). P-003 wontfix-by-design; P-004 deferred to 80.3.
- **80.1 — Phase 80 full-route defect hunt — DONE** ([800](docs/changes/800-phase80-bug-hunt-tracker.md)): all 18 routes visited (signed-in `andrew` + authless), each console-checked. 5 findings logged (P-001…P-005), no S1 blockers.
- **79.3 — Remaining interop gaps (Phase 79, slice 3) — DONE** ([793](docs/changes/793-pleroma-peertube-interop.md)): Pleroma inbound verified clean (ExtensionData round-trip); fixed PeerTube `Video` self-contained media rendering (`GetSelfMediaIri` + ObjectView player). 3 new tests. **Phase 79 COMPLETE.**
