...
- **83.3 — Outbound delivery retry + dead-letter observability (Active Slice).** Harden the outbound delivery worker: (1) verify the retry/backoff + dead-letter behavior is correct + tested for the non-permissive-peer case (a 401/403 from a strict instance should dead-letter, not retry forever); (2) expose the dead-letter queue (count + peek) via the health/observability surface from 83.2; (3) a coded integration test that a delivery to a signature-rejecting peer is dead-lettered after the retry budget (not retried indefinitely). Coded tests (server).
...
- **83 — Iri identity correctness + production readiness (NEW).** Phase 82 made outbound federation conformant + hardened key management; 82.3 found that `Iri` equality is fragment-blind (correct for `#Public`/audience IRIs, wrong for key IRIs) and fixed the key stores surgically. Phase 83 does the larger, per-usage `Iri` identity-correctness work + moves to production readiness (deployment hardening, observability, config validation). Coded tests for core/server-side behavior.
  ...
  - **83.3 — Outbound delivery retry + dead-letter observability (seed).** Harden the outbound delivery worker: (1) verify the retry/backoff + dead-letter behavior is correct + tested for the non-permissive-peer case (a 401/403 from a strict instance should dead-letter, not retry forever); (2) expose the dead-letter queue (count + peek) via the health/observability surface from 83.2; (3) a coded integration test that a delivery to a signature-rejecting peer is dead-lettered after the retry budget (not retried indefinitely). Coded tests (server).
...
- **81.3 — Pagination + `Delete`/`Tombstone` conformance — DONE** (pruned to [ROADMAP.md](docs/ROADMAP.md)): 8 `Iris.Core.Tests` round-trip tests + 4 `Iris.Server.Tests` `last` assertions; 2 conformance fixes (server `last` link + client inbox `orderedItems`). [changes/813](docs/changes/813-phase81-pagination-delete-tombstone-conformance.md)
