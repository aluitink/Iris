...
**Active focus: Phase 19.0b — AP-native rework (PROPOSAL, plan written, awaiting review).** Plan: `docs/changes/161-ap-native-rework-plan.md`. Directive: make Iris fully AP-native — all actor/community activities (follow Accept/Reject, mute, block, flag, …) flow through the actor's **outbox** (client authors → outbox POST → server delivers); the AP client becomes a **pure protocol layer**. Investigation found the code is *closer* to AP-native than the docs suggest (the client already posts every activity to the outbox; the `IActivityPubClient` docs are stale). The rework: (A) outbox gains Accept/Reject branches + new AP-core client `AcceptAsync`/`RejectAsync`; (B) remove the `follows/`/`mutes/`/`relays/` Iris-only POST endpoints + handlers; (C) split the convenience methods off the core `IActivityPubClient` into a `LocalModerationClient`. **This gates 19.1.2 (F1) because it changes the follow Accept/Reject mechanism.**
