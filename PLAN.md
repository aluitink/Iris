...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` + `SharedTwoHostFixture` + per-method reset + `ServerRefFor` are in place. **Thirteen RISKY single-instance classes converted** (all single-instance RISKY classes done) + **four two-instance federation classes converted**: UpdatePropagation (3), LikeAnnounceUndoPropagation (2), MuteUndoPropagation (1), CrossInstanceAcceptPropagation (2). Full fast suite green (899/899, 5m43s). **remaining:** ~18 two-instance federation classes (cross-wired delivery transports, reset covers both instances; some have `DeliveryCounter` or 3-instance topologies that need extra handling). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/258](docs/changes/258-29.3-signature-validation-risky-classes.md).
...
- 29.3 (follow-up, partial 4): **Signature-validation RISKY classes** — converted the last four single-instance classes that use HTTP signature validation (`BlocksCollectionIntegrationTests` 8, `FlagsCollectionIntegrationTests` 7, `ReplyIntegrationTests` 7, `CommunityJoinRequestIntegrationTests` 10) to the shared host. Key fix: the key store is preserved across `Reset()`, so `SeedPersonWithExistingKey` / `SeedCommunityWithExistingKey` (new) restore actors/communities with the *same* key instance the self-fetcher holds — no signature mismatch. **Thirteen RISKY single-instance classes converted total; all single-instance RISKY classes done.** Full fast suite green (899/899, 5m37s). → [docs/changes/258-29.3-signature-validation-risky-classes.md](docs/changes/258-29.3-signature-validation-risky-classes.md)
