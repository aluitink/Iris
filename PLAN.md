...
**Phase 31 — explorer UI & server namespace polish.** 31.1 (WebFinger RFC 7033 host handling + client base-URI retry) and 31.2 (`PagedCollection` initial-load re-render + error-state spinner guard) complete — the fire-and-forget first page now re-renders the card without a manual Refresh click. Next: 31.3 (sample should not seed fake `remote.example` activities). Phase 30 (30.2 health check + readiness probe) resumes after the Phase 31 Inbox is drained.
