...
2. **139.2 Scenario 5 (audience/visibility) — deferred high-priority follow-up** — the Phase 139 closeout flagged this as an S1 privacy gap: Iris uses pure AS2 `to`/`cc` addressing with no first-class visibility field, and the read paths (public feed, follow feed, community feed, outbox GET, both searches) do NOT filter by visibility — a DM post is visible in the public timeline, every follower's feed, and search. Scope: decide + implement a visibility model (or document the intentional limitation) and filter the read paths. (From the Phase 139 closeout; the top tracked high-priority deferred item now that 139.3 is complete.)
...
*(Scenarios 2, 3, 4, 6, 7, 8 of 139.3, plus the s6-F1/s6-F2 follow-ups, are in [docs/ROADMAP.md](docs/ROADMAP.md) — see the one-liner entries there.)*
