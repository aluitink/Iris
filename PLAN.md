...
- **82.1 — Outbound signature conformance (offline, deterministic) (next).** Verify Iris's `ISignatureSigner` produces spec-correct HTTP `Signature` headers (W3C HTTP Signatures / ActivityPub `draft` profile): correct `keyId` (the actor's `publicKey` IRI), base64 `signature` over the signed string, `digest` (SHA-256) on POSTs, `created`/`expires`, and the correct `headers` list per profile — `ClientToServer` (bodyless GETs: `host`/`(request-target)`/`date`) vs `ServerToServer` (POSTs: add `digest`/`content-type`). Add deterministic offline tests (no network): build an `HttpRequestMetadata`, sign with a fixed test key, assert the `Signature` header fields + verify the signature against the public key. This is the foundation that makes outbound federation to non-permissive instances work. Fix any conformance gap found. **In-scope for coded tests** (core/server-side, not web UI).
...
- **82 — Real-world signed federation + production readiness (NEW).** Phase 81 verified inbound from real AP servers; Iris already has outbound HTTP Signatures (`SigningHandler`). Verify + harden the *outbound* side so Iris federates with non-permissive instances (e.g. `mastodon.social`, which rejects unsigned fetches), then move to production readiness. Coded tests for core/server-side behavior; Playwright-driven where UI-facing.
  - **82.1** — Outbound signature conformance (offline, deterministic): verify `ISignatureSigner` produces spec-correct `Signature` headers (`keyId`, base64 `signature`, `digest` on POSTs, `created`/`expires`, per-profile `headers` list — `ClientToServer` vs `ServerToServer`). Deterministic offline tests with a fixed test key + verify against the public key. (Active Slice.)
  - **82.2** — Real-world signed delivery: verify a real signed outbound delivery (e.g. a `Follow`/`Create` to `mastodon.social`) is accepted by a non-permissive instance (the mirror of 81.1–81.3's inbound verification).
...
- **80.3 (defects) — P-004 object-detail parent preview — DONE** ([803](docs/changes/803-phase80-p004-object-detail-parent-preview.md)): object-detail "in reply to" link now shows the parent's content preview (not the raw IRI).
