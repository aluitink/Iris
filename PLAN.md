...
- **S4 — remote communities missing from the Following tab — NEXT (2026-09-20).** S2-sev QA fix, next in the queue after S2/S14 (just fixed). Remote communities a local actor follows do not appear in the Following tab. See [s04](docs/qa/s04-communities-following-remote.md) (re-confirmed 22nd pass). **S2/S14 (proxy 401 for unsigned GETs) — DONE (2026-09-20):** the anonymous seam now signs the public GET as the local instance actor; live-verified 200 on mastodon.social. See [change doc 14816](docs/changes/14816-anonymous-proxy-signs-as-instance-actor.md).
...
- **⑤ Lemmy interop unblock (2026-09-20):** (1) actor docs carry `published`; (2) a person's Follow to a *cached* remote community delivers over the wire (host-based local/remote split). New regression test fails on old code, passes on fix. [change doc 1597](docs/changes/1597-lemmy-follow-delivery-and-actor-published.md)
