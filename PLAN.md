...
**Phase 31 — explorer UI & server namespace polish.** 31.1–31.10 complete (WebFinger RFC 7033 + retry, `PagedCollection` initial-load re-render, carla fake-remote seed gated off, directory actors-only + rename, actor-detail profile header + tabs, community profile header + tabs, the shared `CollectionBrowser` component, the server `iris:` namespace derived from the public base URI + hosted namespace document, the followed-feed `/feed` verification — 6 integration tests confirm the feed returns the followed actors' data, every activity type, for local + remote follows, in-memory + file-backed, no server change — and remote-Likes recording: `LikeActivityHandler` now records the like edge when the **liked object is local** (not when the liker is local), so a remote actor's like on a local post is surfaced on the object's `/likes` collection + like count, with the symmetric `Undo(Like)` removing it). Phase 30 (30.2 health check + readiness probe) resumes after the Phase 31 Inbox is drained.
...
> **Next phase queued (after 30 + 31 close):** **Phase 32 — production social platform app.** Before starting this phase, ensure everything is committed and create a new branch. This is a new Blazor app (`Iris.Web`) that is a real, deployable social platform on top of the libraries — production persistence (EF Core + PostgreSQL), local username/password auth, a full-stack docker-compose, and the MVP feature set (timeline, compose, communities, notifications, moderation, search). Fully scoped in [docs/plans/production-app-overview.md](docs/plans/production-app-overview.md) (a 3-level plan tree — read that file first, it links to the rest). 31.9 and 31.10 (feed-drops-Create, remote-likes-not-recorded) are worth closing *before* starting Phase 32, not just for Phase 31's own sake — the new app's home timeline and engagement UI directly inherit both bugs if they're still open. 30.2/30.3 (health check + structured logging) similarly de-risk Phase 32's docker-compose health-check + production-observability needs. **Phase 32's literal step 0** (before any new code — [production-app-overview.md](docs/plans/production-app-overview.md) §6): bring down the running sample `docker-compose.yml` stack, relocate it to `samples/docker-compose.yml` (fix `build.context` `.`→`..`, drop `iris-ui`'s now-conflicting `8088:8090` mapping, update `scripts/docker-smoke-test.sh` + [docs/reference/DEPLOYMENT.md](docs/reference/DEPLOYMENT.md) paths). The production app then publishes on host port `8088` to take over the existing `https://iris.luit.ink` reverse-proxy target.
