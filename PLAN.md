...
- **132.3 — Nested reply threading in the object detail view (DONE)** — The object-detail Replies tab (132.2) now renders a *nested* thread instead of a flat list. New `ReplyThreadNode` (Iri + resolved IObject + children; `ChildCount` drives the affordance) and a recursive `ReplyNode.razor` (ObjectView card + "Show/Hide N replies" toggle + indented children, **collapsed by default**). `ObjectDetail.BuildReplyTreeAsync` walks the loaded direct replies recursively — each node's `/replies` fetched (bounded: max depth 3, 20/node, best-effort) — since the server's `/replies` serves direct replies only (a reply's own replies live under *that* reply's `/replies`). "Show more replies" + empty state preserved; CSS (`.reply-thread`/`.reply-node`/`.reply-node-children`, indent + left lineage rule) synced to both app.css copies. Live-verified (Playwright, fresh context + CDP cache-bypass): 3-level thread renders nested, collapsed by default, expand reveals the nested reply, collapse hides it, no console errors. Build clean; suite green. [changes/13203](docs/changes/13203-phase132-nested-reply-threading.md)

- **132.2 — Object details view: Replies/Likes/Shares tab bar (DONE)** — The object-detail page (`/object?iri=…`) now renders a tab bar (Replies / Likes / Shares) mirroring the actor-detail view. Each tab shows its count from the object document's `iris:repliedCount`/`likedCount`/`sharedCount` (132.1). Replies tab = the existing flat replies list (moved under the tab, "Show more replies" preserved). Likes/Shares tabs = a new `InteractionActorsPanel` that walks the object's `/likes`/`/shares` collection, resolves each `Like`/`Announce` activity's actor (handles both full activity docs and bare Link edges), and renders a paginated, hydrated `ActorCard` grid. ARIA `tablist` markup reuses the existing `index.html` roving-tabindex/arrow-key script (no new JS). Verified live (Playwright, cache-bypass context to defeat the WASM immutable framework cache): counts render correctly (`Replies (2)`, `Likes (1)` on a liked post, `Shares (1)`), Likes lists likers, Shares lists boosters, empty state shows `No likes yet.`, no console errors. Build clean; full suite green. The *nested/expandable* reply thread is a larger follow-up → 132.3. [changes/13202](docs/changes/13202-phase132-object-details-tabs.md)

- **132.1 — Interaction tracking (DONE)** — Like / boost / reply counts for local + remote objects. (A) The object-document endpoint now renders `iris:repliedCount` (was only on collection-page items) alongside `likedCount`/`sharedCount`, computed live from the reply reverse index. (B) After the proxy stores a remote (non-local) object, it walks the remote object's `/likes`/`/shares`/`/replies` (bounded, best-effort) and records the discovered likers/announcers/replies as edges in the local reverse indexes, so a proxied read shows the real counts; a non-walkable collection falls back to "serve the known edges." `ObjectDocumentRepliedCountIntegrationTests` (2) + `Proxy_GetOfRemoteNote_SyncsInteractionEdgesIntoLocalStores` (1). [changes/13201](docs/changes/13201-phase132-interaction-tracking.md)
