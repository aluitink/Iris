...
- **Phase 12 (in progress, slices 12.1–12.15):** Wave 1 is complete (`sharedInbox`, `Update`, `Delete`, `Tombstone`, Ed25519, `Move`) and the conformance suite is in place; Wave 2 has advanced with `Like`, followed-feed/home-timeline, reply threading, federated `Update`/`Delete` propagation, global search/directory, delivery retry/dead-letter (at-least-once delivery), and the `Block` moderation edge — recorded (moderation store + `blocks` collection + client `BlockAsync`/`GetBlocksAsync`), *applied* (a blocked actor's content is excluded from the blocker's followed feed, and outbound delivery to an actor that blocked the sender is suppressed), and now *reversible* (the un-block, `Undo` of `Block` via `RemoveBlockAsync` + client `UnblockAsync`).
