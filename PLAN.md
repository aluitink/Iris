...
- **114.4 — (candidate)** — Next Phase D slice, pick one: (a) remaining one-off display font-sizes (0.65–1.8rem, ~15) → a display ramp if a repeatable pattern emerges, or (b) a concrete matrix ☐ distribution feature (post boosts, or community join flow). Replenish before selecting.
...
- **114.3 — Card-List Spacing Rhythm (DONE)** — Unified the inter-item gap of the four primary bordered-card lists (posts / actors / community cards / communities) to `--space-3` (12px); two 8px outliers (`.actor-list-grid`, `.directory-list`) promoted. Nested content keeps the tighter `--space-2`. Clean two-tier rhythm. Presentational. Build clean, Web.Tests 95/95, live-verified. [changes/1143](docs/changes/1143-phase114-card-spacing-rhythm.md)
