...
Phases 0–9 are complete. Phase 8 delivered the server-side Docker foundation; Phase 9 (Real-World Deployment Preparation) is complete — all five deliverables done and grounded in the real config/client/server/test surface. **Phase 10 (Project & Test Review) is in progress** — Slices 1.1–1.5 are done: removed dead `CommunityIris.cs` + `LazyActorDocumentFetcher.cs`, consolidated IRI-resolution + page-flatten + collection-IRI dedup, moved `CollectionPage` to `Iris.Core`, consolidated the parallel cache engines into `CachingReadThrough<T>`, consolidated `Accept`/`Reject` activity handlers into `FollowResponseActivityHandler<T>`, and extracted shared cores for the inbox POST + community-collection endpoint handlers (444→466, 22 new tests). Per-slice detail (what was built, key types, test counts, resolved decisions) lives in [docs/CHANGELOG.md](docs/CHANGELOG.md); the phased plan and open questions are in [docs/ROADMAP.md](docs/ROADMAP.md).
...
| 10 — Project & Test Review | 🚧 in progress | Slices 1.1–1.5 done (444→466, 22 new tests). 1.1: dead-code sweep + IRI-resolution dedup. 1.2: CollectionPage → Iris.Core + CollectionPageFactory + ResolveCollectionIri. 1.3: parallel cache engines → CachingReadThrough<T>. 1.4: Accept/RejectActivityHandler → FollowResponseActivityHandler<T> base. 1.5: inbox POST + community-collection endpoint handlers → shared cores. Remaining: harness consolidation, API surface pass, doc sync. |
