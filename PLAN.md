...
1. **54.7: WASM cross-instance GET proxy fallback not wired up** — ACTIVE. A Playwright session hit a CORS error fetching a remote/external actor from the browser with no proxy fallback. `Iris.Client.Pipeline.ProxyFallbackHandler` already has a `crossInstanceReadsViaProxy` mode built for exactly this (a CORS-blocked cross-origin `GET` is a network failure with no status code, so the 401/403 fallback path never engages — [ProxyFallbackHandler.cs](src/Iris.Client/Pipeline/ProxyFallbackHandler.cs)), but [IActorSessionAccessor.cs](apps/Iris.Web.Client/Accounts/IActorSessionAccessor.cs) constructs `ActivityPubClientOptions` with only `ActorId` set — `RouteCrossInstanceReadsViaProxy`/dial-base are never configured for the WASM client, `Client`, `LocalModeration`, and `MediaClient`. Investigate wiring this option (and confirm the dial-base is the app's own origin) so every AP GET issued by the WASM client falls back to the proxy endpoint instead of failing on CORS.
2. **54.8: Redundant per-object likes/shares collection fetch** — `EngagementBar.razor` (`OnParametersSetAsync`) issues a full `GetLikesAsync`/`GetSharesAsync` walk per rendered object to seed `_likeCount`/`_boostCount`/`_liked`/`_boosted`, even inside timeline/collection views where many objects render at once. Investigate whether collection-yielded objects already carry `likeCount`/`shareCount`/`replyCount` and context-specific `isLiked`/`isShared`/`isReplied` (server-side) — if so, thread those fields through to `EngagementBar` (e.g. as parameters) so it only falls back to the live `/likes`/`/shares` fetch when that data isn't already available (e.g. on the standalone `ObjectDetail` page), instead of always re-fetching.
3. **54.9: Possible duplicate home-feed fetch** — a Playwright session observed what looked like two calls to the feed endpoint from the Home tab. Investigate [HomeTimeline.razor](apps/Iris.Web.Client/Components/Pages/HomeTimeline.razor) (`OnInitializedAsync` awaits `Session.EnsureReadyAsync()` then calls `StateHasChanged()`, which re-triggers `PagedCollection`'s `OnParametersSet`) and [PagedCollection.razor](apps/Iris.Web.Client/Components/Pages/PagedCollection.razor)'s `_loadedFor` guard, plus whether Blazor Web App prerender + WASM circuit boot double-mounts the component — confirm via the browser network tab whether it's a real double-fetch (fix) or two distinct/cached requests (no-op).
