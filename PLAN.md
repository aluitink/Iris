...
- **84.6 — Shared-state scale-out (converging multi-instance) (Active Slice).** 84.5 made the single-instance-per-origin constraint explicit + enforced (the `SingleInstanceLock` startup guard: a second live instance on the same persistence fails fast). This slice is the *convergence* half — the shared state two instances need to coexist over one origin without diverging. **(Part 1 DONE — the document-derived shared key provider)** `DocumentDerivedKeyProvider` (new, `Iris.Server/Identity`): an `IKeyProvider` whose sync `TryGetIdentity` reads a thread-safe in-memory map, and whose **`RefreshFromActorsAsync` is the convergence primitive** — it re-derives every local actor's key IRI from the durable actor document's `publicKey.id` (via `GetPublicKeyIri`, the 84.2/84.4 boundary point; `#key-1` fallback for a legacy actor), guarded by `IKeyStore.TryGetKey` so a retired key is not (re)bound. Drop-in for the `InMemoryKeyProvider` (same `IKeyProvider` seam). **3 convergence tests** prove the divergence is real (rotate on A leaves B stale at `#key-1`) and B's `RefreshFromActorsAsync` converges B to `#key-2` with **no restart**; a refresh drops an actor whose current key is retired; a legacy actor falls back to `#key-1`. (commit `2909da6`; [change doc 846](docs/changes/846-phase84-document-derived-shared-key-provider.md).)
  - **(Part 1b DONE — the when-of-refresh wiring)** `KeyProviderRefreshService` (new, `Iris.Server/Identity`): a `BackgroundService` (the `PersistenceDegradedModeProbe` pattern) that, on startup + every `ActivityPubServerOptions.KeyProviderRefreshInterval` (default 30 s; non-positive = startup-only), resolves the instance's `IKeyProvider` and, if it is a `DocumentDerivedKeyProvider`, re-runs `RefreshFromActorsAsync` over the shared `IPersistenceProvider` — so a rotation on another instance becomes visible to this instance's signer **automatically, with no explicit refresh call** (the sync `SigningHandler.TryGetIdentity` can't do the async document read itself — CODING_STYLE forbids sync-over-async in library code, so the *when* lives in a background service). Registered unconditionally in `AddActivityPubServer` but self-gated: a no-op for any non-document-derived provider (the single-instance `InMemoryKeyProvider` default + every existing harness are unaffected). **3 new tests** (auto-convergence with no explicit refresh — polled/bounded; inert for a non-document-derived provider; non-positive interval disables the tick but the startup pass still converges a fresh provider). (commit `84e6f9a`; [change doc 847](docs/changes/847-phase84-key-provider-refresh-hosted-service.md).)
  - **(Part 2 DONE — the shared delivery queue)** `SharedDeliveryQueue` (new, `Iris.Server/Delivery`): a shared, file-backed `IDeliveryQueue` — a single JSON journal two (or more) instances over the same origin enqueue into and consume from, guarded by a cross-process file lock (a `.lock` file opened with `FileShare.None`). Model: a **visibility-timeout claim protocol** (the SQS `ReceiveMessage` pattern) — each journaled record is a `DeliveryJob` + a monotonic `Seq` + a `Status` (`Pending`/`Claimed`) + a claim stamp (`ClaimedBy` = the instance id, `ClaimedAt` = when). `EnqueueAsync` appends a `Pending` record; `TryDequeueAsync` purges records claimed past the drop horizon (24 h, so the journal is bounded) then claims the lowest-`Seq` `Pending`-or-expired-`Claimed` record. **At-least-once** (a crashed/slow claimer's job is reclaimable past the 10-min visibility timeout; the receiver dedupes by `Id`, C-07) — the same guarantee as `FileBackedDeliveryQueue`. `UseSharedDelivery` extension (the scale-out counterpart to `UseFileBackedDelivery`; registers the shared queue + a shared dead-letter store). **7 new tests** (cross-instance visibility; a delivery queued on A is delivered by B's worker — the acceptance test; claim exclusivity within the window; reclamation past the window; purge past the drop horizon; `Count` reflects the shared pool; a torn journal line is skipped). (commit `5d50278`; [change doc 848](docs/changes/848-phase84-shared-delivery-queue.md).)
  - **(Part 3 DONE — the cache-invalidation channel)** `CacheInvalidationChannel` (new, `Iris.Server/Caching`): a file-backed, cross-process invalidation journal (JSON lines + a cross-process file lock) — `PublishActorInvalidationAsync` appends an event (actor IRI + monotonic `Seq` + UTC timestamp); `PollAsync(sinceSeq)` returns new events; `PurgeAsync` bounds the journal (default 1 h retention). `CacheInvalidationService` (new, `Iris.Server/Caching`): a `BackgroundService` (the `KeyProviderRefreshService` pattern) that, on startup + every `CacheInvalidationPollInterval` (default 5 s), polls the channel and invalidates the local `RemoteActorCache` + `LocalActorDocumentCache` entries for the affected actor IRIs. The `KeyRotationService` publishes an invalidation event after re-stamping the actor document (best-effort). `UseCacheInvalidationChannel` extension (the scale-out counterpart to the in-process invalidation; registers the channel + the service). The single-instance default is a no-op publisher (no channel, no poller). **6 new tests** (a rotation on A invalidates B's cache on poll — the acceptance test; a publish is visible to the other instance; the sequence is monotonic; purge removes old events; a torn journal line is skipped; the service invalidates the `RemoteActorCache`). (commit `f7b831f`; [change doc 849](docs/changes/849-phase84-cache-invalidation-channel.md).)
  - **remaining (next turn):** **lift the 84.5 single-instance guard** — with all three convergence pieces in place (the document-derived key provider + the shared delivery queue + the cache-invalidation channel), multi-instance over one origin is supported when all three are configured. Change the `SingleInstanceGuardHostedService`/`SingleInstanceLock` behavior from fail-fast to a warning (or config-gate it: fail-fast when the scale-out pieces are NOT all configured, warning when they are). Update the 84.5 doc + the guard's XML comments to reflect the new multi-instance support.
...
- **84 — Production hardening & scale-out readiness (Active phase; 84.1–84.5 DONE, 84.6 in progress).**
  - ~~**84.1** — Config-driven durable dead-letter store~~ — **DONE** (`Iris:Delivery:DeadLetterJournalPath` config key rebinds the dead-letter store to file-backed, decoupled from the delivery queue). [841](docs/changes/841-phase84-config-driven-durable-dead-letter-store.md)
  - ~~**84.2** — Local key-rotation lifecycle~~ — **DONE** (`KeyRotationService`: mint `#key-N`, re-register, re-stamp the actor document's `publicKey` with a `replaces` pointer; `RetireKey`). [842](docs/changes/842-phase84-local-key-rotation-lifecycle.md)
    - ~~**84.3** — Operator key-rotation endpoint~~ — **DONE** (admin-gated `POST /ap/v1/keys/rotate` + `/keys/retire`). [843](docs/changes/843-phase84-operator-key-rotation-endpoints.md)
    - ~~**84.4** — Key-rotation durability + per-actor rotation~~ — **DONE** (`KeyProviderRehydration` re-derives each actor's key from its persisted `publicKey.id`; `?actor=` on rotate). [844](docs/changes/844-phase84-key-rotation-durability-per-actor.md)
      - ~~**84.5** — Multi-instance (scale-out) state readiness~~ — **DONE** (the single-instance-per-origin constraint + a startup guard: `SingleInstanceLock` + `SingleInstanceGuardHostedService`, gated behind `Iris:InstanceLockPath`). [845](docs/changes/845-phase84-single-instance-per-origin-guard.md)
    - **84.6 — Shared-state scale-out (converging multi-instance) (Active Slice — parts 1, 1b, 2 done, see Active Slice `remaining`).** Part 1 (the document-derived shared key provider) + part 1b (the when-of-refresh wiring — auto-convergence, no explicit refresh) + part 2 (the shared delivery queue with a cross-process claim) are DONE ([846](docs/changes/846-phase84-document-derived-shared-key-provider.md) · [847](docs/changes/847-phase84-key-provider-refresh-hosted-service.md) · [848](docs/changes/848-phase84-shared-delivery-queue.md)); remaining = cache invalidation, then lift the 84.5 guard.
- ~~**82 — Real-world signed federation + production readiness**~~ — **COMPLETE** (82.1 outbound signature conformance, 82.2 real-world signed delivery, 82.3 key-management hardening). [821](docs/changes/821-phase82-outbound-signature-conformance.md) · [822](docs/changes/822-phase82-real-world-signed-delivery.md) · [823](docs/changes/823-phase82-key-management-hardening.md)
- 85 - Notifications are showing Replied to you but they are replies to another users thread.
...
- **84.2 — Local key-rotation lifecycle — DONE** ([842](docs/changes/842-phase84-local-key-rotation-lifecycle.md)): `KeyRotationService` (mint `#key-N`, re-register, re-stamp the actor document's `publicKey` with a `replaces` pointer; `RetireKey`). **4 new tests.**
- **84.1 — Config-driven durable dead-letter store — DONE** ([841](docs/changes/841-phase84-config-driven-durable-dead-letter-store.md)): `Iris:Delivery:DeadLetterJournalPath` config key rebinds the dead-letter store to file-backed, decoupled from the delivery queue. **5 new tests.**
