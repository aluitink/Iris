...
- **138.22 (edit/update propagation) — verified existing capability:** the `UpdateActivityHandler` correctly handles a Lemmy-side post/comment `Update` for a locally-archived copy — local store refreshed in place, owner guard rejects non-owner updates, tombstone re-animation guard works, unknown objects not created. 5 integration tests (`LemmyUpdatePropagationIntegrationTests`).
