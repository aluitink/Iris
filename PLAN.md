...
- **6 open. No blockers (all S2/S3-sev).** (Pass 74: S19 re-confirmed — Update activity 202 but not persisted to DB; Requests tab still no endpoint. Improvement: Update body now includes changed field.)
- **Top priority:** S21 (/c/{handle} route still broken — redirect page not active; auto-follow FIXED), S19 (requests endpoint missing + no Accept/Decline in notifications + Edit community Save not persisted to DB — Update 202 but DB unchanged), S20 (Home feed Communities tab non-functional — visual-only toggle, 0 new API requests — 6th pass), S4 (remote communities missing from Following tab — 11th pass), S17 (profile tabs over-fetch — 32 outbox requests for 3 tabs), S16-UX (REGRESSION: vote count inconsistent — profile 0, object 1; badge missing from both). S2 (proxy 401 — 2 errors, no change) / S14 (proxy 401 + CSP on actor detail — identical to Pass 32) still blocked on dev.
- **Last pass:** 74 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S21 PARTIALLY FIXED (auto-follow works, /c/{handle} redirect not active); S2 partially improved (11→2 errors); S16-UX REGRESSED (vote count inconsistent again). Remaining open: S2, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21 (/c/{handle} only).
