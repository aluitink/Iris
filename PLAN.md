...
1. **140.1 Deletes with embedded signatures** — Investigate why the signature validation fails for Delets when they contain an embedded signature. The delete may be for an actor whose public key can no longer be fetched (the actor was deleted). Handle these gracefully; consult the Mastodon codebase for how to process an embedded signature.
...
3. **139.3 Scenario 9 (migration safety)** — Apply the current EF Core migrations to a populated/older-schema DB copy; confirm migration completes cleanly, no data loss, and the app boots against the migrated schema. (Next 139.3 review scenario after s8.)
