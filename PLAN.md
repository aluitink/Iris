...
- **74 — Better Mastodon compatibility (in progress).** **74.1 DONE** ([change doc 741](docs/changes/741-sensitive-blur-includes-image.md)): sensitive-content blur now covers the image/media gallery (not just text). `MediaGallery` gained a `Blurred` param; `ObjectView` passes it based on `IsSensitive && !Revealed`. Live-verified: image blurred in initial state, revealed on Show, re-blurred on Hide. Build 0 warn/0 err; full suite: 1665 passed, 0 failed, 17 skipped. **Next: 74 — Mastodon wire-format standardization (investigate + fix).**
...
- **74.1 — Sensitive blur includes image (Phase 74, slice 1) — DONE** ([change doc 741](docs/changes/741-sensitive-blur-includes-image.md)): `MediaGallery` gained a `Blurred` param; all media wrapped in `.media-gallery-wrap` which gets `object-content--blurred` (blur(8px) + pointer-events:none) when the post is sensitive and not revealed. Live-verified: image blurred initially, revealed on Show, re-blurred on Hide. Full suite: 1665 passed, 0 failed, 17 skipped.
- **73.6 — Poll voting (Phase 73, slice 6) — DONE** ([change doc 736](docs/changes/736-poll-voting.md)): Server: `POST /local/v1/u/{handle}/votes/{**pollIri}` records a vote on a stored Question object (Basic/cookie auth, idempotent via voters array, 409 on expired). Client: `VoteAsync` on `ILocalModerationClient` (POST JSON `{option: index}`, same handler-resolution pattern as mutes/relays). Core: `GetPollDataFromJson` parses the vote response. UI: poll options clickable when signed in + not expired + not already voted; clicking shows a check mark, updates bars/counts, shows "You voted" footer. Live-verified: posted a poll, voted on "C#" → UI updated (1 vote, "You voted"), server state confirmed (voters=[actor], votesCount=1, totalVotes=1), re-vote is a no-op. **Phase 73 is now COMPLETE** (all 6 slices done). Full suite: 1665 passed, 0 failed, 17 skipped.
- **73.5 — Cross-instance @mention resolution (Phase 73, slice 5) — DONE** ([change doc 735](docs/changes/735-cross-instance-mention-resolution.md)): `DetectMentionsAsync` regex extended to match `@user@domain` tokens; cross-instance mentions resolved via WebFinger (RFC 8410) through the home instance's proxy endpoint (browser can't reach remote instances directly — CORS); unresolvable mentions silently dropped. Same-instance resolution unchanged. Live-verified: `@andrew@iris.luit.ink` resolved to correct actor IRI in tag array; `@ghost@remote.example.com` silently dropped. Full suite: 1665 passed, 0 failed, 17 skipped.
- **73.4 — Community posts carry media + CW (Phase 73, slice 4) — DONE** ([change doc 734](docs/changes/734-community-media-cw.md)): relaxed the CW gate in `Compose.razor` from `ReplyToIri is null && CommunityIri is null` to `ReplyToIri is null` (community posts now show the CW checkbox); `PostToCommunityAsync` extended with `media`, `sensitive`, `summary` params — `sensitive` → ExtensionData, `summary` → `note.Summary`, each media file → `Image`/`Document` in `note.Attachment`. Live-verified: community compose shows attachment + CW; posted note carries `sensitive: true`, `summary`, correct audience (202). Full suite: 1665 passed, 0 failed, 17 skipped.
...
- **73.2 — Multiple media attachments (Phase 73, slice 2) — DONE** ([change doc 732](docs/changes/732-multiple-media-attachments.md)): multi-file compose, `MediaAttachment` record, `Image`/`Document` per type. Live-verified. Full suite: 975 passed, 17 skipped, 0 failed.
