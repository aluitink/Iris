...
- **74 — Better Mastodon compatibility (in progress).** **74.1 DONE** ([change doc 741](docs/changes/741-sensitive-blur-includes-image.md)): sensitive-content blur now covers the image/media gallery. **74.2 DONE** ([change doc 742](docs/changes/742-mastodon-wire-format-fields.md)): minted Notes now carry `url` (the note IRI), explicit `sensitive` (always present, `false` or `true`), and `replies` (empty `OrderedCollection` pointer to `{noteId}/replies`) — the three fields Mastodon always emits that Iris previously omitted. `EnrichNoteForMastodon` called from `MintActivityIds`. Live-verified: public post has all three; sensitive post preserves `sensitive: true`; replies endpoint returns matching `OrderedCollection`. Build 0 warn/0 err; full suite: 1665 passed, 0 failed, 17 skipped. **Next: continue 74 wire-format standardization — remaining gaps: `atomUri`, `conversation`/`context`, `contentMap`, `likes`/`shares` as Collections (vs `iris:` count extensions), `inReplyToAtomUri`.**
...
- **74.2 — Mastodon wire-format fields (Phase 74, slice 2) — DONE** ([change doc 742](docs/changes/742-mastodon-wire-format-fields.md)): minted Notes now carry `url` (note IRI), explicit `sensitive` (always present), and `replies` (empty `OrderedCollection` pointer). `EnrichNoteForMastodon` called from `MintActivityIds`. Live-verified: public + sensitive posts carry all three; replies endpoint live. Full suite: 1665 passed, 0 failed, 17 skipped.
- **74.1 — Sensitive blur includes image (Phase 74, slice 1) — DONE** ([change doc 741](docs/changes/741-sensitive-blur-includes-image.md)): `MediaGallery` gained a `Blurred` param; media wrapped in `.media-gallery-wrap` with `object-content--blurred` when sensitive + not revealed. Live-verified. Full suite: 1665 passed, 0 failed, 17 skipped.
...
- **73.5 — Cross-instance @mention resolution (Phase 73, slice 5) — DONE** ([change doc 735](docs/changes/735-cross-instance-mention-resolution.md)): WebFinger lookup for `@user@domain`; unresolvable mentions dropped. Full suite: 1665 passed, 0 failed, 17 skipped.
- **73.4 — Community posts carry media + CW (Phase 73, slice 4) — DONE** ([change doc 734](docs/changes/734-community-media-cw.md)): relaxed CW gate; `PostToCommunityAsync` extended with media/sensitive/summary. Full suite: 1665 passed, 0 failed, 17 skipped.
