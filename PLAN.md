...
- **63.1 — UI/UX review (usability + presentation; detailed visual pass).** Phase 62 (bug hunt) is **COMPLETE** — all blocker/bug findings fixed and the 62.4 clean-entry re-pass converged (two consecutive clean passes on a DB-backed same-origin server); see change doc [621](docs/changes/621-phase62-closeout-bug-hunt.md) and the [620 tracker](docs/changes/620-bug-hunt-tracker.md). 63 is the detailed visual/usability review of the production app: go route-by-route (signed-in `andrew` + authless) and evaluate **presentation and usability**, not correctness (correctness is 62's job, done). Look for: layout/spacing/alignment issues, contrast/readability, inconsistent typography, broken or awkward empty states, confusing labels/copy, missing affordances, poor responsive behavior, hover/focus states, and anything that looks or feels off. **Method (binding, from 62's lesson):** drive the app with **native DOM input events** via `page.evaluate` (`el.click()`, native value-setter + `dispatchEvent(new Event('input'))`) — **never** MCP `browser_click`/`browser_type` for an assertion (synthetic clicks do not reliably fire Blazor WASM `@onclick`). Watch the console for errors. Log every finding in a **new 630 UX tracker** (`docs/changes/630-ux-review-tracker.md`) with class `ux` (so it routes back here) + severity, and route any *perf* observation to 64. **Timebox:** ~5 routes per turn — stop, checkpoint, continue next turn; roll overflow into the buffer. Fix small UX nits in-slice (CSS/razor only, no behavior change); defer structural redesigns to 63.2+.

  **Checkpoint (turn 1):** Reviewed signed-in routes **Home, Search (empty state), Communities, Profile (tabs + 390px mobile), Settings (all tabs incl. Danger), Directory** + authless **Login/Register** + **390px mobile** nav. Contrast all ≥ 6:1, no horizontal overflow at any width, mobile nav collapses to an aria-labelled "Menu" toggle. **2 findings, both fixed in-slice + verified live:** [U-01](docs/changes/630-ux-review-tracker.md) base-`a` now underlined (WCAG 1.4.1), [U-02](docs/changes/630-ux-review-tracker.md) auth `.field label` now matches `.form-group label` (uppercase/600/letter-spaced). Both `app.css` copies kept in sync. Build 0 warn/0 err; full suite **1228 passed / 0 failed**.

  **Checkpoint (turn 2): 63.1 route-by-route pass COMPLETE.** Reviewed the remaining routes: **Compose** (meta row fits at 390px), **Notifications** (filter tabs, deleted-account captions, remote-name fallback), **Actor detail** (`/actor`), **Object detail** (`/object`, contrast sweep 0 elements <3:1), **Register** (authless, U-02 fix confirmed), **Settings Relays + Moderation** sub-tabs (clear empty states). **One more finding, fixed in-slice + verified live:** [U-03](docs/changes/630-ux-review-tracker.md) — the Notifications active filter tab was **never highlighted** (CSS `.filter-tab[aria-pressed="true"]` never matched because the razor bound a boolean, which Blazor renders as `aria-pressed=""`). Fix: razor now binds the string `? "true" : "false"` (matching EngagementBar) **and** the CSS selector widened to also match the empty-value form. Verified live: selected tab is now accent-highlighted with proper `"true"`/`"false"` aria. **Also routed to 64:** every "deleted their account" notification fires an avatar proxy fetch that 410-Gones (15 console errors + 15 wasted requests on page 1) — graceful UI fallback, pure network noise. **Build 0 warn/0 err; full suite green (one known-flaky delivery test fails in the full run, passes in isolation).** All 3 findings (U-01/U-02/U-03) fixed + verified; 63.1 done. **Note for the loop:** the server `Iris.Web.csproj` `BuildAndCopyClient` target only republishes the client when `Iris.Web.Client/publish/` is missing — a razor change requires deleting that dir first or the stale WASM is served.
...
| 63 | UI/UX review — usability + presentation (visual pass; fixes 62's UX-class findings) | in progress (63.1 done; 63.2 = design/IA brainstorm next) |
| 64 | Request spam & network efficiency — cut redundant/duplicate calls on load (6 topics drafted in [620 tracker](docs/changes/620-bug-hunt-tracker.md#phase-64--request-spam--network-efficiency-draft-topics)) | queued |
...
- 63.1 — UI/UX visual/usability pass, route-by-route (native DOM input; log to 630 tracker) — **complete** (3 findings U-01/U-02/U-03 fixed in-slice + verified; [630 tracker](docs/changes/630-ux-review-tracker.md))
- 63.2 — UI/UX **brainstorm + structural pass**: now that the nit pass (63.1) is done, do the "what makes a good interface" design review — evaluate information architecture, density, hierarchy, and the data-organization questions Phase 63 raised (is the data well organized/readable/functional?). Decide which, if any, are worth a structural change; spec them as concrete slices. (The 3 small 63.1 nits are already fixed in-slice, so this is the design-thinking half, not a nit-fix pass.)
