...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` + per-method reset + `ServerRefFor` are in place. **Thirteen RISKY single-instance classes converted so far** (across four partials): Community (11), Object (11), CommunityModeration (12), CommunitySearch (9), CommunityOutboxCollection (5), CommunityFeedCorrectness (4), InteractionCollection (9), MutesCollection (8), RelaysCollection (8), Blocks (8), Flags (7), Reply (7), CommunityJoinRequest (10). Full fast suite green (899/899, 5m37s). **remaining:** (a) two-instance federation classes (~20 — cross-wired delivery transports, reset covers both instances). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/258](docs/changes/258-29.3-signature-validation-risky-classes.md).
...
- 29.3 (follow-up, partial 4): **Signature-validation RISKY classes** — converted the last four single-instance classes that use HTTP signature validation (`BlocksCollectionIntegrationTests` 8, `FlagsCollectionIntegrationTests` 7, `ReplyIntegrationTests` 7, `CommunityJoinRequestIntegrationTests` 10) to the shared host. Key fix: the key store is preserved across `Reset()`, so `SeedPersonWithExistingKey` / `SeedCommunityWithExistingKey` (new) restore actors/communities with the *same* key instance the self-fetcher holds — no signature mismatch. **Thirteen RISKY single-instance classes converted total; all single-instance RISKY classes done.** Remaining: ~20 two-instance federation classes. Full fast suite green (899/899, 5m37s). → [docs/changes/258-29.3-signature-validation-risky-classes.md](docs/changes/258-29.3-signature-validation-risky-classes.md)
- 29.3 (follow-up, partial 3): **Basic-auth RISKY classes with client rebuild** — converted `MutesCollectionIntegrationTests` (8) + `RelaysCollectionIntegrationTests` (8) to the shared host. Pattern: fixture builds host once (initial key for self-fetcher + `CredentialValidator`); test class rebuilds `_client`/`_local` in `InitializeAsync` using reseeded keys (non-readonly fields). Works because these use Basic auth (not signature validation), so the key-replacement issue doesn't apply. **Nine RISKY single-instance classes converted total.** Full fast suite green (899/899, 5m31s). Remaining: ~4 signature-validation RISKY classes + federation classes. → [docs/changes/257-29.3-basic-auth-risky-classes.md](docs/changes/257-29.3-basic-auth-risky-classes.md)
