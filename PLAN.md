...
- **6 open. No blockers (all S2/S3-sev).** (Pass 83: S16-UX re-confirmed — badge missing on object detail, vote count consistent. IRI format changed: old URLs return "Object not found".)
...
- **Last pass:** 83 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S21 PARTIALLY FIXED (auto-follow works, /c/{handle} redirect not active); S2 partially improved (11→2 errors); S16-UX partially improved (vote count consistent again); S19 facet 2 committed but not deployed. Remaining open: S2, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19 (all facets), S20, S21 (/c/{handle} only).
