...
**Phase 156 - New Post/Compose enhancements** — *Slices A + B done* (A: client composes Mastodon-faithful `content` HTML so `@mention`/`#hashtag` tokens render as tappable `<a>` links + the structured `tag` array; B: display-side linkify in `ObjectView` so a plain-text note carrying those tokens in its `tag` array also renders tappable links — pre-rendered HTML is untouched, so no double links). **Remaining slice (next turn):** (Slice C) backend resolution of `@handle@domain` / `#hashtag` tokens in incoming notes that lack matching `tag`/`cc` references, so new mentions/hashtags resolve. Also an open item: the `Mention` tag currently serializes as a compact IRI string in the stored document (server ingestion normalization, pre-existing) rather than the full `{type:"Mention",href}` object — worth confirming interop parity with remote Mastodon. See docs/changes/1561-… + 1562-….
