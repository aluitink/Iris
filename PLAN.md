...
- **8 open. No blockers (all S2/S3-sev).** (Pass 53: S3 re-confirmed — Create activities not persisted to DB (5th consecutive pass); root cause: server generates Create IRI in 202 response but never stores it.)
...
- **Last pass:** 53 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8 fixed; S18 partially fixed; S21 partially resolved (caching). Remaining open: S2, S3, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21.
