...
- **3 open + 2 new. No blockers (all S2/S3-sev).** (Pass 95: S22 FIXED — follow notifications now display (7 items); S19 facet 2 PARTIALLY FIXED — Accept/Decline buttons visible but hit 404 endpoints (NEW BUG S23); S4 re-confirmed 21st pass.)
- **Top priority:** S23 (Accept/Decline buttons hit 404 — `POST /local/v1/u/{handle}/requests/accept/{iri}` and `.../reject/{iri}` not registered), S4 (remote communities missing from Following tab — 21st pass), S17 (profile tabs over-fetch — 39 outbox requests for 3 tabs), S16-UX (poll "You voted" badge missing on object detail page). S2 (proxy 401 — 2 errors, no change) / S14 (proxy 401 + CSP on actor detail) still blocked on dev.
- **Last pass:** 95 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S20 FIXED; S21 FIXED; S22 FIXED (follow notifications display); S19 facet 1 FIXED; S19 facet 3 FIXED; S19 facet 2 PARTIALLY FIXED (buttons visible but 404 — blocked by S23). Remaining open: S2, S4 (remote), S14, S16-UX, S17, S18 (partial), S19 (facet 2 — blocked by S23), S23 (Accept/Decline 404).
