...
**Phase 54 — Post-1.0 polish & hardening (IN PROGRESS).** 54.1–54.4 + 54.7 COMPLETE. 54.7 (WASM cross-instance GET proxy fallback) wired the existing client proxy-fallback into the WASM app + added cookie-auth to the server proxy endpoint + fixed a target-IRI encoding mismatch — the WASM app can now load remote actors/objects (verified live against mastodon.social). Next slice 54.8 (redundant per-object likes/shares fetch) is Active.
...
**54.8: Redundant per-object likes/shares collection fetch** — `EngagementBar.razor` (`OnParametersSetAsync`) issues a full `GetLikesAsync`/`GetSharesAsync` walk per rendered object to seed `_likeCount`/`_boostCount`/`_liked`/`_boosted`, even inside timeline/collection views where many objects render at once. Investigate whether collection-yielded objects already carry `likeCount`/`shareCount`/`replyCount` and context-specific `isLiked`/`isShared`/`isReplied` (server-side) — if so, thread those fields through to `EngagementBar` (e.g. as parameters) so it only falls back to the live `/likes`/`/shares` fetch when that data isn't already available (e.g. on the standalone `ObjectDetail` page), instead of always re-fetching. No new coded tests per the binding WASM manual-test policy; verify via the browser network tab (fetch count per timeline render before/after).
...
1. **54.8: Redundant per-object likes/shares collection fetch** — ACTIVE. `EngagementBar.razor` (`OnParametersSetAsync`) issues a full `GetLikesAsync`/`GetSharesAsync` walk per rendered object to seed `_likeCount`/`_boostCount`/`_liked`/`_boosted`, even inside timeline/collection views where many objects render at once. Investigate whether collection-yielded objects already carry `likeCount`/`shareCount`/`replyCount` and context-specific `isLiked`/`isShared`/`isReplied` (server-side) — if so, thread those fields through to `EngagementBar` (e.g. as parameters) so it only falls back to the live `/likes`/`/shares` fetch when that data isn't already available (e.g. on the standalone `ObjectDetail` page), instead of always re-fetching.
2. **54.9: Possible duplicate home-feed fetch** — a Playwright session observed what looked like two calls to the feed endpoint from the Home tab. Investigate [HomeTimeline.razor](apps/Iris.Web.Client/Components/Pages/HomeTimeline.razor) (`OnInitializedAsync` awaits `Session.EnsureReadyAsync()` then calls `StateHasChanged()`, which re-triggers `PagedCollection`'s `OnParametersSet`) and [PagedCollection.razor](apps/Iris.Web.Client/Components/Pages/PagedCollection.razor)'s `_loadedFor` guard, plus whether Blazor Web App prerender + WASM circuit boot double-mounts the component — confirm via the browser network tab whether it's a real double-fetch (fix) or two distinct/cached requests (no-op).
3. **54.10: Settings tab bar overcrowded** — the Settings tab bar (Account/Password/Notifications/Communities/Relays/Moderation/Danger, [Settings.razor](apps/Iris.Web.Client/Components/Pages/Settings.razor)) is becoming squished as tabs accumulate. Investigate folding the Password tab's change-password form into the Account tab (as a collapsible section or sub-card) to drop a tab and free up space.
4. **54.11: Settings → Account → Edit profile takes two clicks** — from Settings' Account tab, "Edit your profile" links to [Profile.razor](apps/Iris.Web.Client/Components/Pages/Profile.razor), which renders the read-only `ActorProfile` view with its own "Edit profile" button; the user must click that button again to reach `EditProfileForm`. Investigate deep-linking straight to the edit form (e.g. `/profile?edit=true` or a dedicated route) so navigating from Settings lands directly on the editable screen.
5. **54.12: No way to set actor icon/image via edit profile** — `EditProfileForm.razor` only edits display name, bio, and follower-approval; there's no field for the actor's `icon` (avatar) or `image` (banner), even though `ActorProfile.razor` already renders an existing `IconIri`. Investigate adding icon/image URL (or upload-and-set) fields to the edit-profile flow, backed by the existing media-upload/proxy infrastructure.
6. **54.13: Media upload 400s on a 1.3 MiB image — cause unconfirmed** — a compose-post attachment upload of a 1.3 MiB PNG returns `400`. **Key clue: the error response's headers show `Server: nginx/1.29.1`**, which suggests nginx itself (not the .NET app/Kestrel) generated the response — so the app-side theories below are unconfirmed guesses, not a diagnosed root cause; check nginx access/error logs first to see whether the request ever reached the app. Candidates to rule in/out: (a) nginx-level rejection before proxying (malformed multipart request/headers, a header-size limit, or some other nginx-native 400 cause — note nginx's own body-too-large behavior is normally `413`, not `400`, so `client_max_body_size` may not even be it); (b) a second/intermediate proxy layer ("double proxied" — confirm what's actually in the chain between the browser and the app container, e.g. host nginx → docker port mapping → app, and whether there's a distinct inner proxy); (c) if the request does reach the app, [WebAppFactory.cs](apps/Iris.Web/WebAppFactory.cs)'s Kestrel `MaxRequestBodySize` defaults to 1 MiB (`DefaultMaxRequestBodySize`, unset in [docker-compose.yml](apps/Iris.Web/docker-compose.yml)) vs. the media endpoint's own 10 MiB cap (`MaxMediaUploadBytes` in [ActivityPubServerExtensions.cs](src/Iris.Server/ActivityPubServerExtensions.cs)) — `LocalMediaUploadHandler` catches `BadHttpRequestException` from an over-limit body and returns `400` instead of `413`, which would also produce a 400 but from the app, not nginx. Also confirm the client's multipart upload request ([MediaClient.cs](src/Iris.Client/MediaClient.cs)) is formatted correctly. Next step: reproduce and capture nginx logs + whether the app's own logs show the request at all before picking a fix.
7. **54.5: Responsive regression at tablet/laptop breakpoints** — extend the 54.1 mobile pass (375px) to 768px and 1024px: verify no overflow/clipping on the dense pages (Settings cards, admin users/moderation tables, actor detail tabs).
8. **54.6: Search stale-results cosmetic** — during a re-search, the "Searching…" spinner renders while stale previous results remain visible below; hide the stale results (or show them dimmed) while a new search is in flight (noted in 54.2, deferred).
9. **54.14: Mention/hashtag autocomplete popover + `tag` field round-tripping** — two related investigations for compose:
