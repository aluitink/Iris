...
- **135.1 - Communities and the Lemmyverse** - We need to be able to interact with Lemmy communities. We need to better understanding how the Lemmy servers expect us to interact. We should deploy a local lemmy server container and interact with it, we likely need to use public fqdns so let's utilize the iris-dev2.luit.ink fqdn for this. The lemmy container definitons/compose should live in it's own folder separate from our app. **First slice DONE (135.1a):** `RemoteCommunityPersister` persists remote community `Group` documents to the durable `ICommunityStore` (triggered by `IrisActorDocumentFetcher` when a fetched document is a `Group`); the `/ap/v1/actor?iri=...` endpoint now also serves a cached remote community. Remaining: deploy a local Lemmy container (own folder, `iris-dev2.luit.ink` fqdn) and drive real interop (follow/accept a Lemmy community, see it as known content, post content).
