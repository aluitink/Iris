...
- **8 open. No blockers (all S2/S3-sev).** (Pass 65: S21 re-confirmed — no auto-follow + /c/{handle} 502 (7th consecutive pass). Dev fix `8539f3f` on `interop-testing` but NOT deployed to live container.)
- **Top priority:** S3 (Create-IRI 404 — Create activities not persisted to DB — 7th pass), S21 (no auto-follow on community creation + /c/{handle} route missing — 7th pass; Dev fix `8539f3f` not yet deployed), S19 (requests endpoint missing + no Accept/Decline in notifications + Edit community Save silent no-op — re-confirmed on 3 communities), S20 (Home feed Communities tab non-functional — visual-only toggle, 0 new API requests — 4th pass), S4 (remote communities missing from Following tab — 10th pass), S17 (profile tabs over-fetch — 30 outbox requests for 3 tabs), S16-UX (poll "You voted" badge missing on object detail page — vote count consistent). S2 (proxy 401 — 2 errors, no change) / S14 (proxy 401 + CSP on actor detail — identical to Pass 32) still blocked on dev.
- **Last pass:** 65 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8 fixed; S18 partially fixed; S21 partially resolved (caching); S2 partially improved (11→2 errors); S16-UX partially improved (vote count consistent, badge visible in profile). Remaining open: S2, S3, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21.
