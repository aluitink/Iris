...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` (per-xunit-collection, build-the-host-once) landed in 29.3 + the two cleanest read-only single-instance classes were converted (measured: Iris.Server.Tests fast run 5m37s → ~5m16s, ~6%). **This turn:** added the per-method reset mechanism (the missing piece) — `Clear()` on all 12 in-memory stores + `InMemoryPersistenceProvider.Reset()`, `Clear()` on `MemoryCache`/`CachingReadThrough`/`LocalCollectionPageCache`, and `SharedHostFixture.Reset()` (clears persistence + the collection-page response cache, which was leaking between methods on a shared host). Converted **3 RISKY single-instance classes** using `IAsyncLifetime` per-method reset + reseed: `CommunityEndpointIntegrationTests` (11), `ObjectEndpointIntegrationTests` (11), `CommunityModerationIntegrationTests` (12). Full fast suite green (899/899). **remaining:** 8 more RISKY single-instance classes + the two-instance federation classes (their shared-fixture shape is more involved — cross-wired delivery transports, and the reset must cover both instances). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/255](docs/changes/255-29.3-per-method-reset-mutating-classes.md).
