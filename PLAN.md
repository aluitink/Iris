...
**54.13: Media upload 400s on a 1.3 MiB image — ROOT CAUSE CONFIRMED, fix built + verified (re-apply the change)** — a compose-post attachment upload of a 1.3 MiB PNG returns `400`. The diagnosis is complete (see the Up Next entry for the full write-up): the 400 is Kestrel's global **1 MiB `MaxRequestBodySize`** cap (`DefaultMaxRequestBodySize` in [WebAppFactory.cs](apps/Iris.Web/WebAppFactory.cs)), not nginx. **The fix is known and was live-verified** — in `LocalMediaUploadHandler` ([ActivityPubServerExtensions.cs](src/Iris.Server/ActivityPubServerExtensions.cs)), set `IHttpMaxRequestBodySizeFeature.MaxRequestBodySize = MaxMediaUploadBytes` before `ReadFormAsync` and change the `catch (BadHttpRequestException)` to propagate the exception's status code (413 for oversized, 400 otherwise). Re-apply that ~20-line change, rebuild, Docker-redeploy, and live-verify (1.3 MiB → 201, 11 MiB → 413). The code was reverted at the user's earlier request to keep the tree clean; the user has since let the loop resume, so action it.
...
## Recently Completed
  ...
  - 54.7 / 54.4 / 54.3 / 54.2 / 54.8 (Phase 54) — moved to the [ROADMAP.md](docs/ROADMAP.md) ledger (WASM cross-instance proxy fallback, global error-boundary, accessibility/keyboard regression, empty/loading-state audit, redundant per-object likes/shares fetch).
