...
1. **General UI/UX review** (recurring) — Next pass after improvements land. Last pass: Home, Notifications, Profile (own + remote), Compose (end-to-end post, HTTP 202), Communities (list + detail) all consistent and functional; 0 console errors. Two minor observations logged (Following-list names not clickable; cosmetic proxy console noise). No defects. [change doc](docs/changes/997-ui-ux-review.md)
...
- **Signature Validation Review — COMPLETE (Create + Delete diagnoses).** (1) **Create 401 (content-digest):** Fixed by adding `SignatureBase9421.ContentDigestHeaderName` to the header collection in `ToMetadata`. Verified: other hachyderm actors now validate. skinnylatte-specific rejections persist and are a hachyderm-side key mismatch (not an Iris bug). (2) **Delete 401 (embedded proof + key cache):** Delete activities from deleted actors (410 Gone) carry a W3C `RsaSignature2017` embedded proof. The proof's `creator` is the key IRI, resolved via `RemoteKeyCache` (1h fresh + 1h stale) or a fresh fetch. When the actor is deleted, the key can only be resolved if it's in the cache. Mastodon's implementation has the same limitation. **Iris's current behavior (rejecting when the key can't be resolved) is correct** and matches the ActivityPub spec. Potential future improvement: persist remote actor keypairs to the DB (larger architectural change). [change doc](docs/changes/sigval-delete-embedded-proof-diagnosis.md)
