...
1. 28.2: **outbox-publish relay fan-out for `Update`/`Delete`** — extend the `DeliverToRelaysAsync` fan-out (added in 28.1 for `Create`/`Announce`) to the outbox-publish `Update` and `Delete` branches so a local actor's content edit/deletion also reaches their subscribed relays (F-06, ActivityPub §5.1.3), mirroring the inbox post path.
2. 28.3: **relay lifecycle integration test** — lock the full relay subscription lifecycle end-to-end (2-instance `TestServer`): subscribe to a relay (F-06 edge recorded in `IRelayStore`), publish a `Create` to the outbox (fanned out to the relay), then unsubscribe (`Undo(Add)` / relay removal) and confirm a subsequent post is **not** fanned out to the de-subscribed relay.
