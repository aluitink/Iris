...
**Phase 32 — production social platform app.** Step 0 (relocate the sample compose stack, 32.0) is **complete**. Now on **32.1 — bare `Iris.Web` host**: stand up `apps/Iris.Web` as a Blazor Web App that boots `Iris.Server` with the existing in-memory persistence and confirms the library's endpoints (WebFinger / actor doc / inbox / outbox) work inside the new host. → [production-app-web-host.md](docs/plans/production-app-web-host.md)
