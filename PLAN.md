...
- **Phase 12 (in progress, slices 12.1–12.17):** Wave 1 is complete (`sharedInbox`, `Update`, `Delete`, `Tombstone`, Ed25519, `Move`) and the conformance suite is in place; Wave 2 has advanced with `Like`, followed-feed/home-timeline, reply threading, federated `Update`/`Delete` propagation, global search/directory, delivery retry/dead-letter (at-least-once delivery), the `Block` moderation edge — recorded (moderation store + `blocks` collection + client `BlockAsync`/`GetBlocksAsync`), *applied* (a blocked actor's content is excluded from the blocker's followed feed, and outbound delivery to an actor that blocked the sender is suppressed), and *reversible* (the un-block, `Undo` of `Block` via `RemoveBlockAsync` + client `UnblockAsync`) — the `Flag` moderation *report* — recorded (`FlagActivityHandler` + `flags` collection + client `FlagAsync`/`UnflagAsync`/`GetFlagsAsync`) and *reversible* (the un-flag, `Undo` of `Flag` via `RemoveFlagAsync`; a flag has no apply half — it does not sever the relationship) — and the `Mute` (an Iris-specific *local* moderation decision, no ActivityStreams type — recorded via a Basic-authenticated local endpoint + `mutes` collection + client `MuteAsync`/`UnmuteAsync`/`GetMutesAsync` and *applied* as a soft feed exclusion that hides the muted actor's content without severing the follow). **F-07 moderation is now complete** (Block + Flag + Mute).
