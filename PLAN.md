...
- **Mastodon interop tests server (in progress)** Docker scaffold committed. Instance API + WebFinger + ActivityPub actor all respond. Test user `mstest` (pw: `Password1`) created via direct SQL. **VERIFIED END-TO-END:** Iris user `mastodtest` followed `mstest@mastodon.luit.ink` → Follow delivered to Mastodon's inbox → Mastodon followers collection shows `totalItems: 1` → Mastodon sent Accept back → Iris processed the Accept. **Bug found + fixed:** `DefaultLocalActorResolver` classified cached remote actors as local (commit `22a09ae`, [change doc](docs/changes/993-local-actor-resolver-fix.md)). **Networking fix:** added HTTPS (self-signed cert) to the Mastodon proxy on port 443; added hosts entry + CA trust in the Iris container. **Known issue:** delivery metrics `delivered` counter stays at 0 even though deliveries succeed (metrics tracking bug, separate from the delivery path). **Next:** (1) test posting from Iris → verify on Mastodon timeline, (2) test following from Mastodon → Iris, (3) fix delivery metrics counter.
