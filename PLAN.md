...
- **138.21 (local rebuild verification) — verified archival acceptance bar:** the local store is self-sufficient after a Lemmy thread sync — all objects retrievable by IRI, reply edges correctly link parent→children (post → 2 top-level + 1 nested), vote edges recorded (2 likes, 1 dislike), member outbox has all 4 Creates, nested `inReplyTo` chain complete. 5 integration tests (`LocalRebuildVerificationIntegrationTests`).
- **138.20 (full-thread backfill on first peer) — implemented:** when the community feed walks a followed actor's remote outbox, embedded objects (Pages/Notes from `Announce(Create)` relay envelopes or bare `Create`s) are now persisted to the local object store and the `Create` activities are recorded in the community's local members' outboxes. Historical content is available locally (not just proxied). Idempotent: `TryGetObjectAsync` guard + `AddToOutboxAsync` upsert. 2 integration tests (`CommunityBackfillIntegrationTests`).
- **138.19 (shares/boosts interop) — verified platform asymmetry, no code changes:** Lemmy's `Announce` is community-relay-only (no user boost); Iris's relay-unwrap treats it as a fan-out envelope; the merge path attributes content to the `Create`'s actor (original author); the UI renders `LemmyVoteBar` (no Boost button) for Lemmy-sourced content. 1 new integration test. [Change doc](docs/changes/13819-phase138-shares-boosts-interop.md).
- **138.18 (score reconciliation policy) — decided: separate counters + derived `iris:score`:** Iris keeps separate like/dislike counters (mirroring Lemmy's upvotes/downvotes) and derives the Lemmy-equivalent net score as `likedCount - dislikedCount` (`iris:score`). 4 new extension terms; rendered on object document + collection-page items. Also fixed `InMemoryPersistenceProvider.Reset()` to clear the dislike store. 3 integration tests (`ObjectDocumentScoreReconIntegrationTests`). [Change doc](docs/changes/13818-phase138-score-reconciliation-policy.md).
 - **138.17 (Lemmy → Iris dislikes/downvotes) — verified existing capability + doc comments:** the existing `DislikeActivityHandler` records a Lemmy downvote on a synced post (`IDislikeStore` edge) and the `UndoActivityHandler` handles `Undo(Dislike)`; XML doc comments added. 4 integration tests (`LemmyDislikeInboundIntegrationTests`). [Change doc](docs/changes/13817-phase138-lemmy-dislike-inbound.md).
