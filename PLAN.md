...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` + `SharedTwoHostFixture` + `SharedThreeHostFixture` + per-method reset + `ServerRefFor` are in place. **Thirteen RISKY single-instance classes converted** (all single-instance RISKY classes done) + **nineteen two-instance federation classes converted** + **one three-instance federation class converted** (`OutboxAudienceMatch`, 2). Full fast suite green (899/899, 5m36s). **remaining:** ~2 three-instance federation classes (`OutboxAudienceMetadata` [3], `OutboxCreateFanOut` [3]). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/258](docs/changes/258-29.3-signature-validation-risky-classes.md).
