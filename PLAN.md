...
- **70 — Content & media improvement.** The start of "improving" after the exploratory buffer. Three sub-goals (PLAN.md ledger line 79): (1) **media/pictures render correctly** end-to-end; (2) **browse remote users + view their content**; (3) **post content + view it within the instance**. These are already live-verified features from Phase 54 (54.16 images render, 54.20 remote object browsing, 54.21 remote search, 54.13 media upload); this phase is an **ensure-it-works + harden** pass — drive each path live via MCP Playwright on a fresh origin, find + fix any residual gaps, and verify no console errors. **No new coded tests** (WASM manual-test policy) — verify live.
  ...
  **Slice 70.1 — media/pictures render correctly (sub-goal 1).** Drive the full media path live on a fresh origin: upload an image via compose (`IMediaClient` → same-origin media IRI as an `Image` attachment) → verify the blob is stored on the media volume → verify it serves 200 → verify the image renders in `/home` (the `Create` + `Announce` branches of `ObjectView`, `.object-media img`) and in the object-detail view and the 54.16/61.4 gallery. Fix any residual gap (broken/missing image, wrong same-origin rewrite, a media type 61.4 didn't cover, a >1 MiB upload regression 54.13 fixed). Verify 0 console errors. **No new coded tests** (WASM manual-test policy).

  **remaining (70.2 / 70.3):** (2) browse remote users + view their content (drive a live remote actor page, e.g. RayvenMX, end-to-end); (3) post content + view it within the instance (compose → outbox → `/home` round-trip). Both already live-verified in Phase 54 (54.20/54.21, 32.4b) — this phase re-verifies + hardens.
