...
| **73.3** | **Audience / visibility selector.** `PostAsync` hardcodes `to: [AsPublic]` for every post type. Add a `<select>` in compose meta (Public / Followers / Direct), map to the appropriate `to` array (Public = `as:Public`; Followers = the actor's followers-collection IRI; Direct = a recipient picker via the existing mention search). The client's `PostNoteAsync`/`PostReplyAsync` already accept a `to` parameter — compose/UI only. |
...
- 73.3 — Audience / visibility selector (Public / Followers / Direct)
