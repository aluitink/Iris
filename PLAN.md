...
- **140 — Operational investigation: inbound signature-verification failures:** production container logs show a steady stream of rejected HTTP signatures on inbound federated requests from external servers; suspicion is that at least some are *valid* signatures being wrongly rejected (not genuine forgeries/replays). Recreate the `iris-web` container fresh, tail/monitor logs while inbound traffic arrives, and characterize the failure modes (which peers, which key-fetch/signature-string/algorithm step is failing, timing/clock-skew, digest mismatches, key-rotation races, etc.). If current logging is insufficient to pinpoint the cause, extend the signature-verification path's logging (structured, with enough detail per failure — signing string, headers used, key id, comparison result — without leaking private key material) so future occurrences are diagnosable from logs alone. Fix root cause(s) found; otherwise leave a precise follow-up with evidence.

- **141 — Cross-cutting consistency review (topical, no fixes):** a review-only pass across the app looking for inconsistencies in design and behavior — actor pages vs. actor cards, object pages vs. object cards, and other repeated UI patterns (badges, action bars, empty/error states, spacing/typography), plus backend consistency (naming, response shapes, capability checks, error handling) across equivalent endpoints. Catalog findings only — **no bug fixing in this phase**; every finding gets logged with page/component, expected vs. actual, and severity, then routed to **Phase 142** for remediation. Deliverable: a findings doc (`docs/plans/phase-141-consistency-review.md` or similar) that Phase 142 consumes as its worklist.
- **142 — Consistency review fixes:** implement the fixes for findings logged in Phase 141. Scope and detailed worklist to be pulled from the Phase 141 findings doc once that review closes.
