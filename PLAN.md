...
- **132.3 - Nested reply threading in the object detail view** - The Replies tab of the object detail view (132.2) currently shows a *flat* list of the object's direct replies. Extend it to a *threaded* stream: render each reply's own replies nested beneath it (replies-to-replies), with each reply expandable/collapsible to reveal its subtree (collapsed by default, showing a "N replies" affordance). The reply-collection data model already supports nesting (each reply's `inReplyTo` links it to its parent); the work is the UI — thread construction, per-reply nesting depth, and collapse/expand state.
