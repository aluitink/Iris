...
- **115.10 — (candidate)** — Continue D-column matrix reconciliation: pick the next cluster of implemented-but-unverified D items — the **Follow graph** rows (follow, unfollow, follower list, following list) or the **Search & directory** rows (global search, actor search, object search, community search). Live-verify via Playwright + reconcile. Replenish from `docs/plans/production-app-feature-matrix.md` ☐ rows (~12 remain).
- **115.11 — (candidate)** — If a D item turns out to be a *genuine* web/server gap (not just unreconciled), implement + fix it as a vertical slice (impl + tests + live-verify). (115.5 was exactly this: a live search 500.)
