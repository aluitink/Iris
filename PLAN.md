...
1. **139.3 Data lifecycle & persistence review: Scenario 8 — Media lifecycle** — Confirm attached media survives instance restart, proxy rewrite, and a dead-source-URL scenario (502, not a broken image icon forever). Evidence: screenshot + DB/media-store check.
...
- 139.3 Data lifecycle & persistence review: Duplicate/replay delivery idempotency (scenario 7) — **PASS** (verified; +1 test). The Phase 136.17 / C-07 guard (`InboxProcessor.ProcessAsync` → `Activities.TryAddActivityAsync` add-if-absent by IRI) is confirmed: a redelivery returns `false` ⇒ dispatch skipped ⇒ endpoint still 202; duplicate rows are blocked twice over (the add-if-absent check + the PKs on `Activities.Id` and `BoxItems(Direction, ActorId, ItemIri)`). Closed the evidence gap: new `EfPersistenceContractTests.RedeliveredActivity_StoredOnce_SingleRowInDatabase` counts the physical rows in `Activities` + `BoxItems` directly against a real Postgres (each exactly 1). Live UI check: notifications page renders 12 distinct notifications, no duplicates, console-clean. [change doc](docs/changes/1393-7-duplicate-replay-idempotency.md)
