...
- **Mastodon interop tests server (in progress)** Docker scaffold committed (mastodon/ dir: docker-compose + nginx proxy, Mastodon v4.7.2, port 8092, shared network). Instance API responds. **Remaining:** (1) create admin + test users via `rails runner` (v4.7.2 schema: Account/User split, no `admin` on User, no `username` on users table — inspect `db/schema.rb` for exact columns), (2) enable open registration (`SiteSetting`), (3) use MCP Playwright to sign up a mock user, (4) evaluate following/interacting with Iris users, (5) note issues, iterate until satisfied.
