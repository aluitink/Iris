...
- **143 — Consistency review fixes (from Phase 142 findings):** (1) F-142.17 duplicate "Load more" button — **done** (`918f835`); (2) F-142.13/14 boost attribution in actor Posts tab — **done** (`7179ae3`); (3) F-142.1/2 community doc `?refresh` — **done** (`626072e`); (4) F-142.4 — **reviewed, no change**; (5) F-142.19 — **reviewed, no change**; (6) F-142.11 — **reviewed, no change**; (7) F-142.5 community 500 logging — **done** (`0939aa4`); (8) F-142.3 shared `?refresh` predicate — **done** (`0939aa4`, `18f0fc4`); (9) F-142.7 cache key omits limit — **deferred** (invalidation refactor needed); remaining: F-142.6/8 cache-contract minors + cosmetic (F-142.10/12/15/18/20). See [phase-142 findings](docs/plans/phase-142-consistency-review.md).
...
- **142 (cross-cutting consistency review) — complete:** Playwright UI review + backend code audit; 20 findings cataloged (11 UI + 9 backend) with severity; no fixes made. Deliverable: [phase-142 findings](docs/plans/phase-142-consistency-review.md) — consumed by Phase 143.
- **141 (Collection & engagement tracking consistency) — closed (141.2/141.3/141.4/141.5 done, 141.1 deferred):** 141.2 O(n) activity sweep eliminated (`ObjectIri` column + composite index, `GetActivitiesForObjectAsync`). 141.3 UI like/boost persistence verified PASS via Playwright. 141.4 cross-collection audit: fixed community feed/outbox enrichment gap; 3 minor findings documented (public-feed scope, replies locality gate, `isDisliked` latent gap). 141.5 closeout: regression green. [plan](docs/plans/phase-141-engagement-tracking-consistency.md) · [141.4 change doc](docs/changes/1414-phase141-cross-collection-audit.md).
