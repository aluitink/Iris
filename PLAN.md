...
1. **General UI/UX review** (recurring) — Next pass after improvements land. Last pass (this turn): Home, Notifications, Profile (own + remote), Compose (end-to-end post, HTTP 202), Communities (list + detail) all consistent and functional; 0 console errors. Two minor observations logged (Following-list names not clickable; cosmetic proxy console noise). No defects. [change doc](docs/changes/997-ui-ux-review.md)
2. **139.3 Scenario 10 (data volume growth sanity)** — Seed a larger-than-typical dataset (thousands of posts/activities) and confirm query performance and pagination don't degrade catastrophically; feed/search remain responsive (correctness, not raw speed). (139.3 review scenario after s9.)
3. **139.3 Scenario 11 (retention / right-to-deletion)** — Delete an account (Phase 53.1); confirm the account's content is handled per the documented retention model (Phase 136.19) — no orphaned references, no broken links elsewhere. (Final 139.3 review scenario.)
...
- **140.1 Deletes with embedded signatures — FIXED.** The RFC 9421 validation path rejected Deletes whose header key was unresolvable (actor 410 Gone) or whose header signature failed verification, without checking the body-embedded W3C `RsaSignature2017` proof — but modern Mastodon (4.5+) signs Deletes with an RFC 9421 header signature AND an embedded body proof, so the proof survives after the actor goes 410. The legacy (draft-cavage-03) path already had the fallback; the RFC 9421 path did not. Added the embedded-proof fallback (`TryVerifyEmbeddedProofAsync`) to `ValidateRfc9421Async` at both rejection points (unresolvable header key, failed header verification), mirroring the legacy path. +4 tests (`Rfc9421EmbeddedSignatureDeleteTests`) — the two positive ones confirmed to fail with the fix reverted. [change doc](docs/changes/1401-deletes-with-embedded-signatures.md)
- 139.3-s6 Finding 2 (follow-up): **foreign-IRI object pages — FIXED.** The object-document catch-all reconstructed the lookup IRI as `base + RoutePrefix + path` (a *local* IRI), so a stored foreign object (different host) was unreachable by path → 404. Added an explicit `?iri=` query parameter to `ObjectDocumentHandler` that overrides the path-based reconstruction (only when absolute; a relative/blank `?iri=` is ignored → falls back to path). A stored foreign object is now served by its exact IRI via `GET /ap/v1/object?iri={iri}`; local-object path lookup is unchanged. +6 integration tests (`ForeignObjectDocumentEndpointTests`). Live-verified: a stored Mastodon status 404s by path, 200 via `?iri=` (correct `id`/`type`/`attributedTo`). [change doc](docs/changes/1393-6f2-foreign-iri-object-pages.md)
- 139.3-s6 Finding 1 (follow-up): **bound the outbound connection phase — FIXED.** The server-side outbound transport (innermost handler of every server→server dial) was a plain `HttpClientHandler` whose `ConnectTimeout` fell back to the platform default (35 s, .NET 8+), so an unreachable peer (TCP accepted, TLS stalls) held the dial ~60–70 s and stalled the community feed. Added `internal static ServerOutboundTransport.Create()` (a `SocketsHttpHandler` with an explicit 5 s `ConnectTimeout`, matching the feed's `HttpClientTimeout`; 24 h/2 min pooling; auto-redirect + decompression) and swapped all six server-side outbound registrations in `ActivityPubServerExtensions.cs` (actor-doc fetch, outbound object fetch, remote-collection fetch, community feed, followed feed, delivery worker). +7 unit tests (`ServerOutboundTransportTests`). Live-verified: `technology` community feed (follows lemmy.world) loads ~3 s, 0 console errors. [change doc](docs/changes/1393-6f1-bound-outbound-connection-phase.md)
- 139.3 Data lifecycle & persistence review: Media lifecycle (scenario 8) — **PASS** (+ graceful-degradation fix). (a) Restart survival: 5,257 `Media` rows + 5,239 blobs on the durable `irisweb_iris-media-data` volume; a 287 KB blob served 200 after recreation (the 54.16 ephemeral-dir bug stays fixed). (b) Proxy rewrite: cross-origin banner → `/ap/v1/media/proxy?url=…` 200 jpeg; local avatar served directly; wire form keeps the original remote URL. (c) Dead source: proxy returns 502 (empty body) for a dead source; **fixed** the client gap — `MediaGallery` `<img>`/lightbox now carry `@onerror` (`ImageFailed`) that re-renders a failed tile as a "media unavailable" link-out placeholder instead of a broken-image icon (verified live via Playwright: tile → `media-gallery-item--unavailable`, broken `<img>` removed). Separately logged a document-cache-coherence finding (the local-IRI proxy path serves a cached copy that doesn't re-validate against the store on restart). No new coded tests (web-test policy: verified live). [change doc](docs/changes/1393-8-media-lifecycle.md)

*(Scenarios 2, 3, 4, 6, 7 of 139.3 are in [docs/ROADMAP.md](docs/ROADMAP.md) — see the one-liner entries there.)*
