...
- **64 — request spam & network efficiency.** Reduce the *number* of calls the WASM client makes (not latency). Topics are the 7 rows in the [620 tracker §Phase 64](docs/changes/620-bug-hunt-tracker.md#phase-64--request-spam--network-efficiency-draft-topics). Work them by value (N+1/dup first). **No new coded tests** (WASM manual-test policy) — verify live by counting requests on a fresh origin.
  ...
  **remaining:** optional minted-id extension (render the minted Like/Announce IRI on the object so the per-engaged-card id-recovery walk disappears — a bounded server addition, deferred), #6 (public feed double-fetch, PagedCollection.razor), #7 (deleted-account avatar 410s, NotificationRow.razor).
...
| 64 | Request spam & network efficiency — cut redundant/duplicate calls on load (7 topics in [620 tracker](docs/changes/620-bug-hunt-tracker.md#phase-64--request-spam--network-efficiency-draft-topics)) | **in progress** (64.1 actor-coalescing: #1+#2; 64.2 likes/shares fan-out: #4+#5, 10+10→5+5; 64.3 feed content-dedup: #3; remaining optional minted-id ext, #6, #7) |
