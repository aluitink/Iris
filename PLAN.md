...
- **4 open + 1 new. No blockers (all S2/S3-sev).** (Pass 93: S22 root cause identified — follow activities exist in Activities + BoxItems but API `type=Follow` returns 0; other types work (Like: 13, Announce: 77, Mention: 3); S4 re-confirmed 19th pass; S17 re-confirmed 39 outbox requests; S16-UX re-confirmed.)
- **Top priority:** S22 (follow notifications not shown — ROOT CAUSE: `/local/v1/notifications?type=Follow` doesn't resolve local Follow activities from inbox; 7 Follow activities exist in DB targeting andrew but API returns 0), S19 facet 2 (Accept/Decline buttons — CANNOT RE-VERIFY, blocked by S22), S4 (remote communities missing from Following tab — 19th pass), S17 (profile tabs over-fetch — 39 outbox requests for 3 tabs), S16-UX (poll "You voted" badge missing on object detail page). S2 (proxy 401 — 2 errors, no change) / S14 (proxy 401 + CSP on actor detail) still blocked on dev.
- **Last pass:** 93 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S20 FIXED; S21 FIXED; S2 partially improved; S16-UX partially improved; S19 facet 1 FIXED; S19 facet 3 FIXED; S19 facet 2 CANNOT RE-VERIFY (blocked by S22). Remaining open: S2, S4 (remote), S14, S16-UX, S17, S18 (partial), S19 (facet 2), S22 (follow notifications not shown — root cause identified).
