...
- **62.4 — Clean-entry re-pass + 62 closeout.** 62.3 is done: all remaining bug-class findings are **fixed + live-verified** on a DB-backed same-origin server (`localhost:8089`, `IRIS_ADVERTISE_BASE=https://iris.luit.ink`), and two deeper S1 defects found while re-verifying were also fixed. **62.3 fixes (all in the tracker with Verify cells):** **B-003** (S2) — Directory card `aria-label` leaked a `RangeSelectIterator` type name (`DirectoryCard.razor` bound `@Actor.Name`, a `List<string>`); added a `DisplayName` property (first non-empty name → preferred username → `Actor.Id`), cards now expose "Show posts by <name>". **B-010** (S3) — Notifications Delete rows linked to the actor IRI; `NotificationRow.razor` now detects self-delete (`Type=="Delete" && objectIri==actorIri`) and renders a muted "deleted their account" caption. **B-011** (S3) — raw numeric remote IDs shown as names; added `DisplayNameFallback(iri)` (numeric last-segment → URI host, e.g. `fairy.id`). **B-014** (S2) — compose "no POST" was the B-016 CORS block + MCP-input artifact; `@bind` counter updates under a native input event and the signed POST is now same-origin (full Post completion deferred to this re-pass in a real browser). **B-015** (S1, discovered 62.3) — login always "unknown username": `WebAppFactory` bound `IUserAccountStore`/`IInstanceMetadataStore` to the in-memory stores via `TryAddSingleton` *before* `AddEntityFrameworkPersistence`'s `TryAddSingleton<EfUserAccountStore>`, so the in-memory binding shadowed the EF store (first-registration-wins) and the EF host used an empty store; moved the in-memory defaults into the in-memory branch — EF login now finds accounts. **B-016** (S1, discovered 62.3) — cross-origin CORS on every local AP read/write: the 55.2 override set `advertiseBase` to the browser origin, so `SameOriginApHandler` (rewrites only requests matching `_advertiseBase.DnsSafeHost`) never rewrote the FQDN; `Program.cs` now keeps the original FQDN (`canonicalAdvertiseBase`) as the rewrite base (passed to `ActorSessionAccessor` as `rewriteBase` → `SameOriginApHandler`) while the effective base (browser origin) still drives `DialBaseUri`/`ProxyBaseUrl`/`NamespaceIri` (55.2 preserved). **Build clean (0 warn/0 err); full suite green** (one flaky 16s delivery integration test `FollowEdgeConvergenceIntegrationTests.Follow_Unfollow_Refollow_Cycle…` fails intermittently in the full run but passes in isolation and is unrelated to auth). **62.4 work:** (1) full clean-entry re-pass over all 18 routes (signed-in `andrew` + authless) using native DOM input events (never MCP synthetic input for assertions — see tracker Methodology note); (2) complete the B-014 compose Post in a real browser; (3) confirm zero open blocker/bug findings; (4) 62 closeout — change doc, distill 64 topics, review the Test-debt ledger. **Stop condition: two consecutive clean re-passes, or 3 re-passes max — whichever first; if still dirty at 3, roll the remainder into the buffer (66–69).** **Build gotcha to remember:** `apps/Iris.Web.Client/publish/` caches a stale WASM (the `BuildAndCopyClient` target skips re-publish if `blazor.webassembly.js` exists) — clear that dir before a Docker rebuild or you'll test an old client. **Verification env note:** 62.3 verified on a same-origin server (`localhost:8089`); the Docker deploy dials `localhost:8088` with `AdvertiseBase=https://iris.luit.ink` (a dev-deploy config side effect — the app code is correct; a stale `localhost:8088` actor IRI in the dev DB is data drift, not a code defect).
...
- 62.4 — clean-entry re-pass until clean + 62 closeout (change doc, distill 64 topics, review Test-debt ledger) ← active
