...
- [ ] Phase 2 — Client Library (in progress)
  ...
  - `dotnet build Iris.slnx` 0 warnings; `dotnet test Iris.slnx` 204/204; `dotnet format` clean.
  - Next: `JsonLdHandler`/`RetryHandler`; client ↔ `TestServer` integration tests (signing, Basic-auth → PEM, discovery, cache hit/bypass, paged enumeration).
