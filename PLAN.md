...
- **Phase 80 — UI/UX polish + defect hunt (ACTIVE; 80.1 + 80.2 + 80.3-defects DONE).** All spec gaps (F-01–F-31) and interop (Phases 76–79) are closed; the next value was a fresh full-app defect hunt + polish pass over the 18 routes (the established 62/63/64 pattern). Playwright-driven (WASM manual-test policy): no new coded web tests. 80.1 (defect hunt) + 80.2 (bug-class fixes) + 80.3's defect item (P-004) are all complete — **all 5 findings (P-001…P-005) resolved**: P-001 `ObjectView` `?? parent.Value`, P-002 `Compose` `?? "community"`, P-005 admin-page login redirect (80.2, `247b8bb`); P-004 object-detail parent preview (80.3, this turn); P-003 wontfix-by-design (global sign-in-required app). Change docs: [800 tracker](docs/changes/800-phase80-bug-hunt-tracker.md) · [801](docs/changes/801-phase80-defect-hunt-fixes.md) · [803](docs/changes/803-phase80-p004-object-detail-parent-preview.md). **Remaining (80.3, open-ended):** subjective visual/interaction polish across the 18 routes (spacing, empty/error/loading states, a11y) — a deliberate pass, decide next turn whether to do it or close Phase 80 and move on.

## Active Slice

- **80.3 — UX polish pass (visual/interaction) — P-004 DONE, broader polish remaining.** The defect-class item **P-004** (object-detail "in reply to" link rendered the raw IRI) is fixed + verified: `OnInitializedAsync` preview fetch now targets `ActivityParentIri ?? ParentIri`, and the direct-object branch renders `@(_parentPreview ?? parent.Value)` (mirrors the feed card); verified from a clean entry on a reply's object-detail page (preview shown, IRI in `title`, console-clean). Change doc: [803](docs/changes/803-phase80-p004-object-detail-parent-preview.md). **`remaining:`** subjective visual/interaction polish across the 18 routes (spacing, empty/error/loading states, a11y) — open-ended, a deliberate focused pass. **Also pending (operator/`rm`-allowed turn):** delete 2 untracked Phase 79 scratch artifacts polluting the working tree — `tests/Iris.Core.Tests/ScratchInteropInspectionTests.cs` + `.tmp-reflect/` (untracked, don't affect the committed tree). Decide next turn: do the broader polish pass, or declare Phase 80 complete and define the next phase.
...
- **80 — UI/UX polish + defect hunt.** All spec gaps (F-01–F-31) and interop (Phases 76–79) are closed, so the next value is a fresh full-app defect hunt + polish pass over the 18 routes (the established 62/63/64 pattern). Playwright-driven (WASM manual-test policy): no new coded web tests.
  - ~~**80.1** — Fresh full-route defect hunt~~ — DONE (18/18 routes, 5 findings logged in the 800 tracker).
  - ~~**80.2** — Fix blocker + bug-class findings~~ — DONE (P-001/P-002/P-005 fixed + verified; P-003 wontfix-by-design; P-004 deferred to 80.3).
  - **80.3** — UX polish pass. **Defect item P-004 DONE** (object-detail "in reply to" raw-IRI label → parent content preview; change doc [803](docs/changes/803-phase80-p004-object-detail-parent-preview.md)). **`remaining:`** subjective visual/interaction polish (spacing, empty/error/loading states, a11y) — open-ended; decide next turn whether to do it or close Phase 80 and define the next phase.
...
- **80.3 (defects) — P-004 object-detail parent preview — DONE** ([change doc 803](docs/changes/803-phase80-p004-object-detail-parent-preview.md)): the object-detail "in reply to" link now shows the parent's content preview (not the raw IRI) — `OnInitializedAsync` preview fetch targets `ActivityParentIri ?? ParentIri`, direct-object branch renders `@(_parentPreview ?? parent.Value)`. **All 5 Phase 80 findings (P-001…P-005) now resolved.** Verified from a clean entry (preview shown, IRI in `title`, console-clean); build 0 warn/0 err; suite 0 failed. 80.3's broader subjective polish remains open-ended.
- **80.2 — Phase 80 bug-class fixes — DONE** ([change doc 801](docs/changes/801-phase80-defect-hunt-fixes.md)): fixed + verified P-001 (`ObjectView` `?? parent.Value` literal → `@(_parentPreview ?? parent.Value)`), P-002 (`Compose` `?? "community"` literal → `@(CommunityName ?? "community")`), P-005 (removed `[Authorize(Policy="Admin")]` from 4 admin pages → friendly "not an admin" message restored, no redirect). P-003 wontfix-by-design (global sign-in-required app); P-004 deferred to 80.3. Build 0 warn/0 err; full suite 0 failed; each fix re-verified from a clean entry on the republished WASM.
- **80.1 — Phase 80 full-route defect hunt — DONE** ([tracker 800](docs/changes/800-phase80-bug-hunt-tracker.md)): all 18 routes visited (signed-in `andrew` + authless), each console-checked. 5 findings logged (P-001…P-005), no S1 blockers.
