...
1. **General UI/UX review** (recurring) — Next pass after improvements land. Pass 9 (2026-09-19): verified the 138.27-S2 vote-bar generalization (16 vote bars on Lemmy community content), boost-card parity (tinted banners + moderation + stretched link), notification full-card render (Announce shows "Boosted by" + post-only timestamp, no duplicate), and the signed-out auth bar — clean entry, authless + signed-in, 0 new defects (only external lemmy.ml 500 / proxy 401 console noise). [change doc](docs/changes/14813-phase148-votebar-generalization-and-ui-polish.md)
