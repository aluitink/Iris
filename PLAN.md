...
- **8 open. No blockers (all S2/S3-sev).** (Pass 57: S19 re-confirmed — Edit community no-op re-confirmed on `qa-pass46-test` (0 Update activities in DB, checkbox state not persisted); notification action buttons still missing (3 follow requests pending, none actionable).)
...
- **Last pass:** 57 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8 fixed; S18 partially fixed; S21 partially resolved (caching); S2 partially improved (11→2 errors); S16-UX partially improved (vote count consistent, badge visible in profile). Remaining open: S2, S3, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21.
