...
- **②④⑤ Community simplification — unify members with followers (Lemmy-shape). ② Phases 1-6 DONE; ④ control surfaces (partial) + ⑤ live-interop re-verify remain.** Members = followers, Join/Leave → Follow/Undo, `manuallyApprovesMembers` gates the Follow, single **Follow** button (labeled **Join**/**Leave** for communities), dead `ICommunityStore` member methods retired (kept `EdgeKind.CommunityMember` for the startup migration). **Peering** is the single owner-only extra. **④ done:** "My communities" tab on `/communities` (owned communities + Delete). **④ remaining:** Peers section per owned community; co-owner Leave flow. **⑤** live Lemmy interop re-verify (gated by `IRIS_LIVE_INTEROP`). See [docs/plans/community-simplification.md](docs/plans/community-simplification.md) + [change doc](docs/changes/1593-community-management-my-communities-tab.md).
...
- **4 open. No blockers (all S2/S3-sev).** (Pass 90: S20 FIXED — Communities tab fires `?source=communities`; S21 FIXED — `/c/{handle}` redirect active; S19 facet 1 FIXED — Requests tab loads; S19 facet 2 CANNOT RE-VERIFY — follow requests gone from DB; S4 re-confirmed 16th pass; S17 re-confirmed 39 outbox requests; S16-UX re-confirmed; S19 facet 3 STILL OPEN — Edit Save not persisted.)
- **Top priority:** S19 facet 3 (Edit community Save not persisted — Update 202 but DB unchanged), S19 facet 2 (Accept/Decline buttons — CANNOT RE-VERIFY, follow requests gone from DB; need new test accounts), S4 (remote communities missing from Following tab — 16th pass), S17 (profile tabs over-fetch — 39 outbox requests for 3 tabs), S16-UX (poll "You voted" badge missing on object detail page). S2 (proxy 401 — 2 errors, no change) / S14 (proxy 401 + CSP on actor detail) still blocked on dev.
- **Last pass:** 90 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S20 FIXED (Communities tab fires `?source=communities`); S21 FIXED (auto-follow + /c/{handle} redirect); S2 partially improved (11→2 errors); S16-UX partially improved (vote count consistent); S19 facet 1 FIXED (Requests tab loads); S19 facet 2 CANNOT RE-VERIFY (follow requests gone); S19 facet 3 STILL OPEN (Edit Save not persisted). Remaining open: S2, S4 (remote), S14, S16-UX, S17, S18 (partial), S19 (facets 2+3).
