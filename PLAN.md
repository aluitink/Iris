...
1. **139.4 UI/UX & accessibility review** — All 19 routes verified for items 1-9, 11 (states, deep links, authless, console, network, keyboard, ARIA, responsive). Item 10 (contrast): 0 violations on 8 routes (dark theme). Item 12 (design tokens): ~30 hardcoded CSS values found in secondary surfaces. Remaining: item 10 for 11 routes + 4 cross-page scenarios + fix ~30 hardcoded CSS values. [plan](docs/plans/139-4-ui-ux-accessibility-review.md)
2. **General UI/UX review** (recurring) — Next pass after improvements land. Pass 2 (2026-09-19): Home, Notifications, Profile (own + remote), Compose (end-to-end post, HTTP 202), Communities (list + detail) all consistent and functional. 0 new defects. Console: 2 known cosmetic 404 proxy errors. [change doc](docs/changes/997-ui-ux-review.md)
