...
S99 | OPEN-QA | dev2 | No theme/dark-mode setting in Settings (no Appearance section) — FIXED: added a per-account Appearance/Theme setting. Server: NotificationPreferences.Theme (dark|light) + NotificationPrefsRequest.Theme; GET /local/v1/account/notification-preferences returns theme, PUT stores it (normalized: anything but "light" -> "dark"); round-trips through InMemoryUserAccountStore. Client: new Settings "Appearance" tab (Dark/Light radios); choosing a theme applies data-theme="light" on <html> immediately (optimistic) + PUTs the pref + persists to localStorage. Light theme = :root[data-theme="light"] token overrides in app.css (all components already read the tokens, so it re-skins the whole app); an early <head> script in index.html applies the saved theme before first paint (no dark flash), with dark as the unstyled default. Verified: theme applies live + persists (DB NotificationPrefsJson {"Theme":"light"}), GET round-trips, reverting to dark works, early-paint light on hard reload, Content-tab notif toggle does NOT clobber theme, 0 console errors. No new tests (UI + additive model field; verified live per protocol). Full suite 1521/1496 pass/25 skip/0 fail
