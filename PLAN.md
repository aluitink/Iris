...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` (per-xunit-collection, build-the-host-once) + per-method persistence+cache reset + `ServerRefFor` deferred lookup are in place. **Seven RISKY single-instance classes converted so far** (across two partials): Community (11), Object (11), CommunityModeration (12), CommunitySearch (9), CommunityOutboxCollection (5), CommunityFeedCorrectness (4), InteractionCollection (9). Full fast suite green (899/899). **remaining:** (a) ~6 RISKY single-instance classes with custom clients (Mutes, Blocks, Relays, CommunityJoinRequest, Flags, Reply — need client rebuild in `InitializeAsync`); (b) two-instance federation classes (~20 — cross-wired delivery transports, reset covers both instances). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/256](docs/changes/256-29.3-clean-risky-classes-serverref.md).
