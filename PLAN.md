...
**Phase 27 — protocol conformance & edge-case hardening (in progress).** 27.1 (cross-instance `Update` propagation — a real code fix: wired the outbox-publish `Update` branch to `UpdateActivityHandler` + tightened the owner guard to require `attributedTo` for all actors) and 27.2 (inbound `Move` with `iris:` extension round-trip — test-only lock: the ActivityStreams `ExtensionData` already preserves `iris:` extensions through the wire round-trip) are done. 27.3 (cross-instance `Undo(Like)` of a remote object) remains.

## Active Slice

None in progress — 27.2 is complete.
...
>
> 1. **27.3 — cross-instance `Undo(Like)` of a remote object:** verify that undoing a like of a remote object (object IRI on a different instance) federates correctly — the 24.1 fix covered `Like`/`Announce` of remote objects (resolving the object's author for the delivery target); lock that the `Undo(Like)` path also resolves the author correctly.
