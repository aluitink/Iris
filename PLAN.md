...
**88.5+ — Phase 88 remaining gap-fix slices (next).** 88.1–88.4 are complete (matrix reconciled, remaining feature areas live-verified, key/algorithm info added, actor pages made public). The next slice is the first of 88.5+: the highest-value remaining feature-matrix gap (candidates in priority order — see Up Next 88.5+). Playwright-driven; **no new coded web tests** (WASM manual-test policy).

**Note for the loop (stale-WASM, re-confirmed 88.4):** after a `.razor` change, the server `Iris.Web.csproj` `BuildAndCopyClient` target only re-publishes when `Iris.Web.Client/publish/.../blazor.webassembly.js` is *missing*, and the `wwwroot` copy uses `SkipUnchangedFiles`. The reliable path is `docker compose -f apps/Iris.Web/docker-compose.yml build --no-cache iris-web && up -d --force-recreate iris-web` — the **Dockerfile's build stage `rm -rf`s the stale `publish/`+`bin/`+`obj/` and re-publishes fresh** (the host-side stale `publish/` is irrelevant to the container). Then verify in a **fresh Playwright context** (`browser.newContext()`) — the default MCP page caches the content-hash WASM across rebuilds.
...
> **CRITICAL RUNTIME RULE FOR WEBPAGE ACCESS:** You must prevent stale data or frontend caching on every action. For every new task, navigation, or data-refresh step, execute a **hard reload / cache bypass** *before* reading any data or evaluating elements — navigate with the `networkidle` lifecycle wait and do not proceed until all fresh server network requests have completely settled. Never rely on previously opened states, in-memory page references, or a cached (content-hash) Blazor WASM; when re-verifying after a rebuild, use a fresh browser context.
...
- **88 — Feature-matrix reconciliation + gap verification (next phase).** All phases through 87 are complete, but the feature matrix (`docs/plans/production-app-feature-matrix.md`) is stale — its checkboxes were never updated as features landed over Phases 32–87. This phase reconciles the matrix against the actual implementation (mark each box A/C/D done as verified) and live-verifies the remaining genuinely-unverified features (Playwright-driven per the WASM manual-test policy — no new coded web tests). Any real gaps found become fix slices.
  - ~~**88.1** — Reconcile the feature matrix~~ — **DONE**. Code-inspection + live-app verification of all ~40 feature rows; updated the matrix's A/C/D boxes (33 ✅ / 6 🟡 / 1 ❌). Gaps: key/algorithm info (missing), View-others-profile (auth-gated, not public), infinite-scroll (button not auto), follow-request queue (no dedicated endpoint), community moderation/search (no client UI), user role management (missing).
  - ~~**88.2** — Live-verify the remaining unverified feature areas~~ — **DONE** ([882](docs/changes/882-phase88-live-verification-remaining-feature-areas.md)). Community create + detail (owner, 4 tabs) + per-user moderation lists all pass live. Instance admin code-verified (no admin session available). 0 console errors.
  - ~~**88.3** — Add read-only key/algorithm info to Settings~~ — **DONE** ([883](docs/changes/883-phase88-read-only-key-algorithm-info.md)). `GET /local/v1/account/key-info` (resolves the actor key from `IKeyStore` by `{actorIri}#key-1`) + a Settings → Account → "Security" subsection showing algorithm/key IRI/RFC 7638 thumbprint (read-only). Live-verified (Rsa + key IRI + thumbprint render; 0 console errors).
  - ~~**88.4** — Make actor detail pages public~~ — **DONE** ([884](docs/changes/884-phase88-public-actor-pages.md)). Removed the `[Authorize]` gate on `/actor`; anonymous visitors now see the profile + Posts/Followers/Following (the `iris` client wraps `SameOriginApHandler` so FQDN IRIs read same-origin, avoiding CORS). Follow/Moderate stay behind sign-in. Live-verified (verifier87 + andrew profiles render with real posts; 0 console errors).
  - **88.5+** — Remaining gaps: user role management (promote/demote), community-scoped block/mute UI, community search UI, mentions filter tab, infinite-scroll (auto-observe), follow-request queue endpoint.
- ~~**87 — Live community verification (Feed → Group, end-to-end)**~~ — **COMPLETE** (87.1 live verification passed; no 87.2 fix slice needed). [871](docs/changes/871-phase87-live-community-verification-feed-to-group.md)
- ~~**86 — Community-type interop (Feed → Group mapping)**~~ — **COMPLETE** (86.1 mapping + 86.2 server verification). [861](docs/changes/861-phase86-feed-to-group-community-mapping.md) · [862](docs/changes/862-phase86-feed-community-server-verification.md)
- ~~**80** / **81** / **82** / **83** / **84** — **COMPLETE** (UI/UX polish, federation hardening, signed federation, Iri correctness, production hardening + scale-out). See Recently Completed + ROADMAP.md.
...
**93 - Profile page** - Mastodon supports an image and icon in the actor object, one is a profile header background and one is the visible user icon on all post. I think in general, we need to increase the size of this to make our look and feel more colorful and visual, When on the actors profile page or an actors detail page it should be large and layed on top of the image. The profile page Your Posts section seems to be broken, I no longer see items. The Replies tab is showing me replies for other peoples posts. The Likes tab is also empty. Edit profile, the checkbox for require approval is oddly placed.
**94 - Development mode anti-forge tokens** - Create a way to configure disable the anti-forge tokens, it seems to cause some caching issues, ideally when we are playwright testing, we would disable this to ensure we don't get stuck with an stale anti-forge token.
## Inbox
...
- **88.4 — Make actor detail pages public — DONE** ([884](docs/changes/884-phase88-public-actor-pages.md)): removed the `[Authorize]` gate on `/actor` so logged-out visitors can view an actor's profile + Posts/Followers/Following. Anonymous reads use the `iris` client wrapped in `SameOriginApHandler` (FQDN IRIs rewritten to same-origin to dodge CORS); Follow/Moderate stay behind sign-in. Live-verified (verifier87 + andrew profiles, real posts, 0 console errors). **0 new coded web tests** (WASM manual-test policy).
- **88.3 — Read-only key/algorithm info in Settings — DONE** ([883](docs/changes/883-phase88-read-only-key-algorithm-info.md)): closed the one ❌ missing feature-matrix item. `GET /local/v1/account/key-info` (resolves the actor key from `IKeyStore` by `{actorIri}#key-1`) + a Settings → Account → "Security" subsection showing algorithm/key IRI/RFC 7638 thumbprint (read-only). Live-verified (Rsa + key IRI + thumbprint render; 0 console errors). **0 new coded web tests** (WASM manual-test policy).
