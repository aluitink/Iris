...
- **3 open + 1 fixed-pending-reverify. No blockers (all S2/S3-sev).** (Pass 94: S4 re-confirmed 20th pass; S17/S16-UX unchanged; Peers tab verified working; session hydration delay confirmed — feed takes ~15s to render after login, some pages flash "Sign in" before session loads. User report matches this behavior. S22 FIXED — notification follow-filter fix deployed `ee47565`.)
- **Top priority:** S22 (follow notifications — FIXED, pending re-verify), S19 facet 2 (Accept/Decline buttons — was blocked by S22, now unblocked), S4 (remote communities missing from Following tab — 20th pass), S17 (profile tabs over-fetch — 39 outbox requests for 3 tabs), S16-UX (poll "You voted" badge missing on object detail page). S2 (proxy 401 — 2 errors, no change) / S14 (proxy 401 + CSP on actor detail) still blocked on dev.
- **Last pass:** 94 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S20 FIXED; S21 FIXED; S2 partially improved; S16-UX partially improved; S19 facet 1 FIXED; S19 facet 3 FIXED; S19 facet 2 CANNOT RE-VERIFY (blocked by S22). S22 FIXED (notification follow-filter fix deployed `ee47565`). Remaining open: S2, S4 (remote), S14, S16-UX, S17, S18 (partial), S19 (facet 2 — unblocked), S22 (fixed, pending re-verify).
