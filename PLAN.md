...
- **Phase 80 — UI/UX polish + defect hunt (ACTIVE; 80.1 + 80.2 DONE).** All spec gaps (F-01–F-31) and interop (Phases 76–79) are closed; the next value is a fresh full-app defect hunt + polish pass over the 18 routes (the established 62/63/64 pattern). Playwright-driven (WASM manual-test policy): no new coded web tests. 80.1 (defect hunt) + 80.2 (bug-class fixes) complete — 5 findings (P-001…P-005), 3 bug-class fixed + verified (P-001 `ObjectView` `?? parent.Value`, P-002 `Compose` `?? "community"`, P-005 admin-page login redirect); P-003 wontfix-by-design (global sign-in-required app); P-004 deferred to 80.3. Change docs: [800 tracker](docs/changes/800-phase80-bug-hunt-tracker.md) + [801](docs/changes/801-phase80-defect-hunt-fixes.md). Remaining: 80.3 (UX polish).

## Active Slice

- **80.3 — UX polish pass (visual/interaction).** Polish the 18 routes for visual/interaction quality: spacing, states (empty/error/loading), a11y. **Known items to address:** P-004 (object-detail "in reply to" link renders a raw IRI when the parent is unfetched — add a preview/handle, mirroring the feed-card fix). Re-verify from a clean entry after each change; update the 800 tracker + this file at the end of the slice.
...
- **80 — UI/UX polish + defect hunt.** All spec gaps (F-01–F-31) and interop (Phases 76–79) are closed, so the next value is a fresh full-app defect hunt + polish pass over the 18 routes (the established 62/63/64 pattern). Playwright-driven (WASM manual-test policy): no new coded web tests.
  ...
  - **80.3** — UX polish pass (visual/interaction: spacing, states, empty/error/loading, a11y) — includes P-004 (object-detail "in reply to" raw-IRI label → preview/handle).
