...
- **⑤ Mastodon un-follow fan-out — IN PROGRESS (2026-09-20).** The un-follower's home feed still showed the unfollowed user's posts for the feed-cache TTL (30 s) after an un-follow. Fix: `IFollowFeedService.InvalidateActorFeedCache` + call it in `OutboxPublishHandler` on a `Follow` (gained a target) and an `Undo` of a `Follow` (lost a target — the un-follow). Code + regression test done; full fast suite green (1402 passed, 0 failed). Deployed (container healthy); QA to re-verify the un-follower's `/feed` drops the unfollowed user's posts on the next read. [change doc 14815](docs/changes/14815-unfollow-feed-cache-invalidation.md)
...
- **⑤ Mastodon un-follow fan-out fix (2026-09-20):** after an un-follow, the un-follower's home feed (`GET /u/{handle}/feed`) served the **stale** per-actor feed cache (30 s TTL) and kept showing the unfollowed user's posts. Added `IFollowFeedService.InvalidateActorFeedCache` (no-op default) + `FeedService` impl (`_feedCache.TryRemove`), and call it in `OutboxPublishHandler` on a `Follow` (following set gained a target) and an `Undo` of a `Follow` (lost a target — the un-follow). The delivery side was already correct (QA 993 verified the edge removal + `Undo` delivery + Mastodon `StatusReachFinder` no longer targeting); this closes the client-facing feed-cache gap. New regression test (`Feed_Cache_InvalidateActorFeedCache_DropsOnlyThatActor`) drops only the named actor's entry. Full fast suite green: 1402 passed, 0 failed. Deployed (container healthy). [change doc 14815](docs/changes/14815-unfollow-feed-cache-invalidation.md)
- **Lemmy Undo (unfollow) 400 fix (2026-09-20):** the outbound Undo of a Follow now **embeds the original Follow** (Lemmy's untagged-enum ActivityStreams parser rejects a bare IRI link → 400 → the un-follow was dead-lettered). Person + community outbox paths both fixed; the stored activity keeps the bare link (only the outbound delivery materializes the embedded form — backward-compatible, Iris inbound `UndoActivityHandler` accepts either). New regression test asserts the embedded shape. Full fast suite green: 1401 passed, 0 failed. **Live-verified:** Follow→200 (count 1→2), Undo→200 (count 2→1, no dead-letter). [change doc 14814](docs/changes/14814-lemmy-undo-follow-embedded.md)
