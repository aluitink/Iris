...
- [ ] Phase 2 — Client Library (in progress)
  ...
  - Next: Phase 3 — Server Foundation (persistence interfaces, `AddActivityPubServer`/`MapActivityPubEndpoints` with versioned route prefix, actor doc with conditional `privateKey`, WebFinger/NodeInfo, server caching, + the Basic-auth → PEM + inbound-signature-validation integration tests deferred from Phase 2).
