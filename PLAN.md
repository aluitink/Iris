...
| 11 — Implementation Gaps & Usability Exploration | 🚧 in progress | Walk every feature end-to-end as a user; catalog gaps; expand integration tests to cover realistic journeys + error paths. Slice 11.1 closed the carried-forward client/server page-1 interop gap (server page-1 `OrderedCollection` now emits `next` via `ExtensionData`; client reads it to walk past page 1; `ClientServerCollectionInteropTests`, 478→482). Slice 11.2 walked all eight capabilities end-to-end as a user (research-only; new `docs/PHASE_11_USER_JOURNEYS.md` + J-1…J-22 usability-friction register). Slice 11.3 closed J-21 — `IrisClientBundle` now exposes `Discovery` + `ResolveActorAsync` (default WebFinger-backed, reusing the bundle's cache, `WithDiscovery(...)` override), 4 unit + 1 e2e tests (482→486); the headline is that the *write* path — post/follow/follow-a-community — is not reachable through the client (J-6/J-9; J-21 now resolved). |
