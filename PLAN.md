...
**Phase 31 — explorer UI & server namespace polish.** 31.1 (WebFinger RFC 7033 host handling + client base-URI retry) complete — server 404s a non-matching host; client retries a failed dial-base resolution against the account's own host. Next: 31.2 (`PagedCollection` initial-load `StateHasChanged` fix). Phase 30 (30.2 health check + readiness probe) resumes after the Phase 31 Inbox is drained.
...
2. 31.2: **`PagedCollection` initial load never re-renders — "Loading…" stuck until Refresh is clicked (feed + actor detail).** Root cause confirmed (live MCP Playwright review, 2026-09-05): `PagedCollection.razor`'s `OnParametersSet` fires `_ = LoadInitialAsync()` (line 181) as a **fire-and-forget** task. `LoadInitialAsync` (lines 185–211) sets `Items = null` (the spinner state), awaits the fetch, and populates `Items` on completion — but **never calls `StateHasChanged()`**. Because the load is not an `@onclick` handler (which Blazor auto-re-renders) and is not awaited by `OnParametersSet`, the render pass that committed the "Loading…" spinner is never re-run after `Items` is populated. The fix: add `StateHasChanged()` at the end of `LoadInitialAsync` (after the empty-state fallback, ~line 210), mirroring what Blazor does automatically for the `@onclick` handlers `LoadMoreAsync`/`RefreshAsync`. This one fix repairs **every** `PagedCollection` instance: the followed-feed card (Feed.razor) **and** all the actor-detail collections (outbox, liked, followers, following, inbox, mutes, blocks, flags — ActorDetail.razor), which all showed the same stuck "Loading…". Confirming test: clicking Refresh while the spinner shows makes the items appear (RefreshAsync is an `@onclick` handler → auto-`StateHasChanged`); after the fix the initial navigate should render without a Refresh click.
3. 31.3: **Sample should not seed fake `remote.example` activities.** The sample seeds `carla` on the `remote.example` host label with her own note/reply/like and follow edges, making the seeded graph look like cross-instance federation when it is all one server. Remove the fake remote-host seed (or gate it behind an explicit opt-in) so the sample's default data is honest.
4. 31.4: **Actors page — directory search should return only actors; rename to "Directory".** `/actors`' whole-directory search currently runs the instance-wide `/ap/v1/search`, which also returns content (notes). It should search the actor directory only (actors/communities), and the page should be renamed "Directory" (nav + title) to match its purpose.
5. 31.5: **Actor detail — break up into tabs + profile-style header.** `/actor` is one long page (outbox, liked, followers, following, inbox, moderation counts/collections, follow/unfollow, inbound follows, moderation writes, relays, raw inspector). Organize it into tabs, and make the top look like an actor profile (show the actor's icon/picture from the document, not just the raw object view).
...
- 31.1: **WebFinger RFC 7033 host handling + client base-URI retry** — server 404s a non-matching `host` (even for an existing handle); client retries a failed dial-base resolution against the account's own advertised host when the hosts differ. 6 new tests. Full fast suite green (1422/1422). → [docs/changes/274-31.1-webfinger-rfc7033-host-and-retry.md](docs/changes/274-31.1-webfinger-rfc7033-host-and-retry.md)
