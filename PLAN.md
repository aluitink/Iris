...
**Phase 31 — explorer UI & server namespace polish.** 31.1–31.9 complete (WebFinger RFC 7033 + retry, `PagedCollection` initial-load re-render, carla fake-remote seed gated off, directory actors-only + rename, actor-detail profile header + tabs, community profile header + tabs, the shared `CollectionBrowser` component, the server `iris:` namespace derived from the public base URI + hosted namespace document, and the followed-feed `/feed` verification — 6 integration tests confirm the feed returns the followed actors' data, every activity type, for local + remote follows, in-memory + file-backed; no server change). Next: 31.10 (remote Likes not recorded). Phase 30 (30.2 health check + readiness probe) resumes after the Phase 31 Inbox is drained.
