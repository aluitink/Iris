...
- **64 — request spam & network efficiency.** Reduce the *number* of calls the WASM client makes (not latency). Topics are the 7 rows in the [620 tracker §Phase 64](docs/changes/620-bug-hunt-tracker.md#phase-64--request-spam--network-efficiency-draft-topics). Work them by value (N+1/dup first). **No new coded tests** (WASM manual-test policy) — verify live by counting requests on a fresh origin.
  ...
  **Checkpoint (turn 3): 64.3 home-feed content-object coalescing — topic #3 done.** The home timeline rendered a single post **twice** — once as the author's `Create` (note embedded, from the followed actor's outbox) and once as a follower's `Announce` (boost, note link-only, from the viewer's own outbox which is prepended). The feed's by-IRI de-dup couldn't remove them (distinct activity IRIs) and the client's `IsContentItem` renders both → 2 engagement bars → 2× the per-card `/likes`+`/shares` walks. **Fix:** `FeedService.TruncateDedup` now coalesces **by content-object IRI** too — a `Create`/`Announce` referencing the same object keeps one representative, preferring the item carrying the object **embedded** (the author's `Create`, renders content + `iris:` counters in place) over a link-only reference (the boost); the representative keeps its first-seen position (stable ordering); the `MaxItems` cap applies last. **Verified:** 5 new `FeedServiceTests` (embedded-Create wins over link-only-Announce; two link-only Announce → first wins; ordering-independent promotion; `Like`+`Create` both kept; no over-dedup across objects) + live on fresh origin `:8095` (`GET /ap/v1/u/andrew/feed`, the exact endpoint the WASM home timeline fetches): the RayvenMX status now appears **once** (the embedded `Create`), the boost `Announce` is dropped, **0** duplicated content-object IRIs in the feed, `Like`s preserved. Full fast suite green (Iris.Server.Tests 974/975, one known-flaky delivery test passes in isolation; Iris.Web 62/62). WASM client unchanged (no republish needed).
  **Checkpoint (turn 2): 64.2 per-item likes/shares fan-out — topics #4 + #5 done.** The EngagementBar's 54.8 fast path (seed counts from the server-rendered `iris:likedCount`/`sharedCount`/`isLiked`/`isShared` and skip the count-walk) was **not engaging** — the full `/likes` + `/shares` walk fired per card (10+10 on `/home`). **Root cause:** `IActorSessionAccessor.IrisNamespaceBase` derived the `iris:` namespace from the multi-instance-**rewritten** dial base (`_advertiseBase ?? _browserBase` = `http://localhost:PORT/ns#`), but the server writes the counters under the **canonical FQDN** namespace (`https://iris.luit.ink/ns#`). Mismatch → `GetLikedCount`/`GetSharedCount` returned null → fast path skipped. **Fix:** `IrisNamespaceBase` now derives from `_rewriteBase ?? _browserBase` (`_rewriteBase` holds the canonical FQDN — the pre-rewrite original `Program.cs` passes as `rewriteBase`). **Verified live on fresh origin `:8093`** (republished client + cookie-authenticated `/home` as `andrew`): `/likes` **10 → 5**, `/shares` **10 → 5** — the un-engaged cards now seed counts from the embedded counters and issue **no** walk; the residual 5+5 is only the ~4 items andrew liked/boosted (engagement-proportional id-recovery, not feed-size N+1). Stable across two reloads; **0 console errors**; timeline renders (10 cards, counts `0,0` un-engaged / `1,1` engaged). Build 0 warn/0 err; full suite green in isolation.
  **Checkpoint (turn 4): 64.4 public-feed double-fetch — topic #6 done.** On `/` (the authless landing), the public feed's first page (`GET /ap/v1/public/feed?limit=20`) was fetched **twice** on mount. **Root cause:** `Home.razor` rendered the public feed's `<PagedCollection>` **keyless**; `OnInitializedAsync` awaits `Session.EnsureReadyAsync()` (an async auth-state resolve → re-render), and on that re-render the parent's `<NotAuthorized>` child content disposed/recreated the keyless card, resetting its `_loadedFor` and re-firing the initial first-page fetch. **Fix:** a stable `@key="@PublicFeedKey"` (a `const string` field, `"public-feed"`) on the card — mirroring `HomeTimeline.razor`'s proven `@key="@FeedKey"` — keeps the same component instance across the re-render so the initial load fires once. **Razor-generator gotcha (discovered):** `@key="public-feed"` (a string *literal*) and `@key` on any element inside this file's `@if`/`<NotAuthorized>` block do **not** compile under this SDK's Razor source generator (`CS1662` + `CS0246` `Inject`/`NavigationManager` cascade); the key value must be a **C# variable reference** (`@key="@PublicFeedKey"`), as in `Profile.razor`/`HomeTimeline.razor`. **Verified live on fresh origin `:8120`** (anonymous, cookies cleared, `GET /`): `public/feed?limit=20` fetched **exactly once** (was twice); landing renders (hero + "Public timeline" + Refresh button). The only console errors are a CORS `ERR_FAILED` on the feed fetch — a **test-env artifact** of serving from `http://localhost:8120` while `IRIS__ADVERTISEBASE` is the external `https://iris.luit.ink` (the feed IRI is the external FQDN); unrelated to this change, does not occur under the app's real same-origin deployment. Build 0 warn/0 err; `Iris.Web.Tests` 62/62 (WASM-side change → Playwright-verified, no new coded web test per policy).
  **remaining:** optional minted-id extension (render the minted Like/Announce IRI on the object so the per-engaged-card id-recovery walk disappears — a bounded server addition, deferred), #7 (deleted-account avatar 410s, NotificationRow.razor).
...
| 64 | Request spam & network efficiency — cut redundant/duplicate calls on load (7 topics in [620 tracker](docs/changes/620-bug-hunt-tracker.md#phase-64--request-spam--network-efficiency-draft-topics)) | **in progress** (64.1 actor-coalescing: #1+#2; 64.2 likes/shares fan-out: #4+#5, 10+10→5+5; 64.3 feed content-dedup: #3; 64.4 public-feed double-fetch: #6, 2→1; remaining optional minted-id ext, #7) |
