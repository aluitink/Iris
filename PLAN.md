...
- **106 — Profile Improvements (DONE)** — profile page now has Followers and Following tabs (hydrated `ActorCard` via `ActorListPanel`, matching ActorDetail) and the Likes tab shows the liked target as a clickable link to the object view (with content preview when the Like carries an embedded object). **0 new coded web tests** (policy). Full suite **1419 passed / 0 failed**. Live Playwright-verified: Followers shows andrew+bob, Following shows bob, Likes shows the liked note with a working link. 0 console errors. [changes/1061](docs/changes/1061-phase106-profile-improvements.md)

- **105 — Directory Improvements (DONE)** — fixed the "This instance" / "All known" scope toggle: `GlobalSearchService` now accepts the instance base IRI and uses IRI-prefix matching to distinguish local from cached remote actors (replacing the unreliable `preferredUsername` heuristic, which included remote actors from Mastodon/Lemmy that also carry a `preferredUsername`). Falls back to the store heuristic when the base IRI is unavailable. **4 new unit tests**. Full suite **1419 passed / 0 failed**. Live Playwright-verified: "This instance" shows 4 local people only; "All known" shows 5 (adds a localhost:8088 actor from a prior dev run). 0 console errors. [changes/1051](docs/changes/1051-phase105-directory-improvements.md)
