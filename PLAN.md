...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` + `SharedTwoHostFixture` + per-method reset + `ServerRefFor` are in place. **Thirteen RISKY single-instance classes converted** (all single-instance RISKY classes done) + **eighteen two-instance federation classes converted**: UpdatePropagation (3), LikeAnnounceUndoPropagation (2), MuteUndoPropagation (1), CrossInstanceAcceptPropagation (2), CrossInstanceRejectPropagation (2), ModerationUndoPropagation (2), RelayLifecycle (1), PersonFollowsPersonUnfollowPropagation (2), CommunityFollowsPersonUnfollowPropagation (2), CommunityFollowsCommunityUnfollowPropagation (2), MoveExtensionRoundTrip (1), DuplicateInboundDeliveryIdempotency (2), RelayFanOut (3), PostFederation (2), OutboxPublishServerDelivers (2), OutboxRelayFanOut (3), OutboxAnnounceFanOut (5), UpdateDeleteRelayFanOut (3). Full fast suite green (899/899, 5m12s). **remaining:** ~4 two-instance federation classes (OutboxAudience* [3-instance], AnnouncePropagation with DeliveryCounter, OutboxCreateFanOut [3-instance]). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/258](docs/changes/258-29.3-signature-validation-risky-classes.md).
