...
- **138.15 (Lemmy → Iris likes) — verified, no code changes:** the existing `LikeActivityHandler` records a Lemmy upvote on a synced post identically to a remote Mastodon like (like edge in `ILikeStore`; community inbox → members' outboxes). 3 integration tests (`LemmyLikeInboundIntegrationTests`). [Change doc](docs/changes/13815-phase138-lemmy-like-inbound.md).
- **138.14 (Iris reply → Lemmy comment) — verified, no code changes:** Phase 136.7's cross-instance reply integrity already delivers an Iris reply to a remote `Page` parent (Lemmy's post format) to the parent's home instance. 2 two-instance integration tests (`LemmyReplyToPostIntegrationTests`) confirm the reply `Note` is stored on the parent's home + the reply edge is recorded, and that `inReplyTo` carries the `Page` IRI. [Change doc](docs/changes/13814-phase138-iris-reply-to-lemmy-comment.md).
- **138.13 (Lemmy comment threading) — verified, no code changes:** the existing `CreateActivityHandler` already handles inbound Lemmy comments (`Create(Note)` with `inReplyTo`) delivered to a community's inbox: the `Note` is stored in the object store, the reply edge is recorded in the replies store, and the `Create` is recorded in the community's local members' outboxes. 3 integration tests (`LemmyCommentThreadingIntegrationTests`). [Change doc](docs/changes/13813-phase138-lemmy-comment-threading.md).
- **138.12 (Lemmy community relay inbound) — `Announce(Create(Page))` surfaces in Iris feed:** the `AnnounceActivityHandler` now unwraps an embedded `Create` (Lemmy's relay pattern), stores the embedded `Page` in the object store, and records the `Create` in the community's local members' outboxes via `CommunityContentRecorder`; the handler also checks the community store for local recipients. 3 unit tests (`LemmyCommunityRelayIntegrationTests`). [Change doc](docs/changes/13812-phase138-lemmy-community-relay-inbound.md).
- **138.11 (cross-post fidelity check) — live Lemmy acceptance verified:** 4 delivery defects fixed (PEM CRLF, missing `endpoints.sharedInbox`, instance actor type, host-based `IsOnInstance` + Note-level `to` + `Accept` header + `SignatureHeader` spaces); Note-vs-Page resolved (top-level cross-posts carry a `Page`); live Lemmy acceptance verified (cross-posted `Create(Page)` includes `to`/`cc` on the activity itself, satisfying Lemmy's `CreateOrUpdatePage`). [Change doc](docs/changes/13811-phase138-cross-post-fidelity-check.md).
- **138.8 (peering failure-mode drill) — Iris retry/dead-letter path + Lemmy give-up window:** 2 live tests (`PeeringFailureModeTests`, gated on the local Lemmy container): (1) delivery to the real Lemmy shared inbox completes without a transport error (wire path works against a real non-Iris peer); (2) delivery to an unreachable port is dead-lettered with `TransportError` after exactly the configured retry budget — the same path that fires when a peer goes down. Researched and documented Lemmy 0.19's retry behavior: infinite retries with exponential backoff (1.25^(N-1)s, capped at 1 day), `fail_count` persisted in Postgres across restarts. Key asymmetry: Iris dead-letters after 5 attempts (~31s); Lemmy retries indefinitely. [Change doc](docs/changes/13808-phase138-peering-failure-mode-drill.md).
- **138.7 (peering trust/identity checks) — HTTP Signature wire-compatibility with real Lemmy key:** 4 tests in `PeeringTrustIdentityTests` verify Iris's HTTP Signature signing/verification is wire-compatible with a real Lemmy RSA key (public-only load, deterministic base construction, well-formed signatures, lowercase `sha-256=` digest). Live wire test deferred to 138.10. [Change doc](docs/changes/13807-phase138-peering-trust-identity-checks.md).
