...
- **138.5 (Iris → Lemmy community discovery) — cross-instance remote `Group` follow + persist:** 3 integration tests prove the community-follow endpoint drives genuine cross-instance discovery with remote `Group` persistence via the 135.1 `RemoteCommunityPersister` path. [Plan](docs/plans/phase-138-lemmy-community-integration.md).
