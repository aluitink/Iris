...
1. **General UI/UX review** (recurring) — Next pass after improvements land. Pass 3 (2026-09-19): post-139.4 verification — Home, Settings, Compose all consistent and functional after design token migration + 404 fix. 0 new defects. Console: 1 known cosmetic 404 proxy error. [change doc](docs/changes/997-ui-ux-review.md)
2. **139.2 Scenario 5 (audience/visibility) — remaining deferred surfaces** — the S1 read-path gap is now **closed for the public feed, global search, and the follow feed** (a `VisibilityFilter` predicate — public = `as:Public` sentinel or no audience; non-public = visible to a named recipient **or the author** — is wired into all three; anonymous sees public only, a signed requester also sees items addressed to them, and the follow-feed owner sees their own non-public posts). The **follow-feed owner gate** (S5c) is now **done** (403 for non-owners/anonymous; [change doc](docs/changes/1392-5c-follow-feed-owner-gate.md)). Remaining lower-priority surfaces: (a) **object-document endpoint** — add a recipient check (404/403 for a non-recipient on non-public content) — a larger design decision that interacts with federation (remote instances must fetch non-public content to store/deliver it); (b) **federation visibility policy** — suppress non-public content on receipt vs. store-with-visibility-marker. See [change doc](docs/changes/1392-5-audience-visibility-filter.md).
