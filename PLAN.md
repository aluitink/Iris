...
**87.1 — Live community verification (Feed → Group, end-to-end).** Phase 86 (86.1 mapping + 86.2 server verification) is complete. This slice closes the last open 81.2 thread against the **live** app (`http://localhost:8088`, Docker `irisweb-iris-web-1`): seed a local `Feed`-typed community, follow it from `alice`, and confirm the community detail renders + the follow/join round-trips + the directory's Communities tab shows it. Playwright-driven; **no new coded web tests** (WASM manual-test policy). Admin login: `alice` / `alice-password`.
...
- ~~**87 — Live community verification (Feed → Group, end-to-end)**~~ — **COMPLETE** (87.1 live verification passed; no 87.2 fix slice needed). [871](docs/changes/871-phase87-live-community-verification-feed-to-group.md)
  - ~~**87.1** — Seed + verify: provision a local `Feed`-typed community, follow it, and confirm the community detail page renders + the follow/join round-trips + the feed is viewable~~ — **DONE** ([871](docs/changes/871-phase87-live-community-verification-feed-to-group.md)). All verification points passed (server 200 `type:Feed`, UI detail/list/directory render, follow edges 0/10/11, 0 console errors).
...
- ~~**84 — Production hardening & scale-out readiness**~~ — **COMPLETE** (84.1–84.6 all DONE).
  ...
  - ~~**84.2** — Local key-rotation lifecycle~~ — **DONE** (`KeyRotationService`: mint `#key-N`, re-register, re-stamp the actor document's `publicKey` with a `replaces` pointer; `RetireKey`). [842](docs/changes/842-phase84-local-key-rotation-lifecycle.md)
    ...
    - ~~**84.6** — Shared-state scale-out (converging multi-instance)~~ — **DONE** (all three convergence pieces + the 84.5 guard lift: the document-derived key provider + the shared delivery queue + the cache-invalidation channel + the guard lift (fail-fast → warning when all three are configured). Multi-instance over one origin is now supported. **20 new tests.** [846](docs/changes/846-phase84-document-derived-shared-key-provider.md) · [847](docs/changes/847-phase84-key-provider-refresh-hosted-service.md) · [848](docs/changes/848-phase84-shared-delivery-queue.md) · [849](docs/changes/849-phase84-cache-invalidation-channel.md) · [850](docs/changes/850-phase84-lift-single-instance-guard.md).
