...
- **82.2 — Real-world signed delivery (next).** Verify a real signed outbound delivery (e.g. a `Follow` or `Create` from Iris to a **non-permissive** instance such as `mastodon.social`, which rejects unsigned fetches) is **accepted** (a 202 Accepted / queued inbox entry, not a 401/403). This is the live mirror of 81.1–81.3's inbound verification, now on the outbound side: it proves the `SigningHandler` + `ISignatureSigner` (now including the `created` param added in 82.1) produce signatures a real, strict peer accepts over the wire. Approach: use the `IrisSigner` CLI (or a small driver) with a real actor's key to send a signed `Follow`/`Create` to a permissive-accepting but signature-requiring instance, and confirm acceptance (or capture the exact rejection if one occurs). If egress to a strict instance is blocked from the container (as live-capture was in Phase 81), capture the signed request + response on the **host** and record the wire evidence in the change doc. **In-scope for coded tests** where the behavior is core/server-side (e.g. an offline test that the full `SigningHandler` pipeline sets `Date`/`X-Signature-Date`/`Signature`/`Digest` and that the `Signature` verifies); the live delivery itself is verified + documented, not coded.
...
- **82 — Real-world signed federation + production readiness (NEW).** Phase 81 verified inbound from real AP servers; Iris already has outbound HTTP Signatures (`SigningHandler`). Verify + harden the *outbound* side so Iris federates with non-permissive instances (e.g. `mastodon.social`, which rejects unsigned fetches), then move to production readiness. Coded tests for core/server-side behavior; Playwright-driven where UI-facing.
  ...
  - **82.2** — Real-world signed delivery (Active Slice): verify a real signed outbound delivery (e.g. a `Follow`/`Create` to `mastodon.social`) is accepted by a non-permissive instance (the mirror of 81.1–81.3's inbound verification).
  - **82.3** — Key management hardening: per-actor key pairs on the actor doc `publicKey`, the `IKeyProvider` resolution path, key rotation, and `publicKey`↔signing-key consistency.
...
- **Phase 80 — UI/UX polish + defect hunt — COMPLETE** ([800](docs/changes/800-phase80-bug-hunt-tracker.md) · [801](docs/changes/801-phase80-defect-hunt-fixes.md) · [803](docs/changes/803-phase80-p004-object-detail-parent-preview.md) · [804](docs/changes/804-phase80-broader-ux-polish-verification.md)): full-app Playwright defect hunt over 18 routes; 5 findings (P-001…P-005) all resolved; broader UX polish (empty/loading/error + a11y) verified excellent — no code changes.
