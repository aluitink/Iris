...
2. **139.2 Scenario 5 (audience/visibility) — remaining deferred surfaces** — the S1 read-path gap (public feed + global search) is now **closed** (a `VisibilityFilter` predicate is wired into both surfaces; anonymous sees public only, a signed requester also sees items addressed to them). Remaining lower-priority surfaces: (a) **follow feed** — gate the request to the actor's owner / signed follower and filter the merged follows' outboxes by audience (owner-scoped by construction, so lower exposure); (b) **object-document endpoint** — add a recipient check (404/403 for a non-recipient on non-public content) — a larger design decision that interacts with federation (remote instances must fetch non-public content to store/deliver it); (c) **federation visibility policy** — suppress non-public content on receipt vs. store-with-visibility-marker. See [change doc](docs/changes/1392-5-audience-visibility-filter.md).
...
- **139.2 Scenario 5 (audience/visibility) — S1 read-path gap CLOSED (public feed + global search).** Iris used pure AS2 `to`/`cc` with no visibility filter, so a DM (`to=[bob]`, no `as:Public`) was as visible as a public post — it appeared in the public timeline and global search to everyone. Added `VisibilityFilter` (public = `as:Public` sentinel or no audience; non-public = visible only to a named recipient) and wired it into `GET /ap/v1/public/feed` (handler threads the already-resolved requesting actor into `PublicFeedService`, filtered before truncate) and `GET /ap/v1/search` (handler gains `ISignatureValidator`; search always loads + filters content and computes the total from the visible set). Anonymous/unsigned = public only; a signed request also sees items addressed to it. Actors unaffected. +16 tests (`VisibilityFilterTests`); updated `DirectPost_..._CurrentGap` to assert the fixed behavior. Follow feed, object-document, and federation policy remain deferred. [change doc](docs/changes/1392-5-audience-visibility-filter.md)
