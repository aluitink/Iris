...
- **Deployed commit:** `f728af4` (S4 communities-following-remote fix, on top of `0fe7f0f` S2/S14 anonymous-proxy signing + `cffe397` ⑤ un-follow feed-cache invalidation). Rebuilt (`--no-cache`) + redeployed 2026-09-20, `irisweb-iris-web-1` recreated, healthy (WASM `ffbmuf07de`, contains `ResolveFollowingCommunitiesAsync`). **Live-verified (S4):** signed-in `andrew` Communities → Following shows local communities **plus** the remote **"Iris Interop"** (`lemmy.luit.ink/c/interop`) with a Leave button; client `POST /ap/v1/proxy/…/lemmy.luit.ink/c/interop` → 200; 0 console errors.
