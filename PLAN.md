...
- **63.1 — UI/UX review (usability + presentation; detailed visual pass).** Phase 62 (bug hunt) is **COMPLETE** — all blocker/bug findings fixed and the 62.4 clean-entry re-pass converged (two consecutive clean passes on a DB-backed same-origin server); see change doc [621](docs/changes/621-phase62-closeout-bug-hunt.md) and the [620 tracker](docs/changes/620-bug-hunt-tracker.md). 63 is the detailed visual/usability review of the production app: go route-by-route (signed-in `andrew` + authless) and evaluate **presentation and usability**, not correctness (correctness is 62's job, done). Look for: layout/spacing/alignment issues, contrast/readability, inconsistent typography, broken or awkward empty states, confusing labels/copy, missing affordances, poor responsive behavior, hover/focus states, and anything that looks or feels off. **Method (binding, from 62's lesson):** drive the app with **native DOM input events** via `page.evaluate` (`el.click()`, native value-setter + `dispatchEvent(new Event('input'))`) — **never** MCP `browser_click`/`browser_type` for an assertion (synthetic clicks do not reliably fire Blazor WASM `@onclick`). Watch the console for errors. Log every finding in a **new 630 UX tracker** (`docs/changes/630-ux-review-tracker.md`) with class `ux` (so it routes back here) + severity, and route any *perf* observation to 64. **Timebox:** ~5 routes per turn — stop, checkpoint, continue next turn; roll overflow into the buffer. Fix small UX nits in-slice (CSS/razor only, no behavior change); defer structural redesigns to 63.2+.
  ...
  **Checkpoint (turn 1):** Reviewed signed-in routes **Home, Search (empty state), Communities, Profile (tabs + 390px mobile), Settings (all tabs incl. Danger), Directory** + authless **Login/Register** + **390px mobile** nav. Contrast all ≥ 6:1, no horizontal overflow at any width, mobile nav collapses to an aria-labelled "Menu" toggle. **2 findings, both fixed in-slice + verified live:** [U-01](docs/changes/630-ux-review-tracker.md) base-`a` now underlined (WCAG 1.4.1), [U-02](docs/changes/630-ux-review-tracker.md) auth `.field label` now matches `.form-group label` (uppercase/600/letter-spaced). Both `app.css` copies kept in sync. Build 0 warn/0 err; full suite **1228 passed / 0 failed**. **remaining:** object/actor detail pages, Compose, Notifications, Settings sub-tabs (Notifications/Relays/Moderation), Register page, and a final contrast sweep on detail views.
...
| 63 | UI/UX review — usability + presentation (visual pass; fixes 62's UX-class findings) | in progress (63.1) |
...
- 63.1 — UI/UX visual/usability pass, route-by-route (native DOM input; log to 630 tracker) ← active
- 63.2 — fix 63.1's UX nits in-slice (CSS/razor only); defer structural redesigns
- 64 — request spam & network efficiency: 6 topics already drafted in the [620 tracker](docs/changes/620-bug-hunt-tracker.md#phase-64--request-spam--network-efficiency-draft-topics) (actor N+1, proxy N+1, per-item likes/shares fan-out, public-feed double-fetch)
