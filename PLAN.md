...
- **Mastodon interop tests server (in progress)** Docker scaffold committed. Instance API + WebFinger + ActivityPub actor all respond. Test user `mstest` (pw: `Password1`) created via direct SQL. **Verified:** Iris discovers `mstest@mastodon.luit.ink` via fediverse search; actor profile renders in Iris UI. **BUG FOUND:** Iris records the Follow edge locally (Edges table, Kind=0) but does NOT deliver the outbound Follow activity to Mastodon's inbox — `totalItems: 0` on the followers collection after follow. No outbound POST to `mastodon.luit.ink` in Iris logs. **Next:** (1) investigate why outbound Follow delivery is not happening (check if there's a background delivery service, if the inbox URL is being resolved, if signature generation is failing), (2) test posting from Iris, (3) test following from Mastodon → Iris, (4) iterate.
