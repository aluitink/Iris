...
- **138.16 (Iris → Lemmy likes) — verified, no code changes:** the existing outbound Like federation (Phase 24.1 `ResolveObjectOwnerForDeliveryAsync` + server→server delivery) delivers an Iris `Like` of a remote `Page` to the object's home; the like edge is recorded there and the `/likes` collection lists the Iris liker. 2 two-instance integration tests (`LemmyLikeOutboundIntegrationTests`). [Change doc](docs/changes/13816-phase138-iris-like-to-lemmy.md).
