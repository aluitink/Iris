...
- [ ] Phase 2 — Client Library (in progress)
  - Done: `IKeyProvider`/`InMemoryKeyProvider` (session actor→key resolution), `SigningHandler` (`DelegatingHandler`; `ClientToServer` for bodyless GETs, `ServerToServer` for body POSTs; host derived from the request URI), `WebFingerClient` + `IDiscoveryService`/`WebFingerDiscoveryService` (RFC 8410 lookup on the account's own host), and `ActivityPubClient` (signed `GetObjectAsync` → `IObject`, signed `DeliverAsync` → status). 20 new client unit tests.
  - `dotnet build Iris.slnx` 0 warnings; `dotnet test Iris.slnx` 155/155; `dotnet format` clean.
  - Next: extract `IActivityPubClient` + `IActivityPubClientFactory`; `IClientAuthenticator`/`BasicAuthClientAuthenticator` (Basic-auth → PEM private key); client caching (`ActorCache`, `CollectionPageCache`, `WebFingerCache`, `KeyCache` on `ICache<T>`); rich paged collections; `JsonLdHandler`/`RetryHandler`; client ↔ `TestServer` integration tests.
