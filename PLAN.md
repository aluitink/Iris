...
- **83.1 — Global `Iri` fragment-awareness audit + fix (next; follow-up from 82.3).** 82.3 fixed the key stores (fragment-aware addressing) but deliberately did NOT make `Iri` globally fragment-aware, because it breaks 25 existing tests that rely on fragment-blind equality for the `#Public` audience + collection IRIs. This slice does that properly: (1) enumerate the 151 `Iri`-as-dict-key sites + the 25 fragment-blind-dependent tests, and classify each as "should be fragment-aware" vs "should stay fragment-blind"; (2) make `Iri` equality fragment-aware (compare by `Value`), converting it from a `record struct` to a `readonly struct` with explicit equality; (3) update the audience/`#Public`/collection logic + the 25 tests that assumed fragment-blindness to use the correct fragment-aware (or explicitly fragment-stripped) comparison; (4) lock the new semantics with `IriTests` (fragment-sensitive equality, `#key-1` ≠ `#key-2` ≠ bare actor, `#Public` handling). This is the larger, per-usage change 82.3 deferred. Coded tests (core) — in-scope per the WASM manual-test policy.
...
- **83 — Iri identity correctness + production readiness (NEW).** Phase 82 made outbound federation conformant + hardened key management; 82.3 found that `Iri` equality is fragment-blind (correct for `#Public`/audience IRIs, wrong for key IRIs) and fixed the key stores surgically. Phase 83 does the larger, per-usage `Iri` identity-correctness work + moves to production readiness (deployment hardening, observability, config validation). Coded tests for core/server-side behavior.
  - **83.1 — Global `Iri` fragment-awareness audit + fix (Active Slice).** 82.3 deferred this because making `Iri` fragment-aware globally breaks 25 tests that assume fragment-blindness for `#Public`/collection IRIs. This slice: enumerate the 151 `Iri`-as-dict-key sites + the 25 fragment-blind-dependent tests, classify each (fragment-aware vs fragment-blind), make `Iri` equality fragment-aware (`readonly struct` + explicit `Equals`/`GetHashCode` by `Value`), update the audience/`#Public`/collection logic + the 25 tests to the correct comparison, and lock the new semantics with `IriTests`. See Active Slice.
  - **83.2 — Deployment hardening + config validation (seed).** Make a production deployment fail-safe + observable: (1) startup **config validation** — validate `Iris:` options (BaseUri is an absolute http(s) IRI, key store is resolvable, seed actor IRI is well-formed) and fail fast with an actionable error instead of a runtime 500; (2) **observability** — structured logs + a health endpoint reporting the key store's resolvable-actor count + delivery-queue depth (dead-letter count); (3) **graceful degradation** — when the durable store is unavailable, the server should start in a read-only/degraded mode (not crash) and surface it. Coded tests (server) for the validation + health behavior.
...
- **81.3 — Pagination + `Delete`/`Tombstone` conformance — DONE** ([813](docs/changes/813-phase81-pagination-delete-tombstone-conformance.md)): 8 new `Iris.Core.Tests` round-trip tests (Tombstone, Delete, real Mastodon `OrderedCollection`/`OrderedCollectionPage` pagination, empty + single-item edges, `ResolveCollectionItems` `orderedItems` preference) + 4 `Iris.Server.Tests` `last` assertions. **2 conformance fixes:** server collection pages now emit an AS2.0 `last` link; client inbox read path now reads `orderedItems` (the canonical form) via the shared `ResolveCollectionItems`. `Delete`/`Tombstone` verified fully handled (no crash). Suite 1568 passed / 0 failed.
- **81.2 — Misskey + Pleroma wire-format interop round-trip — DONE** ([812](docs/changes/812-phase81-misskey-pleroma-interop-roundtrip.md)): captured real `misskey.io` + `piefed.social` AP fixtures (Misskey Person + 3 Notes + Announce; PieFed `Feed` community actor); 5 new `Iris.Core.Tests` round-trip tests. **No code changes** — Iris deserialized + re-emitted all real Misskey + Pleroma-family documents correctly (Misskey omits `Person.name` — tolerated; custom `Emoji` tags NOT misclassified as hashtags; `Announce` via polymorphic converter; `Feed` extensions survive). **Documented finding:** PieFed `Feed` type not mapped to `Group` (production change, candidate follow-up). Suite 1629 passed / 0 failed.
