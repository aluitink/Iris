...
- **6 open. No blockers (all S2/S3-sev).** (Pass 73: S20 re-confirmed — visual-only toggle (6th pass). S2 re-confirmed — 2 proxy 401 (no change). S14 re-confirmed — 5 errors (no change).)
...
- **Last pass:** 73 (2026-09-20). **Resume checkpoint:** see [docs/qa/passes.md](docs/qa/passes.md) — S13, S15, S7, S8, S3 (UI) fixed; S18 partially fixed; S21 PARTIALLY FIXED (auto-follow works, /c/{handle} redirect not active); S2 partially improved (11→2 errors); S16-UX REGRESSED (vote count inconsistent again). Remaining open: S2, S4 (remote), S6 (fix committed), S14, S16-UX, S17, S18 (partial), S19, S20, S21 (/c/{handle} only).
