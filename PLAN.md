...
- **UI/UX review Pass 10 — three proxy + Lemmy-capability defects fixed — COMPLETE (2026-09-19).** Post-138.25 verification pass (authless + signed-in as pass10review). **Defect 1 (high):** signed-out home feed called the authenticated `POST /ap/v1/proxy/{target}` per remote actor → 15–17 console 401s + CORS error + blank avatars; `UiContext.FetchActorAsync` routed remote actors through the proxy even with no signing client. Fix: skip the proxy when signed out → 0 console errors. **Defect 2 (medium):** proxy outbound transport abort (lemmy.ml Kerberos/GSSAPI TLS "response ended prematurely") escaped as an unhandled 500 (6 console errors). Fix: `ProxyHandler` maps non-cancellation outbound exceptions to a clean 502 (matches the media proxy). **Defect 3 (high):** cached Lemmy community misclassified as Iris → `/feed`+`/members` 404s + "No posts yet". Root cause: `IsLemmy()`'s negative "no `iris:`-namespaced keys" test is defeated by `ActorCountRefreshService` stamping `iris:postsCount`/`followersCount`/`followingCount` onto every stored Group (remote included). Fix: `IsLemmy()` now positive on Lemmy's bare fields (`language`/`featured`/`sensitive`/`postingRestrictedToMods`) + `LemmyFieldNames` + regression test (Lemmy Group with `iris:` counters → `/outbox`+`/followers`). Live-verified: Lemmy community dials `/outbox`+`/followers` (200) and renders its real feed; remaining console errors are only `lemmy.ml` upstream 502s (Defect 2 class). Build 0/0; capability tests 17 passed, proxy-fallback 30 passed, server proxy 23 passed. Commit `132579c`. [change doc](docs/changes/997-ui-ux-review.md)
- **138.25 follow-up — UI surfacing of Lemmy metadata terms — COMPLETE (2026-09-19).** Surfaced the five `iris:` extension terms (rendered server-side by 138.25) in the Blazor WASM UI. `iris:locked` → disabled reply button (EngagementBar + ObjectDetail) with lock icon; `iris:featured` → pinned badge on post cards; `iris:language` → preferred over bare `inLanguage` in ObjectView; `iris:communityNsfw` → NSFW banner on community detail; `iris:postingRestrictedToMods` → mod-only posting banner. Client-only; no server code touched. Build clean; full suite green (1371 Server, 106 Web, 467 Core, 24 LiveInterop, 188 Client, 38 SampleServer, 20 Server.Data). No test >15 s. [change doc](docs/changes/13825-followup-lemmy-metadata-ui.md)
