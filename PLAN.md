...
- **Phase 81 — Federation hardening & real-world interop (ACTIVE; 81.1 + 81.2 DONE).** The app is feature-complete + polished; the next value is making it federate robustly with real-world AP servers. 81.1 (Mastodon) + 81.2 (Misskey + Pleroma-family) done — 9 real-fixture round-trip tests across Mastodon/Misskey/PieFed, **no code changes needed** (Iris handled all three real wire formats correctly). **Documented finding (not fixed):** PieFed's `Feed` community type is not mapped to `Group` (a production change; candidate follow-up). Next: 81.3 (pagination/outbox conformance + `Delete`/`Tombstone`). Coded tests for server-side wire-format behavior (fixture-capture + round-trip pattern, per 79.2/81.1).

## Active Slice

- **81.3 — Wire-format edge cases + outbox/inbox pagination conformance (next).** Verify Iris handles the remaining wire-format edge cases against real/representative documents: **cursor-based outbox/inbox pagination** (`first`/`last` links, `next`/`prev`, `totalItems`, large-collection edge cases — empty pages, single-item pages, `OrderedCollectionPage` vs `OrderedCollection`), and **`Delete`/`Tombstone` handling** (a `Delete` activity, a `Tombstone` object — Iris must deserialize + treat a deleted object as a tombstone, not crash). Same fixture-capture + round-trip pattern (capture real documents from a permissive live instance where reachable; otherwise construct representative documents). Add coded tests in `Iris.Core.Tests`. Fix any conformance gap found. **Candidate follow-up (from 81.2 finding):** map PieFed's `Feed` community type → `Group` (production change; its own slice with detection tests).
...
- **81 — Federation hardening & real-world interop (NEW).** The app is feature-complete + polished; make it federate robustly with real-world AP servers. Test against live servers and fix wire-format/interop gaps. Playwright-driven where UI-facing; coded tests for server-side wire-format behavior (the established interop-verification pattern).
  - ~~**81.1** — Live-server interop sweep: **Mastodon**~~ — DONE (4 real-fixture round-trip tests in `Iris.Core.Tests`; no code changes — Iris deserialized + extracted real `mastodon.online` documents correctly; change doc [811](docs/changes/811-phase81-mastodon-interop-roundtrip.md)).
  - ~~**81.2** — Live-server interop sweep: **Misskey + Pleroma**~~ — DONE (5 real-fixture round-trip tests — 4 Misskey + 1 PieFed/Pleroma-family; no code changes — Iris deserialized + re-emitted real `misskey.io` + `piefed.social` documents correctly; **documented finding:** PieFed `Feed` community type not mapped to `Group`; change doc [812](docs/changes/812-phase81-misskey-pleroma-interop-roundtrip.md)).
  - **81.3** — Wire-format edge cases + outbox/inbox pagination conformance (cursor-based pagination, `first`/`last` links, `totalItems`; large collection edge cases; `Delete`/`Tombstone` handling).
...
- **80.2 — Phase 80 bug-class fixes — DONE** ([801](docs/changes/801-phase80-defect-hunt-fixes.md)): fixed + verified P-001 (`ObjectView` `?? parent.Value`), P-002 (`Compose` `?? "community"`), P-005 (admin-page `[Authorize]` removed → friendly "not an admin"). P-003 wontfix-by-design.
