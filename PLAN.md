...
- **Still open:** **S35 only** (Mastodon cluster — remote-actor discovery 404 is attributed to the Mastodon cluster, not Iris). **S3 + S21 re-verified closed** (Pass 261); **S6 re-verified closed live** (Pass 264 — remote Join/Leave on A's `qa-pass261-test` community drives a server-side Follow/Undo, `POST /ap/v1/u/ii-b1/outbox` 202, no CSP-blocked browser POST, 0 console errors). (S24-D2 outbox-integrity was the last dev-owned open facet — REFUTED as not a defect, see [s24](docs/qa/s24-cross-instance-follow-state-inconsistent.md); S24 fully closed.)
