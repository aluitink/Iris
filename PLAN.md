...
- **83.4 — Graceful degradation / read-only mode on store-unavailable (Active Slice).** When the durable store is unavailable at startup, the server should start in a degraded/read-only mode (not crash) + surface it. The existing `PersistenceHealthCheck` already reports Unhealthy (→ `/ap/v1/health` 503), so an orchestrator can evict a degraded instance. Remaining: a startup probe that, on a failed persistence read, logs a structured "starting in degraded/read-only mode" event + flips a gate the write paths (inbound activity handlers, feed service, moderation, follows) consult to reject/queue writes rather than throw. Multi-file design; see [832 change doc](docs/changes/832-phase83-deployment-hardening-config-validation.md). Coded tests (server).

**Carried-forward remaining (from 83.2 part 3 — graceful degradation):** when the durable store is unavailable at startup, the server should start in a degraded/read-only mode (not crash) and surface it. The existing `PersistenceHealthCheck` already reports Unhealthy (→ `/ap/v1/health` 503) when the store is unreachable, so an orchestrator can evict a degraded instance. Remaining: a startup probe that, on a failed persistence read, logs a structured "starting in degraded/read-only mode" event + flips a gate the write paths (inbound activity handlers, feed service, moderation, follows) consult to reject/queue writes rather than throw. Multi-file design; see [832 change doc](docs/changes/832-phase83-deployment-hardening-config-validation.md).
...
- **83 — Iri identity correctness + production readiness (NEW).** Phase 82 made outbound federation conformant + hardened key management; 82.3 found that `Iri` equality is fragment-blind (correct for `#Public`/audience IRIs, wrong for key IRIs) and fixed the key stores surgically. Phase 83 does the larger, per-usage `Iri` identity-correctness work + moves to production readiness (deployment hardening, observability, config validation). Coded tests for core/server-side behavior.
  ...
  - **83.4 — Graceful degradation / read-only mode on store-unavailable (seed).** When the durable store is unavailable at startup, the server starts in a degraded/read-only mode (not crash) + surfaces it. The `PersistenceHealthCheck` already reports Unhealthy (→ `/ap/v1/health` 503) so an orchestrator can evict a degraded instance; remaining is a startup probe + a gate the write paths (inbound handlers, feed service, moderation, follows) consult to reject/queue writes rather than throw. Multi-file; [832 change doc](docs/changes/832-phase83-deployment-hardening-config-validation.md).
