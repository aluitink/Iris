...
**29.3 (follow-up) — convert the mutating + federation test classes to the shared-host fixture.** The `SharedHostFixture` + `SharedTwoHostFixture` + per-method reset + `ServerRefFor` are in place. **Thirteen RISKY single-instance classes converted** (all single-instance RISKY classes done) + **seven two-instance federation classes converted**: UpdatePropagation (3), LikeAnnounceUndoPropagation (2), MuteUndoPropagation (1), CrossInstanceAcceptPropagation (2), CrossInstanceRejectPropagation (2), ModerationUndoPropagation (2), RelayLifecycle (1). Full fast suite green (899/899, 5m10s). **remaining:** ~15 two-instance federation classes (cross-wired delivery transports, reset covers both instances; some have `DeliveryCounter` or 3-instance topologies that need extra handling). Host construction is only ~11% of the run, so the ceiling is modest — see [docs/changes/258](docs/changes/258-29.3-signature-validation-risky-classes.md).
