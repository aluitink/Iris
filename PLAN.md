...
Phases 0–9 are complete. Phase 8 delivered the server-side Docker foundation; Phase 9 (Real-World Deployment Preparation) is complete — all five deliverables done and grounded in the real config/client/server/test surface. **Phase 10 (Project & Test Review) is in progress** — Slice 1.1 (test audit + dead-code sweep + IRI-resolution dedup) is done: removed the fully-dead `CommunityIris.cs` + `LazyActorDocumentFetcher.cs` (both 0 callers), consolidated the triple-duplicated `IObjectOrLink`→`Iri` resolution into a shared `IriExtensions.ResolveObjectIri`, and added 5 helper tests (444→449, net −69 lines). Per-slice detail (what was built, key types, test counts, resolved decisions) lives in [docs/CHANGELOG.md](docs/CHANGELOG.md); the phased plan and open questions are in [docs/ROADMAP.md](docs/ROADMAP.md).
...
| 10 — Project & Test Review | 🚧 in progress | Slice 1.1 done (test audit + dead-code sweep + IRI-resolution dedup: removed dead `CommunityIris.cs` + `LazyActorDocumentFetcher.cs`, consolidated triple-duplicated IRI resolution into `IriExtensions.ResolveObjectIri`, 5 new tests, 444→449). Remaining: harness consolidation (hoist per-test federation helpers into `Iris.Testing`), parallel-cache-engine / IRI-JSON-helper consolidation, dead-code sweep on remaining source, API surface pass, doc sync. |
