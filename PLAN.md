...
- **138.10 (post-to-Lemmy-community delivery path) — the cross-post leg:** a new `GetCrossPostTargetsAsync` in `OutboxPublishHandler`'s Create branch delivers a `Create` whose `to` explicitly addresses a REMOTE community to that community's inbox (sharedInbox/inbox, signed as the authoring local actor) — the explicit cross-post per [decision 058](docs/decisions/058-outbound-post-to-peered-community-shape.md); best-effort, reuses the once-minted activity id (055). 2 two-instance integration tests (`CrossPostToRemoteCommunityIntegrationTests`): positive cross-post lands in the target community's local member's outbox + a negative control (a plain post not addressed to the remote community does not reach it). Live Lemmy acceptance (post in Lemmy's own listing) exercised live with 138.11. [Change doc](docs/changes/13810-phase138-post-to-lemmy-community-delivery.md).
- **138.9 (push-content-to-Lemmy shape) — decision, no code:** [Decision 058](docs/decisions/058-outbound-post-to-peered-community-shape.md) records that an Iris post landing in a remote community's post list is an explicit cross-post — target community `Group` in `to`, `Create` delivered to its `sharedInbox`/`inbox` signed as the authoring local actor, `Note` object (Note-vs-`Page` deferred to 138.11), client-supplied target IRI (no implicit push into followed communities), same activity id reused. Unambiguous for 138.10. [Change doc](docs/changes/13809-phase138-push-content-to-lemmy-shape.md).
...
- **138.6 (mutual peering handshake) — both /following and /followers populate on both sides:** `AcceptActivityHandler` community branch records both `AddFollowAsync` and `AddFollowerAsync`; 3 integration tests on a cross-wired two-host fixture. [Plan](docs/plans/phase-138-lemmy-community-integration.md).
