...
- **Phase 82 — Real-world signed federation + production readiness (ACTIVE; seeded).** Phase 81 verified Iris can *receive* + *render* real AP servers (inbound). Iris already has outbound HTTP Signatures (`SigningHandler` + `ISignatureSigner` + `IKeyProvider`); this phase verifies + hardens the *outbound* side so Iris federates with non-permissive instances (e.g. `mastodon.social`, which rejects unsigned fetches), then moves to production readiness. First slices seeded below (82.1 offline signature conformance — the most concrete/testable foundation).

## Active Slice

- **82.3 — Key management hardening (next).** Harden the per-actor key management the outbound/inbound federation depends on: (1) **per-actor key pairs on the actor doc `publicKey`** — verify every resolvable Iris actor serves a `publicKeyPem` that matches the key the instance actually signs with (the 82.2 live test proved this for `andrew`; make it systematic + tested for all actors/communities); (2) the **`IKeyProvider` resolution path** — `DelegatingKeyProvider` primary + `#key-1` durable-store fallback: lock the resolution order + the failure mode when an actor has no key (must throw `KeyNotFoundException`, not sign with a null/wrong key); (3) **`publicKey`↔signing-key consistency** — an invariant test that the public key served in the actor doc verifies a signature the instance produces (catches a key-rotation or misconfiguration drift where the served key and the signing key diverge); (4) **key rotation** readiness — document/verify what happens to in-flight + stored signatures when an actor's key is rotated (a candidate follow-up if a rotation mechanism doesn't yet exist). Coded tests for core/server-side behavior (in-scope per the WASM manual-test policy). If a rotation mechanism is found missing, record it as a follow-up rather than building it (keep the slice to hardening + invariants + tests).
...
- **82 — Real-world signed federation + production readiness (NEW).** Phase 81 verified inbound from real AP servers; Iris already has outbound HTTP Signatures (`SigningHandler`). Verify + harden the *outbound* side so Iris federates with non-permissive instances (e.g. `mastodon.social`, which rejects unsigned fetches), then move to production readiness. Coded tests for core/server-side behavior; Playwright-driven where UI-facing.
  ...
  - **82.3** — Key management hardening (Active Slice): per-actor key pairs on the actor doc `publicKey`, the `IKeyProvider` resolution path, `publicKey`↔signing-key consistency, and key-rotation readiness.
...
- **81.1 — Mastodon wire-format interop round-trip — DONE** ([811](docs/changes/811-phase81-mastodon-interop-roundtrip.md)): captured real `mastodon.online` AP fixtures (Person actor + 3 Notes: public+media+hashtag, followers-only cross-instance reply, public+2-mentions); 4 new `Iris.Core.Tests` round-trip tests. **No code changes** — Iris deserialized + extracted all real Mastodon wire-format features correctly (Public sentinel kept in `to`/filtered by `GetAudienceIris`, explicit nulls clean, scalar `attributedTo` accepted, `inReplyTo` list, hashtags verbatim, media `Document`). Suite 1624 passed / 0 failed.
