...
1. **Signature Validation Review** - We are generally failing to handle Delete activity verification, I don't think we are properly validating the signatures. Investigate how mastodon source code generates and signs the deletes, I beleive it is some signature of cannonicalized json. Peridoically monitor for failed validations on Creates (standard http digest signature) and fix these issues. Add debug logging if needed, we should capture everything we need to troubleshoot these issues.
   - **Create 401 diagnosis (content-digest):** The `content-digest` header was missing from the RFC 9421 signature base construction. Fixed by adding `SignatureBase9421.ContentDigestHeaderName` to the header collection in `ToMetadata`. Verified: other hachyderm actors (dalias, etc.) now validate. **skinnylatte-specific rejections persist** and were diagnosed as a **hachyderm-side key mismatch**: the `publicKey` in skinnylatte's actor document does not match the key used to sign. Confirmed via independent cryptographic verification (captured signature + base + key JWK all fail to verify, even after 3 fresh refetches of the actor document). This is not an Iris bug; the correct behavior is to reject.
   - **Delete 401 diagnosis (embedded proof + key cache):** Delete activities from deleted actors (410 Gone) carry a W3C `RsaSignature2017` embedded proof in the body. The proof's `creator` field is the key IRI, which is resolved via the `RemoteKeyCache` (1h fresh + 1h stale) or a fresh fetch of the actor document. When the actor is deleted, the actor document returns 410, so the key can only be resolved if it's still in the cache. **Mastodon's implementation** (`ActivityPub::LinkedDataSignature#verify_actor!`) resolves the key via `Keypair.from_keyid` (local DB lookup) or `FetchRemoteKeyService` (remote fetch) — same limitation: the key must be cached or resolvable. **Iris's current behavior (rejecting Delete activities when the key can't be resolved) is correct** and matches the ActivityPub spec. The W3C `RsaSignature2017` format does not embed the public key, so verification is impossible without a cached/resolvable key. **Potential future improvement:** persist remote actor keypairs to the database (like Mastodon's `Keypair` table) so that keys survive cache eviction and restarts, enabling verification of Delete activities from previously-interacted actors even after deletion. This is a larger architectural change, not a simple fix. [change doc](docs/changes/sigval-delete-embedded-proof-diagnosis.md)
2. **General UI/UX review** (recurring) — Next pass after improvements land. Last pass: Home, Notifications, Profile (own + remote), Compose (end-to-end post, HTTP 202), Communities (list + detail) all consistent and functional; 0 console errors. Two minor observations logged (Following-list names not clickable; cosmetic proxy console noise). No defects. [change doc](docs/changes/997-ui-ux-review.md)
3. **139.2 Scenario 5 (audience/visibility) — remaining deferred surfaces** — the S1 read-path gap is now **closed for the public feed, global search, and the follow feed** (a `VisibilityFilter` predicate — public = `as:Public` sentinel or no audience; non-public = visible to a named recipient **or the author** — is wired into all three; anonymous sees public only, a signed requester also sees items addressed to them, and the follow-feed owner sees their own non-public posts). Remaining lower-priority surfaces: (a) **object-document endpoint** — add a recipient check (404/403 for a non-recipient on non-public content) — a larger design decision that interacts with federation (remote instances must fetch non-public content to store/deliver it); (b) **federation visibility policy** — suppress non-public content on receipt vs. store-with-visibility-marker; (c) **gate the follow-feed request itself** to the actor's owner (authz, not a visibility leak — the content is already filtered). See [change doc](docs/changes/1392-5-audience-visibility-filter.md).
4. **139.3 F2 follow-up (optional product decision) — RESIGNED.** Review (139.3-F2) corrected the F2 premise: the feed paths **do** consult the actor store (`ILocalActorResolver.IsLocalActorAsync`), so a deleted **local** follow/member is already excluded from the home/community feed in any resolver-configured host (production) — the feed gap is already mitigated, no feed-side code change warranted. The genuine residual is the **edge-list / like-counter** surfaces only: a deleted actor can still appear in followers/following + community member lists and still count toward like counters (these read `Edges` with no actor join). A future slice would filter edges referencing a removed actor at read time (a read-path filter, not a DB sweep). Logged as a product decision, not a defect. [change doc](docs/changes/1393-11-retention-right-to-deletion.md)
...
- **139.2 Scenario 5 (audience/visibility) — S1 read-path gap CLOSED (public feed + global search + follow feed).** Iris used pure AS2 `to`/`cc` with no visibility filter, so a DM (`to=[bob]`, no `as:Public`) was as visible as a public post — it appeared in the public timeline, global search, and (with `?depth>0`) the follow feed to everyone. Added `VisibilityFilter` (public = `as:Public` sentinel or no audience; non-public = visible to a named recipient **or the author** — the author clause lets the owner see their own DMs) and wired it into `GET /ap/v1/public/feed` (filtered before truncate), `GET /ap/v1/search` (handler gains `ISignatureValidator`; search loads + filters content, total from the visible set), and `GET /ap/v1/u/{handle}/feed` (merged feed filtered after the query/type filters; meaningful at `?depth>0` where the reply heuristic no longer hides non-public items). Anonymous/unsigned = public only; a signed request also sees items addressed to it; the follow-feed owner additionally sees their own non-public posts. Actors unaffected. +22 tests (`VisibilityFilterTests` + `FeedServiceTests.Feed_Audience_*`); updated `DirectPost_..._CurrentGap` to assert the fixed behavior. Object-document, federation policy, and gating the follow-feed *request* to the owner remain deferred. [change doc](docs/changes/1392-5-audience-visibility-filter.md)
- **139.3 Scenario 11 (retention / right-to-deletion) — PASS (139.3 now complete; all 11 scenarios done).** Deleting a local account produces a coherent post-deletion state matching the Phase 136.19 retention model on every path that must hold: actor 404s, content IRIs serve `Tombstone` (not 404s / not live content), object + actor search exclude the deleted account, the account row is gone (by id + username), other actors untouched. The model's tolerated "bounded stale artifacts" survive by design (the actor's Create activities + outbox box items still embed the original notes; the Follow/CommunityMember/Like edges still resolve). +1 test (`AccountDeletionRetentionTests`, mirrors `AccountDeletionService.DeleteAsync` exactly). **Logged nuance (F2, no code change):** the "not user-visible" wording is path-specific — true for the object-document endpoint (serves the Tombstone) but not strictly for the feed/edge paths (read the surviving rows, no tombstone filter / actor join), so a deleted-but-followed actor's original content can still render in a follower's feed and the actor can still appear in follower/member lists. Logged as a future feed-rendering product decision. [change doc](docs/changes/1393-11-retention-right-to-deletion.md)
- **139.3 Scenario 10 (data volume growth sanity) — PASS.** Seeded a larger-than-typical dataset (a 500-post outbox + a 1,500-object search corpus) via bulk `UNNEST` inserts + the migration's `SearchVector` backfill, then exercised the real EF read paths: the outbox/feed read returned all 500 items in correct newest-first order, and the GIN-indexed FTS search returned the exact known hit count (100) with correct, non-overlapping pagination. Timings: outbox read of 500 = 137.7 ms; object search (limit 20) over 1,500 = 37.4 ms — well within the generous 30 s ceiling (the bar is correctness, not raw speed). No defects. +1 test (`DataVolumeGrowthTests`). Note for 139.5: the outbox/feed read is a full load (no SQL `LIMIT`) — the path to revisit at far larger volumes. [change doc](docs/changes/1393-10-data-volume-growth-sanity.md)
- **139.3 Scenario 9 (migration safety) — PASS.** Upgrading a populated, older-schema (InitialCreate-only) Postgres to the current schema applies the full `Migrate()` chain cleanly: no data loss, schema complete, both backfills ran (`ObjectIri` extracted from the `Create`'s jsonb `object.id`; actor `SearchVector` populated). Re-running `Migrate()` on an already-current DB is an idempotent no-op. +2 tests (`MigrationSafetyTests`, each on an isolated `postgres:16-alpine` Testcontainer; old-schema data seeded via raw SQL since the current EF model can't INSERT into the older column set). No defects found. [change doc](docs/changes/1393-9-migration-safety.md)
- **General UI/UX review — PASS (this pass).** Reviewed Home, Notifications, Profile (own + remote), Compose, Communities (list + detail) via MCP Playwright as andrew:Password1. All pages consistent and functional. Compose end-to-end post verified (HTTP 202, minted IRI, "View your posts →" link; counter resets to 0/500 after success — correct, `Content` cleared on success). Community detail feed: 0 console errors/warnings, mixed Mastodon/Lemmy engagement render correctly, graceful "Content unavailable" link-out for unfetchable remote posts. Two minor observations (no fix): Following-list actor names are plain text (not clickable links); cosmetic 404/418 proxy console noise on unavailable boosted objects (degrades gracefully). No defects. Recurring item stays at top of Up Next. [change doc](docs/changes/997-ui-ux-review.md)
