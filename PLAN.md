...
3. **139.3 F2 follow-up (optional product decision)** — make the feed/edge read paths render a "deleted user" placeholder for a deleted account's content (Mastodon-style) instead of surfacing the original post content + keeping the actor in follower/member lists. Currently the model's "not user-visible" wording holds for the object-document path but not the feed/edge paths (see the 139.3-s11 change doc). Logged as a product decision, not a defect.
