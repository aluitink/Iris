...
- **Phase 80 — UI/UX polish + defect hunt (COMPLETE).** All spec gaps (F-01–F-31) and interop (Phases 76–79) were closed; this phase was a fresh full-app defect hunt + polish pass over the 18 routes (the established 62/63/64 pattern). Playwright-driven (WASM manual-test policy): no new coded web tests. **All 5 findings (P-001…P-005) resolved:** P-001 `ObjectView` `?? parent.Value`, P-002 `Compose` `?? "community"`, P-005 admin-page login redirect (80.2, `247b8bb`); P-004 object-detail parent preview (80.3, `66b873b`); P-003 wontfix-by-design (global sign-in-required app). 80.3's broader polish (empty/loading/error states + a11y) verified in excellent shape — no code changes needed. Change docs: [800 tracker](docs/changes/800-phase80-bug-hunt-tracker.md) · [801](docs/changes/801-phase80-defect-hunt-fixes.md) · [803](docs/changes/803-phase80-p004-object-detail-parent-preview.md) · [804](docs/changes/804-phase80-broader-ux-polish-verification.md).
- **Phase 81 — Federation hardening & real-world interop (COMPLETE).** The app is feature-complete + polished; this phase made it federate robustly with real-world AP servers. **81.1 (Mastodon) + 81.2 (Misskey + Pleroma-family):** 9 real-fixture round-trip tests, **no code changes needed** (Iris handled all three real wire formats correctly). **81.3 (pagination + `Delete`/`Tombstone` conformance):** 8 `Iris.Core.Tests` round-trip tests + 4 `Iris.Server.Tests` `last` assertions; **2 conformance fixes** — server collection pages now emit an AS2.0 `last` link, and the client inbox read path now reads `orderedItems` (the canonical form) via the shared `ResolveCollectionItems`. `Delete`/`Tombstone` verified fully handled (no crash). **Documented finding (not fixed, candidate follow-up):** PieFed's `Feed` community type is not mapped to `Group`. Change docs: [811](docs/changes/811-phase81-mastodon-interop-roundtrip.md) · [812](docs/changes/812-phase81-misskey-pleroma-interop-roundtrip.md) · [813](docs/changes/813-phase81-pagination-delete-tombstone-conformance.md).
- **Phase 82 — Real-world signed federation + production readiness (COMPLETE).** Phase 81 verified Iris can *receive* + *render* real AP servers (inbound); Phase 82 verified + hardened the *outbound* side so Iris federates with non-permissive instances. **82.1 (offline signature conformance):** added the missing `created` param to the outbound `Signature` header + 13 deterministic tests. **82.2 (real-world signed delivery):** live proof — signed `Follow` from `iris.luit.ink` actor `andrew` → `mastodon.social` → **202 Accepted** (×2 targets); unsigned → 401; wrong-key → 401 (acceptance genuinely signature-gated); 3 offline end-to-end conformance tests. **82.3 (key-management hardening):** fixed a real bug — fragment-aware key addressing in the in-memory + file-backed key stores (the default `Iri` equality is fragment-blind and conflated `#key-1`/`#key-2`/bare-actor); `DelegatingKeyProvider` (previously untested) now has 7 resolution/failure-mode tests; the `publicKey`↔signing-key consistency invariant is locked for users + community; local key-rotation is documented as a follow-up. Change docs: [821](docs/changes/821-phase82-outbound-signature-conformance.md) · [822](docs/changes/822-phase82-real-world-signed-delivery.md) · [823](docs/changes/823-phase82-key-management-hardening.md).

## Active Slice

- **84.2 — Local key-rotation lifecycle (Active Slice).** 82.3 documented local key-rotation as a follow-up. Add the lifecycle for the local instance's signing key: **rotate** (mint a new key pair, advertise it via the actor document's `publicKey` + a `Move`/key-rotation activity so peers learn the new key, keep the old key resolvable during an overlap window, then retire it once the rotation is confirmed). The fragment-aware `Iri` (83.1) + fragment-aware key stores (82.3) now make `#key-1`/`#key-2` addressable as distinct keys. Coded tests (server): rotation produces a resolvable new key + the actor document advertises it; an old-key signature still verifies during the overlap window; after retirement the old key is gone but signatures made before retirement still verify (the public key is still fetchable / cached by peers).
...
- ~~**80 — UI/UX polish + defect hunt**~~ — **COMPLETE** (all 5 findings P-001…P-005 resolved; broader UX polish verified clean; change docs [800](docs/changes/800-phase80-bug-hunt-tracker.md) · [801](docs/changes/801-phase80-defect-hunt-fixes.md) · [803](docs/changes/803-phase80-p004-object-detail-parent-preview.md) · [804](docs/changes/804-phase80-broader-ux-polish-verification.md)).
- ~~**81 — Federation hardening & real-world interop**~~ — **COMPLETE** (81.1 Mastodon + 81.2 Misskey/Pleroma: 9 real-fixture round-trip tests, no code changes; 81.3 pagination + `Delete`/`Tombstone`: 8 core + 4 server tests, 2 conformance fixes — server `last` link + client inbox `orderedItems`; change docs [811](docs/changes/811-phase81-mastodon-interop-roundtrip.md) · [812](docs/changes/812-phase81-misskey-pleroma-interop-roundtrip.md) · [813](docs/changes/813-phase81-pagination-delete-tombstone-conformance.md)).
- **83 — Iri identity correctness + production readiness (NEW).** Phase 82 made outbound federation conformant + hardened key management; 82.3 found that `Iri` equality is fragment-blind (correct for `#Public`/audience IRIs, wrong for key IRIs) and fixed the key stores surgically. Phase 83 does the larger, per-usage `Iri` identity-correctness work + moves to production readiness (deployment hardening, observability, config validation). Coded tests for core/server-side behavior.
  - ~~**83.1** — Global `Iri` fragment-awareness audit + fix~~ — **DONE** (made `Iri` equality fragment-aware — a `readonly struct` with explicit `Equals`/`GetHashCode`/`==`/`!=` comparing `Iri.Value` (which includes the fragment), null-safe for `default`; every `Iri`-keyed `Dictionary`/`HashSet` is now fragment-aware by default. **2 real latent bugs surfaced + fixed** (both masked by fragment-blind conflation): `MoveActivityHandler` never invalidated a non-standard key fragment; `FeedServiceTests` seeded an inconsistent `#note` note IRI. 7 new `IriTests` lock the semantics. **7 new tests, no regression.** Suite 1595 passed / 0 failed; [change doc 831](docs/changes/831-phase83-iri-fragment-awareness.md)).
   - ~~**83.2** — Deployment hardening + config validation~~ — **DONE (parts 1 + 2)** (startup config validation via `ActivityPubServerOptionsValidator` + `ValidateOnStart` — fail fast on a malformed `BaseUri`/`InstanceActorId`/`SharedInboxIri`; + `InstanceObservabilityHealthCheck` reporting the resolvable-actor count + dead-letter count on `GET /ap/v1/health`, Degraded when dead letters present. **13 new tests.** Part 3 — graceful degradation (read-only mode on store-unavailable) — carried forward as remaining on the Active Slice; [change doc 832](docs/changes/832-phase83-deployment-hardening-config-validation.md)).
  - ~~**83.3** — Outbound delivery retry + dead-letter observability~~ — **DONE** (verified the non-permissive-peer dead-letter contract — a permanent 401/403 from a strict peer is dead-lettered on the **first** attempt, not retried forever, while a 500 consumes the full budget; + exposed the dead-letter queue via **`GET /ap/v1/dead-letters`** (count + bounded peek, newest-first, `limit` query param default 25/cap 500, read-only, no auth). **6 new tests.** [change doc 833](docs/changes/833-phase83-outbound-delivery-dead-letter-observability.md)).
  - ~~**83.4** — Graceful degradation / read-only mode on store-unavailable~~ — **DONE** (carried-forward 83.2 part 3: `IDegradedModeGate` (stateful, thread-safe flag) + `PersistenceDegradedModeProbe` (a hosted service that, on a failed persistence read at startup or a later re-probe, logs a structured `degraded_mode_entered` event + flips the gate; a successful re-probe clears it — recovery). The four federation write surfaces (actor + community inbox/outbox) now refuse writes with `503 application/problem+json` when degraded (reads still served); inbox checks before signature validation, outbox after. **6 new tests.** [change doc 834](docs/changes/834-phase83-graceful-degradation-readonly-mode.md)). **Phase 83 is COMPLETE** (83.1 Iri fragment-awareness, 83.2 config validation + observability, 83.3 dead-letter retry + observability, 83.4 graceful degradation).
- **84 — Production hardening & scale-out readiness (NEW).** Phase 83 made the instance identity-correct + observable + degrade gracefully; Phase 84 makes it **deploy at scale** — the remaining operational gaps: durability across restarts, distributed state for multi-instance, and the key-rotation lifecycle. Coded tests for server/data-side behavior.
  - ~~**84.1** — Config-driven durable dead-letter store~~ — **DONE** (added a **`Iris:Delivery:DeadLetterJournalPath`** config key: when set, `AddActivityPubServer` rebinds `IDeliveryDeadLetterStore` to the existing `FileBackedDeliveryDeadLetterStore` (16.2) over that path — **independently of the delivery queue** (the queue stays its in-memory default unless `UseFileBackedDelivery` is called); no config = in-memory default (existing tests unaffected). **5 new tests** (persists + survives a restart, capacity eviction + newest-first survive a restart, config selects file-backed, unset keeps in-memory, key doesn't change the `IDeliveryQueue` binding). [change doc 841](docs/changes/841-phase84-config-driven-durable-dead-letter-store.md).)
  - **84.2 — Local key-rotation lifecycle (Active Slice).** 82.3 documented local key-rotation as a follow-up. Add the lifecycle: rotate the local actor's signing key (mint a new key, advertise it via the actor document's `publicKey` + `Move`/key-rotation so peers learn the new key, retire the old key once the rotation is confirmed) + the `#key-N` addressing the fragment-aware `Iri` (83.1) + key stores (82.3) now support. Tests: rotation produces a resolvable new key; the actor document advertises it; old-key signatures still verify during the overlap window.
- **82 — Real-world signed federation + production readiness (COMPLETE).** Phase 81 verified inbound from real AP servers; Phase 82 verified + hardened the *outbound* side so Iris federates with non-permissive instances. Coded tests for core/server-side behavior.
  - ~~**82.1** — Outbound signature conformance (offline, deterministic)~~ — **DONE** (found + fixed the one missing spec param — `created` — added it to the signer + `SignatureHeader` (derived from the signed `Date`, so deterministic); 13 deterministic offline tests pin the exact `Signature` header for both profiles with a fixed RSA key + verify against the public key; [change doc 821](docs/changes/821-phase82-outbound-signature-conformance.md)).
  - ~~**82.2** — Real-world signed delivery~~ — **DONE** (live: signed `Follow` from `iris.luit.ink` actor `andrew` → `mastodon.social` → **202 Accepted** (×2 targets); unsigned → 401 "Request not signed"; wrong-key → 401 "Could not refresh public key" — the acceptance is genuinely signature-gated; 3 new offline end-to-end `Iris.Client.Tests` lock the pipeline conformance; no production code changes; [change doc 822](docs/changes/822-phase82-real-world-signed-delivery.md)).
  - ~~**82.3** — Key management hardening~~ — **DONE** (1 real bug fixed — fragment-aware key addressing in the in-memory + file-backed key stores; `DelegatingKeyProvider` (previously untested) + 7 resolution/failure-mode tests; `publicKey`↔signing-key consistency invariant locked for users + community; 3 key-store fragment-awareness tests; local key-rotation documented as a follow-up; 16 new tests, no regression; [change doc 823](docs/changes/823-phase82-key-management-hardening.md)).
...
- **84.1 — Config-driven durable dead-letter store — DONE** ([841](docs/changes/841-phase84-config-driven-durable-dead-letter-store.md)): the dead-letter queue (83.3) was in-memory only by default (a restart lost every dead-lettered delivery). `FileBackedDeliveryDeadLetterStore` (16.2) already journals to disk, but could only be selected via `UseFileBackedDelivery` (which also forces the file-backed delivery queue). Added a **`Iris:Delivery:DeadLetterJournalPath`** config key: when set, `AddActivityPubServer` rebinds `IDeliveryDeadLetterStore` to the file-backed store (the delivery queue is left untouched — decoupled). Default stays in-memory (no config = current behavior). **5 new tests** (persists + survives a restart, capacity eviction + newest-first survive a restart, config selects file-backed, unset keeps in-memory, key doesn't change the `IDeliveryQueue` binding). Suite 1625 passed / 0 failed (Iris.Server.Tests 1000 → 1005).
- **83.4 — Graceful degradation / read-only mode on store-unavailable — DONE** ([834](docs/changes/834-phase83-graceful-degradation-readonly-mode.md)): carried-forward 83.2 part 3. `IDegradedModeGate` (stateful, thread-safe flag) + `PersistenceDegradedModeProbe` (a hosted service that, on a failed persistence read at startup or a later 30s re-probe, logs a structured `degraded_mode_entered` event + flips the gate; a successful re-probe clears it — recovery without restart). The four federation write surfaces (actor + community inbox/outbox) now refuse writes with `503 application/problem+json` when degraded (reads still served); inbox checks **before** signature validation, outbox **after** (a valid signature is still required — a degraded instance never opens the write surface to unauthenticated clients). **6 new tests** (gate statefulness, probe failing→degraded + healthy→recovery, degraded inbox write 503, degraded read still served, outbox auth-precedence 401). Suite 1620 passed / 0 failed (Iris.Server.Tests 994 → 1000). **Phase 83 COMPLETE** (83.1 Iri fragment-awareness, 83.2 config validation + observability, 83.3 dead-letter retry + observability, 83.4 graceful degradation).
- **83.3 — Outbound delivery retry + dead-letter observability — DONE** ([833](docs/changes/833-phase83-outbound-delivery-dead-letter-observability.md)): verified the **non-permissive-peer** dead-letter contract — a permanent **401/403** from a strict peer is dead-lettered on the **first** attempt (not retried forever), while a **500** consumes the full retry budget; + exposed the dead-letter queue via **`GET /ap/v1/dead-letters`** (count + bounded peek, newest-first; `limit` query param, default 25 / cap 500; read-only, no auth — operator monitoring scrape). `DeadLetterRouteSegment` + `DeadLetterHandler`. **6 new tests** (endpoint count/peek/limit/empty ×3, worker 401/403/500 ×3). Suite 1614 passed / 0 failed (Iris.Server.Tests 988 → 994).
- **83.2 — Deployment hardening + config validation (parts 1 + 2) — DONE** ([832](docs/changes/832-phase83-deployment-hardening-config-validation.md)): startup **config validation** — `ActivityPubServerOptionsValidator` (`IValidateOptions<ActivityPubServerOptions>`) + `ValidateOnStart` in `AddActivityPubServer`: a present-but-malformed `BaseUri`/`InstanceActorId`/`SharedInboxIri` (not an absolute http(s) IRI) fails host start with an actionable `OptionsValidationException` (null options allowed — a host may configure a subset). + **federation observability** — `InstanceObservabilityHealthCheck` (`IHealthCheck`) reports `stored_actors` / `resolvable_actors` (stored actors with a resolvable signing identity, via `IKeyProvider`) / `dead_letters` on `GET /ap/v1/health`, Degraded when dead letters present. **13 new tests** (host-start fail-fast ×3 + valid + all-null, observability counts ×3, validator theory ×5). **Part 3 (graceful degradation / read-only mode) carried forward as remaining.** Suite 1608 passed / 0 failed (Iris.Server.Tests 975 → 988).
- **83.1 — Global `Iri` fragment-awareness — DONE** ([831](docs/changes/831-phase83-iri-fragment-awareness.md)): made `Iri` equality fragment-aware (a `readonly struct` with explicit `Equals`/`GetHashCode`/`==`/`!=` comparing `Iri.Value`, which includes the fragment — null-safe for `default`), so every `Iri`-keyed `Dictionary`/`HashSet` in the codebase is fragment-aware by default (matching the Postgres store + `IriEqualityComparer`). **2 real latent bugs surfaced + fixed** (both masked by fragment-blind conflation): `MoveActivityHandler` never invalidated a non-standard key fragment (`bypassCache: true` + read-after-invalidate → always fell back to `#key-1`); `FeedServiceTests` seeded an inconsistent `#note`-suffixed note IRI. 7 new `IriTests` lock the semantics (`#key-1` ≠ `#key-2` ≠ bare actor, `#Public` distinctness, `default` null-safety). **7 new tests, no regression.** Suite 1595 passed / 0 failed.
- **82.3 — Key management hardening — DONE** ([823](docs/changes/823-phase82-key-management-hardening.md)): **1 real bug fixed** — fragment-aware key addressing in the in-memory + file-backed key stores (default `Iri` equality is fragment-blind, conflating `#key-1`/`#key-2`/bare-actor); `DelegatingKeyProvider` (previously untested) + 7 resolution/failure-mode tests; `publicKey`↔signing-key consistency invariant locked for users + community; 3 key-store fragment-awareness tests; local key-rotation documented as a follow-up (global `Iri` fragment-awareness deferred to 83.1). **16 new tests, no regression.** Suite 1588 passed / 0 failed.
