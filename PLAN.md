...
**Phase 31 — explorer UI & server namespace polish.** 31.1–31.7 complete (WebFinger RFC 7033 + retry, `PagedCollection` initial-load re-render, carla fake-remote seed gated off, directory actors-only + rename, actor-detail profile header + tabs, community profile header + tabs, and the shared `CollectionBrowser` component). Next: 31.8 (server — derive the `iris:` extension namespace from the public base URI + host the namespace document). Phase 30 (30.2 health check + readiness probe) resumes after the Phase 31 Inbox is drained.
