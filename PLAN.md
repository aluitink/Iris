...
- [ ] Phase 2 — Client Library (in progress)
  - Done: `IKeyProvider`/`InMemoryKeyProvider` (session actor→key resolution), `SigningHandler` (`DelegatingHandler`; `ClientToServer` for bodyless GETs, `ServerToServer` for body POSTs; host derived from the request URI), `WebFingerClient` + `IDiscoveryService`/`WebFingerDiscoveryService` (RFC 8410 lookup on the account's own host), `ActivityPubClient`, and the `IActivityPubClient` interface + `IActivityPubClientFactory`/`ActivityPubClientFactory` (wires signing pipeline into an owned `HttpClient`; `GetActorAsync` added). 28 client unit tests.
  - `dotnet build Iris.slnx` 0 warnings; `dotnet test Iris.slnx` 163/163; `dotnet format` clean.
  - Next: `IClientAuthenticator`/`BasicAuthClientAuthenticator` (Basic-auth → PEM private key); client caching (`ActorCache`, `CollectionPageCache`, `WebFingerCache`, `KeyCache` on `ICache<T>`); rich paged collections; `JsonLdHandler`/`RetryHandler`; client ↔ `TestServer` integration tests.
