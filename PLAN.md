...
- **Verified 2026-09-21:** all 13 FQDNs serve HTTP 200 over TLS; WebFinger confirms each Iris instance advertises its own FQDN (e.g. `alice@dev1-iris-a.luit.ink` → `acct:alice@dev1-iris-a.luit.ink`). The FQDNs are mapped to the per-env published ports by an **external (operator) reverse proxy** — there is **no proxy stack in this repo** and none to configure. The per-env compose only publishes the host ports (e.g. `dev1-iris-a` → 10081); the external proxy terminates TLS and routes each FQDN to that port. **All communication with the project goes over the deployed containers via the public FQDNs** — never a local port, `localhost`, or `docker exec`.
 - **Dev1 stack:** `dev1-iris-a` + `dev1-iris-b` healthy (fresh build from `55827c2c`, 2026-09-22 — S24-D2/S36 home-feed own-outbox content filter `61c328fa` + S24-D2 outbox `activity.Actor` filter `98399b73` + S24-D4 remote-actor collection proxy + directory paging + S17 fix + all merged main work deployed).
...
- **S24-D2 (foreign activities in local outbox):** **FIXED (dev2 `98399b73` + feed-surface facet `61c328fa`), live-verified on deployed `55827c2c`.** `GetOutboxAsync` in `EfActivityStore`, `InMemoryActivityStore`, and `FileBackedActivityStore` now filters by `activity.Actor` — only activities authored by the actor are returned; foreign (inbound) activities are excluded. 4 follow-request-queue tests updated to use the dedicated `ILocalModerationClient.GetFollowRequestsAsync` API (the production surface) instead of reading the outbox directly. **Feed-surface facet (`61c328fa`):** `FeedService` own-outbox branch keeps only owner-authored content (`Create` of Note/Article/Question, or own `Announce`), excluding any residual foreign boost fan-out / actor-doc noise from the home timeline (S36). **Live-verified:** on the deployed `55827c2c` build, A's raw outbox = 1 item (own `Create` only — the spurious self-`Follow` is now filtered by the outbox `activity.Actor` filter), and the home feed shows A's post (owner-authored). 113 + 1438 tests pass.
