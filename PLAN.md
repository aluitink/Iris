...
Phases 0–7 are complete; Phase 8 (Sample Docker Composition) is in progress — the server-side Docker foundation (SampleServer Dockerfile + a two-instance compose stack + smoke test) is done and verified. Phase 9 (Real-World Deployment Preparation) is in progress — the FQDN & TLS plan + bootstrap runbook (`docs/DEPLOYMENT_PREP.md`) and the real-user enumeration design (`docs/ENUMERATION_DESIGN.md`) are done, both grounded in the real config/client surface. Per-slice detail (what was built, key types, test counts, resolved decisions) lives in [docs/CHANGELOG.md](docs/CHANGELOG.md); the phased plan and open questions are in [docs/ROADMAP.md](docs/ROADMAP.md).
...
| 9 — Real-World Deployment Preparation | 🚧 in progress | FQDN & TLS plan + bootstrap runbook (`docs/DEPLOYMENT_PREP.md`) + real-user enumeration design (`docs/ENUMERATION_DESIGN.md`), both grounded in the real config/client surface; resolves OQ #2 + #3. Remaining: compatibility matrix, test-harness extension, risk & gap register. Prep only — no live tests yet. |
