...
- **139 — Whole-platform end-to-end review:** now that Phase 138 (Lemmy integration) is complete, a full cross-cutting review of every subsystem (federation/interop, security/trust boundary, data lifecycle, UI/UX + accessibility, performance/scalability, moderation/governance, deployment/ops, extension/API + docs), each with its own detailed test-scenario catalog, a per-area Status cell, and a per-scenario checklist + **Resume checkpoint**. The federation/interop area references the [INTEROP_CONFORMANCE_MATRIX](docs/reference/INTEROP_CONFORMANCE_MATRIX.md) as its "are we consistent" artifact, and should pick up the two Phase 138 follow-ups (S2: generalize the vote-bar gate to capability-driven + rename `LemmyVoteBar` → `VoteBar`; S3: cosmetic `IsLemmy` rename) as UX-area scenarios. Index + area docs: [docs/plans/phase-139-platform-e2e-review.md](docs/plans/phase-139-platform-e2e-review.md).

Progress is tracked **inside the phase's own doc**, not enumerated here: check off slice/scenario checkboxes and update the doc's Resume checkpoint line as work happens. Only touch this Up Next section when the *whole* phase's status changes (starts, a Stage/area fully closes, or the phase completes) — then move the finished line to Recently Completed and add a [docs/ROADMAP.md](docs/ROADMAP.md) entry.
