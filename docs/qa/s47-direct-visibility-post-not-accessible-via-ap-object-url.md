...
- **Status:** CLOSED (dev1 `fbeb600b`, merged to main `9c61057d`, 2026-09-22). Direct-visibility post's `to`/`cc` arrays now correctly include the recipient's IRI, so the VisibilityFilter grants access. AP object URL resolves for both author and recipient.
