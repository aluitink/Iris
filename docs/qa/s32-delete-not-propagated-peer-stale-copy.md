...
## Re-test (Pass 155, 2026-09-21, build `38ae87c`) — S32 re-confirmed LARGELY FIXED (no regression)

Re-verified the Pass 142 test notes on the current build (`38ae87c`, unchanged since Pass 142):

- **Delete (II-S32-3, `…/u/ii-a1/notes/06GC5QJW…`):** `GET A <note>` → **Tombstone** (`formerType` Note, `deleted`=`07:32:19Z`); `GET B <note>` (via B proxy) → **Tombstone** (`formerType` Note, `deleted`=`07:32:19Z` — **same** ts as A). B's cached copy is a Tombstone (no stale live copy). **Delete propagation HOLDS.**
