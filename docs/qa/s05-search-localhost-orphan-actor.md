...
**Re-verification evidence (Pass 274, 2026-09-22, QA, deployed main `1b2e6b62` carrying the `401c08b5` directory refinement):** the S5 stale-local-drop facet **still holds** after the `401c08b5` `IsSameInstanceActor` refinement (which re-introduced the risk that a non-canonical actor whose handle matches a local handle could be re-kept). Directory → **People → "All known"** (signed `ii-a1`@A, clean entry, 0 console errors) lists exactly **one `alice`** — the canonical `https://qa-iris-a.luit.ink/ap/v1/u/alice` — with **no** `http://localhost:8088/ap/v1/u/alice` orphan. Confirmed on the wire too: signed `GET /ap/v1/search?q=&type=Actor&local=false&limit=100` (200) returns the canonical `alice` and **no localhost IRI**. This also confirms the *other* half of `401c08b5` (see the Directory Inbox item): **remote** actors with a preferredUsername (`ii-b1`@qa-iris-b, `probep`@qa-iris-b, `gnomon`@mastodon.social) are **kept** in "All known". **S5 holds (stale-local drop intact).**
