...
- **Status:** **FIXED (Pass 153, build `38ae87c`)** — all facets now work: **A8.2 (direct-view)** `GET B /ap/v1/c/{name}` serves the cached remote Group (200, `aebe420`); **A8.2 (federated-community-discovery)** B's Directory "All known" Communities tab + Search page list the remote community with a Join button (`f9ca2d2`); **A8.3 (follow/join)** Join federates the Follow to the owner (A's community `/followers` = ii-b1); **A8.4 (community-post federation + feed-surfacing)** the cross-post leg works (B → A, attributed to the community) AND **the member cross-post is now SURFACED in the owner community's feed** (the `/feed` endpoint [200] + the UI community feed both show the II-S30-4 cross-post — Pass 153). **S30 is fully FIXED** (all of A8.2/A8.3/A8.4 work).
...
## Re-test (Pass 153, 2026-09-21, build `38ae87c`) — A8.4 feed-surfacing now WORKS; S30 fully FIXED

Re-verified the A8.4 feed-surfacing facet (the last open S30 facet, residual from Pass 143) on `38ae87c` (no code change since `38ae87c`; the community feed query now surfaces the member cross-post). The II-S30-4 cross-post (B document `…/u/ii-b1/documents/06GC5WN0QDRV288ZWDN57NN09G`, posted to the remote A community in Pass 143) is **now surfaced in the owner community's feed**:

1. **The `/feed` endpoint (200) contains II-S30-4.** `GET A /ap/v1/c/ii-a8-community/feed` → **200**, `type OrderedCollection`, totalItems=20; the **II-S30-4 cross-post is present** (`FOUND II-S30-4 cross-post in community feed | iri …/u/ii-b1/documents/06GC5WN0…`). **The feed-surfacing facet is resolved** — the community-attributed member cross-post is now in the owner community feed.
2. **The UI community feed on A shows II-S30-4.** The A community page (`/community?iri=…/c/ii-a8-community`) **Feed** tab renders the II-S30-4 post: a list item with a **"To ii-a8-community"** label (linking to the community) + the content **"II-S30-4 cross-post from B into remote A community (A8.4 re-verify, 38ae87c)"** + a Like button. **The UI surfaces it** (the Pass 143 residual — "the UI community feed on A also doesn't show II-S30-4" — is resolved).

**Note on the feed's scope (informational, not a defect for A8.4):** the `/ap/v1/c/ii-a8-community/feed` collection is an **OrderedCollection of activities** (item-type breakdown: 11 Create + 3 Follow + 2 Undo + 1 Announce + 3 Like) and includes some **public posts that are not community-addressed** (e.g. II-A4-5, II-A4-6) + **Follow/Undo activities** — i.e. it behaves more like a **community activity stream** than a strict community-only post list. The **A8.4 requirement (a member cross-post attributed to the community is surfaced in the community feed) is met** (II-S30-4 is present in both the endpoint + the UI). The broader "should the community feed contain non-community public posts / follow activities?" question is a **separate feed-scope design question** (out of scope for A8.4) — noted for dev, not a blocker.

**Verdict (build `38ae87c`): S30 A8.4 feed-surfacing FIXED → S30 fully FIXED.** (a) The cross-post leg works (Pass 143); (b) the `/feed` endpoint (200) + the **UI community feed now surface the member cross-post (II-S30-4)** (this pass); (c) the "no compose UI" claim was stale (Pass 143). **All of A8.2 (direct-view + discovery) / A8.3 (follow/join) / A8.4 (community-post federation + feed-surfacing) now work. Status: FIXED.** (Informational: the community feed is a broad activity stream — a separate feed-scope design question, out of scope for A8.4.)
