...
- **Status:** **CLOSED (QA re-verified, 2026-09-22, build `7620faa1`)** — the S2/S14 proxy fix (Pass 259) holds: signed-out remote actor reads route through the proxy (same underlying fix as [S2](s02-signed-out-proxy-bypass.md), re-verified on the fresh build).
...
**Re-verification evidence (Pass 54, 2026-09-20, deployed `65ccfa0`):** signed-out `/actor?iri=https://mastodon.social/users/deadline` → **5 console errors**: 1× proxy 401 (`GET /ap/v1/proxy/https%3A%2F%2Fmastodon.social%2Fusers%2Fdeadline` → 401) + 4× CSP-violation on the direct fallback (`Connecting to 'https://mastodon.social/users/deadline' violates the following Content Security Policy directive: "connect-src 'self'"`). Page shows **"Failed to load actor. It may not exist or the server is unreachable."** The proxy 401 for unsigned GETs is unchanged (same root cause as S2). The client still falls back to direct fetches after the proxy 401, triggering CSP violations. **Identical failure mode to Pass 32** — the anonymous proxy seam is deployed but not functional for unsigned Mastodon GETs. STILL OPEN.

**Re-verification evidence (Pass 60, 2026-09-20, deployed `65ccfa0`):** signed-out `/actor?iri=https://mastodon.social/users/deadline` → **5 console errors**: 1× proxy 401 (`GET /ap/v1/proxy/https%3A%2F%2Fmastodon.social%2Fusers%2Fdeadline` → 401) + 4× CSP-violation on the direct fallback (`Connecting to 'https://mastodon.social/users/deadline' violates the following Content Security Policy directive: "connect-src 'self'"`). Page shows **"Failed to load actor. It may not exist or the server is unreachable."** **Identical failure mode to Pass 54 and Pass 32** — the anonymous proxy seam is deployed but not functional for unsigned Mastodon GETs. STILL OPEN.

**Re-verification evidence (Pass 73, 2026-09-20, Dev fix deployed):** signed-out `/actor?iri=https://mastodon.social/users/deadline` → **5 console errors**: 1× proxy 401 (`GET /ap/v1/proxy/https%3A%2F%2Fmastodon.social%2Fusers%2Fdeadline` → 401) + 4× CSP-violation on the direct fallback. Page shows **"Failed to load actor. It may not exist or the server is unreachable."** **Identical failure mode to Pass 60, 54, and 32** — no change. STILL OPEN.
