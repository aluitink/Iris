...
| S24 | Cross-instance follow: Following tab omits remote actors, spurious self-follow in outbox, remote-actor GET 404s | bug / data-integrity | S2 | open — **re-test 2026-09-21 (build `27b1ba6`): D3 (remote-actor GET) FIXED (200); D1 (Following tab) refined to a UI rendering defect — a FRESH follow materializes the wire `following` edge correctly (A `following` now includes ii-b1, in sync with `followers`), yet the Following tab STILL omits the remote ii-b1 (shows only the local community) while the Followers tab renders ii-b1 fine → the Following tab drops remote actors regardless of wire state; D2 variant (A's outbox stores foreign B-authored follow activities) persists; the earlier `following`/`followers` out-of-sync was a transient stale-state artifact of the Undo cycle** | [s24](s24-cross-instance-follow-state-inconsistent.md) |
...
| S28 | Remote Announce (Boost) delivered+stored on author but not surfaced in note's `shares` | bug / data-integrity | S2 | open — **REGRESSED (Pass 102, 2026-09-21, build `27b1ba6`): the remote Announce is now DROPPED at the shared inbox ("no local recipient"), so the author's `shares`/`sharedCount`/Shares tab are all empty (Shares tab "No boosts yet", Boost count 0). Same class as S27 (Like dropped). The S28 fix is in dev's uncommitted WIP, not yet deployed** | [s28-remote-announce-stored-not-in-shares.md](s28-remote-announce-stored-not-in-shares.md) |
...
| S32 | Delete (tombstone) emitted locally but NOT propagated to peer; peer keeps stale live copy | bug / federation | S2 | open — **re-verify 2026-09-21 (build `27b1ba6`), cleanest repro: B genuinely had a live copy of the note; the Delete was rejected on B (`unknown recipient <note-IRI>`), and B's Tombstone is a lazy refetch of A's current doc, not a propagated delete — a non-refetching peer would keep a stale live copy** | [s32-delete-not-propagated-peer-stale-copy.md](s32-delete-not-propagated-peer-stale-copy.md) |
...
| S37 | Remote Like is stored + `/likes` correct + UI "N like" shows, but the Note's `likedCount`/embedded `likes.totalItems`/inline Like-count stay 0/None (count not materialized) — the count-analog of S28 | bug / count-materialization | S3 | **NEW (Pass 107/108, 2026-09-21, build `27b1ba6`)** — reproduced on 2 independent notes (A→B Like): `/likes` `totalItems:1` + UI "1 like", but `GET <note>` `likedCount` None + embedded `likes.totalItems` 0 + inline button 0. Lower severity than S28 (the Like IS stored+rendered; only the denormalized count on the Note is stale). | [s37](s37-remote-like-stored-but-likedcount-not-materialized.md) |

**Open: S2, S3, S4, S14, S17, S19, S20, S21, S24, S28, S30, S32, S35, S36, S37 (15). Fix-committed-not-live: S6. Fixed: S5, S7, S8, S9, S10, S11, S12, S13, S15, S26, S27, S29, S31, S33, S34 (15). Superseded: S25 (by S36). Partially-fixed: S18. Core-fixed-UX-gap: S16.**
