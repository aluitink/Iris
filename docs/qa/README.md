...
| S30 | Cross-instance community join/view blocked (remote community unreachable from peer) | bug / federation | S2 | **PARTIALLY FIXED (Passes 136 + 143, build `38ae87c`)** — **A8.2 direct-view FIXED** (`aebe420`) **+ A8.2 federated discovery FIXED** (`f9ca2d2`) **+ A8.3 follow/join FIXED**; **A8.4 NARROWED (Pass 143, build `38ae87c`)** — the **"no compose UI" claim is STALE**: the community-page **"＋ Post to this community"** entry point + compose `?community=` selector + the **cross-post leg (B → A) WORK** (a B cross-post federates to A **attributed to the A community** — `attributedTo` includes the community, `to`=community+Public) **+ the `/ap/v1/c/{name}/feed` endpoint now returns 200** (was 404 in Pass 138). **RESIDUAL (narrowed A8.4): a member cross-post is NOT surfaced in the owner community's feed** (federated + stored attributed to the community, but the community feed query doesn't include it). **S30 now reduces to the A8.4 community-feed-surfacing facet.** | [s30-cross-instance-community-join-blocked.md](s30-cross-instance-community-join-blocked.md) |
...
**Open: S2, S3, S4, S14, S17, S19, S20, S21, S24, S30, S35, S36, S38 (13) + **S28, S32, S37 PARTIALLY/LARGELY FIXED** (wire-level, build `38ae87c`, Passes 141/142 — residual UI + mechanism facets). Fix-committed-not-live: S6. Fixed: S5, S7, S8, S9, S10, S11, S12, S13, S15, S26, S27, S29, S31, S33, S34 (15). Superseded: S25 (by S36). Partially-fixed: S18, S28, S30, S32, S37. Core-fixed-UX-gap: S16.**
