...
| S30 | Cross-instance community join/view blocked (remote community unreachable from peer) | bug / federation | S2 | **PARTIALLY FIXED (Pass 136, build `f9ca2d2`)** — **A8.2 direct-view FIXED** (`aebe420`: `GET B /ap/v1/c/{name}` serves the cached remote Group 200 + B UI renders the community page) **+ A8.2 federated discovery FIXED** (`f9ca2d2`: B's Directory "All known" Communities tab + Search page now **list the remote community** w/ a Join button) **+ A8.3 follow/join FIXED** (Join federates the Follow to the owner — A's community `/followers` = ii-b1); **A8.4 community feed / post federation STILL OPEN** (`GET B /ap/v1/c/ii-a8-community/feed` → 404 + no compose community selector). **S30 now reduces to A8.4.** | [s30-cross-instance-community-join-blocked.md](s30-cross-instance-community-join-blocked.md) |
