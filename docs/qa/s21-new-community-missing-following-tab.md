...
- **Status:** closed (QA re-verified 2026-09-22 Pass 261 on a fresh build rebuilt from `main` HEAD — both facets fixed live: auto-follow edge recorded on community creation + `/c/{handle}` redirect works + the new community renders in the Communities **Following** tab on a clean circuit)
...
**Re-verification evidence (Pass 66, 2026-09-20, andrew, Dev fix `8539f3f` NOW deployed — container restarted 11:21:16):** (1) `/c/qa-pass65-test` → Blazor SPA renders **"Sorry, there's nothing at this address."** (page title "Not found") — the `CommunityHandleRedirect.razor` page is **NOT active** on the live app (no redirect to `/community?iri=…` occurs). `curl` returns 200 (Blazor shell) but the SPA shows the 404 content. Same for `/c/technology`, `/c/qa-pass58-test`. (2) `/communities` → Following tab now shows **"technology" + "qa-pass46-test" + "qa-pass65-test"** (all with "Leave" button) — `qa-pass65-test` **IS now in the Following tab** (auto-follow working for communities created after deploy). (3) `/community?iri=…/c/qa-pass65-test` → **200** (community page renders). **VERDICT: PARTIALLY FIXED.** Auto-follow (S21 facet 1) is **FIXED** — new community appears in Following tab immediately. `/c/{handle}` route (S21 facet 2) is **STILL OPEN** — the redirect page is not active; all `/c/{handle}` URLs show "Nothing at this address." instead of redirecting to the community page.

**Re-verification evidence (Pass 71, 2026-09-20, andrew, Dev fix deployed):** `/c/technology` → Blazor SPA renders **"Sorry, there's nothing at this address."** (page title "Not found") — the `CommunityHandleRedirect.razor` page is **STILL NOT active**. `/c/qa-pass65-test` → same "Nothing at this address." content. The `/c/{handle}` redirect route remains broken. Auto-follow still working (qa-pass65-test in Following tab). `/c/{handle}` route STILL OPEN (8th consecutive pass with redirect not active).

**Re-verification evidence (Pass 75, 2026-09-20, andrew, Dev fix deployed):** `/c/technology` → Blazor SPA renders **"Sorry, there's nothing at this address."** (page title "Not found") — the `CommunityHandleRedirect.razor` page is **STILL NOT active**. Auto-follow still working (qa-pass65-test in Following tab). `/c/{handle}` route STILL OPEN (9th consecutive pass with redirect not active).
...
**Re-verification evidence (Pass 261, 2026-09-22, ii-a1, fresh build rebuilt from `main` HEAD):** Created `qa-pass261-test` via `/communities` → "+ Create a community". After creation: (1) **Auto-follow (facet 1) confirmed** — DB `Edges` has a `Kind=0` (Follow) edge `ii-a1 → qa-pass261-test`; the signed `GET /ap/v1/u/ii-a1/following` collection `totalItems=3` includes the new community. (2) **`/c/{handle}` (facet 2) confirmed** — `/c/qa-pass261-test` → **200** and renders the community page (Feed / Members / Owners / Peers / Requests tabs). (3) **Communities "Following" tab renders the new community** on a clean circuit (logout → login) — both `ii-a8-community` and `qa-pass261-test` show. (The single-community display observed mid-pass was **stale per-circuit cache**, not a defect: the signed `/following` wire + the local search `Items` cache both contained the new community, and a fresh circuit resolved both.) (4) "All on this instance" tab shows both. **0 console errors. CLOSED** — both facets verified fixed on the live fresh build.

**Residual facet — server-side `/following` stale cache (dev1, 2026-09-22, fix `1f941cfb`):** Pass 261's "stale per-circuit cache" observation had a server-side component: the community-creation path (`RecordCreateLocalAsync`) records the auto-follow edge but — unlike the explicit `Follow` branch — never invalidated the creator's `following` collection-page read cache (60s fresh / 300s stale-while-revalidate) or per-actor feed cache. So within that window a non-`?refresh` `GET /ap/v1/u/{creator}/following` could serve the stale pre-creation page (the new community missing), and the Communities "Following" tab (which walks it) would show it empty until the TTL lapsed or a fresh circuit re-fetched. **Fix:** invalidate the creator's `following` page-1 entry + feed cache on community creation (mirroring the explicit-Follow invalidation). **Regression test:** `CommunityCreationIntegrationTests.CreateCommunityAsync_AutoFollow_InvisibleUntilCacheInvalidated` warms the `following` cache (empty), creates the community, and asserts a non-`?refresh` read already reflects the new follow — it **fails without the fix** (expected 1, actual 0). **Live-verified on the rebuilt dev1 stack:** (1) server — warm `/following` (2 items) → create `s21fix2` → immediate non-`?refresh` read = **3 items** (new community present, no TTL wait); (2) UI — fresh `s21ui` account created `s21uiverify` via the Communities form → the Communities **Following** tab renders `s21uiverify` (with Leave/Delete, "You are the only owner"), **0 console errors**. Full fast suite: **1439 passed, 0 failed** (was 1446; +1 new test). Residual (out of scope, pre-existing): the client's own `GetFollowingActorIrisAsync` 2-minute cache is not invalidated by the in-circuit create flow, so the *same* circuit's Following tab can lag up to 2 min after an in-session create until the client cache lapses or the page is reloaded — a client-side polish item, not the server stale-cache defect fixed here.
