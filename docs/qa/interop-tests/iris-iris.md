...
- **STILL OPEN (reproduced):** **S24 f1** (Following tab omits remote), **S24 f2** (variant: foreign activity in local outbox), **S25** (remote post not in home feed), **S27** (remote Like not applied on author — now UI-testable), **S30** (community discovery + community-post federation — though A8.3 follow now works), **S31** (edit clears `published` + UI no-refresh + drops peer copy), **S33** (unfollow `Undo` not propagated).
