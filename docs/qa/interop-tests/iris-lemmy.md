...
|---|---|---|---|
| L1 | **BLOCKED(registration-pending)** — Lemmy side needs admin approval; Iris side fine | **Iris `il-user`:** (not yet created this pass — will create on resume; the Iris instance is confirmed healthy + registration open, as `im-user` was created on the same host minutes earlier). **Lemmy `iluser`:** could NOT be self-served — the `/signup` page shows the alert *"To join this server, you need to fill out this application, and wait to be accepted."* (registration mode = **Pending**; the summary `/api/v3/site` reports `registration_mode: null` but the UI + a required **"Answer"** application field confirm Pending). **Password policy:** the form requires **10–60 characters** ("Password length must be between 10 and 60 characters") — so the suite's `Password1` (9 chars) is rejected; the Lemmy account needs a **longer password** (record the one used). **L1.1 PASS:** Lemmy landing renders (`Iris Lemmy Interop`, BE 0.19.20) + `/api/v3/site` → 200. **L1.3 control PASS:** webfinger `acct:lemmyadmin@qa-lemmy.luit.ink` → 200, `self` = `https://qa-lemmy.luit.ink/u/lemmyadmin` (Lemmy serves actor docs — **unlike the Mastodon cluster**, see S35). **L1.2 BLOCKED:** `iluser` lands in **pending** state awaiting admin approval; the agent cannot approve (no Lemmy admin credentials). **Operator step to unblock:** (a) approve the pending `iluser` application via Lemmy admin (`/admin` → registrations / `POST /api/v3/user` approve), **or** (b) set the instance `registration_mode` to `Open` and the agent re-creates `iluser` with a 10–60 char password. Until then, **L2–L12 are BLOCKED** (they all require a signed-in Lemmy `iluser`). | 2026-09-21 |

### L-suite blocker note (2026-09-21)
Lemmy is a **Pending-registration** instance (application + admin approval) with a **10–60 char** password policy. Unlike Mastodon (S35 — broken actor docs), Lemmy's federation surface is **healthy** (site API + webfinger + actor docs all work), so **once `iluser` is approved the full L2–L12 suite is runnable**. The only gate is the **operator approving the pending account** (or flipping registration to Open). This is a **test-harness credential** blocker, not an Iris or Lemmy defect.
