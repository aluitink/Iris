...
- **Status:** **S24 CLOSED** — **D1 (Following-tab omits remote actor) FIXED** (Pass 116). **D2 (foreign activities in the local actor's outbox) FIXED** (2026-09-22): `GetOutboxAsync` in all three store implementations (`EfActivityStore`, `InMemoryActivityStore`, `FileBackedActivityStore`) now filters by `activity.Actor` — only activities authored by the actor are returned; foreign (inbound) activities are excluded. **D3 (remote-actor direct GET) FIXED** (the S24-D3 actor-doc fallback). **D4 (remote-actor direct COLLECTION routes 404) FIXED `2804fb55`** (2026-09-22): `CollectionEndpointHandler` now proxies a cached remote actor's `outbox`/`followers`/`following` to the remote instance (new `ProxyRemoteActorCollectionAsync` helper); live-verified both directions on the dev1 stack (A↔B) — all three collection routes 200 (were 404), 0 console errors. All four defects resolved.
