...
- **Status:** **PARTIALLY FIXED** — **D1 (Following-tab omits remote actor) FIXED** (Pass 116). **D3 (remote-actor direct GET) FIXED** (the S24-D3 actor-doc fallback). **D4 (remote-actor direct COLLECTION routes 404) FIXED `2804fb55`** (2026-09-22): `CollectionEndpointHandler` now proxies a cached remote actor's `outbox`/`followers`/`following` to the remote instance (new `ProxyRemoteActorCollectionAsync` helper); live-verified both directions on the dev1 stack (A↔B) — all three collection routes 200 (were 404), 0 console errors. **D2 (foreign activities in the local actor's outbox):** **feed-surface facet FIXED `61c328fa`** (2026-09-22) — the home feed's own-outbox branch now keeps only the actor's own authored content (`Create` of a Note/Article/Question, or their own `Announce`), so foreign boost fan-out + actor-doc noise (the self-`Follow`, `Update`, etc. that pollute the outbox) no longer surface in the home timeline (S36). The raw outbox still physically contains the foreign/actor-doc items (the *outbox-integrity* facet, 6 foreign on page 1 / total 55) — **that write-path facet is STILL OPEN (dev2 investigating)**. **Status: D2 OPEN (outbox-integrity facet only; feed-surface facet fixed).**
