...
- **Status:** **S24 CLOSED** — **D1 (Following-tab omits remote actor) FIXED** (Pass 116). **D2 (foreign activities in the local actor's outbox) FIXED** (2026-09-22) — two complementary fixes: (a) `GetOutboxAsync` in all three store implementations (`EfActivityStore`, `InMemoryActivityStore`, `FileBackedActivityStore`) now filters by `activity.Actor` — only activities authored by the actor are returned, so foreign (inbound) activities are excluded from the outbox read path; (b) `FeedService` own-outbox branch (`61c328fa`) keeps only owner-authored content (`Create` of a Note/Article/Question, or own `Announce`), so any residual foreign boost fan-out / actor-doc noise does not surface in the home timeline (S36). **D3 (remote-actor direct GET) FIXED** (the S24-D3 actor-doc fallback). **D4 (remote-actor direct COLLECTION routes 404) FIXED `2804fb55`** (2026-09-22): `CollectionEndpointHandler` now proxies a cached remote actor's `outbox`/`followers`/`following` to the remote instance (new `ProxyRemoteActorCollectionAsync` helper); live-verified both directions on the dev1 stack (A↔B) — all three collection routes 200 (were 404), 0 console errors. All four defects resolved.
