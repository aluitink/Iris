...
- **Status:** **LARGELY FIXED (Pass 141, build `38ae87c`)** — the Pass-102 regression (Announce dropped at the shared inbox "no local recipient") is RESOLVED (delivered + accepted + stored + surfaced), **AND the wire count facet is now FIXED**: a remote Boost on the note materializes the denormalized `…/ns#sharedCount: 1` (iris: namespace) + `/shares` totalItems=1 + the "Shares (1)" tab (verified on a fresh note in Pass 141). The fix is dev's `38ae87c` (S37 — `RefreshObjectCountsAsync` computes all four denormalized counters; the Boost `sharedCount` converges via the periodic pass). **Remaining (UI facet, S3, low):** the object-detail **Boost button still renders "0"** + the note doc's embedded `shares.totalItems` stays 0 (the client button count doesn't read the denormalized `sharedCount`) — the same UI facet as [S37](s37-remote-like-stored-but-likedcount-not-materialized.md)'s Like button. **Status: LARGELY FIXED — wire count FIXED; Boost-button UI facet open.**
