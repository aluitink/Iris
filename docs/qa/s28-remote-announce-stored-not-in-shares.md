...
- **Status:** **FIXED (live-re-verified 2026-09-22 on dev1, commit `5355e968`)** — the shared-inbox Announce-drop is RESOLVED **and** the Boost's denormalized `sharedCount` now materializes **immediately** (no 30 s periodic wait). The fix (`5355e968`) routes an inbound `Announce` at the shared inbox to the **announced object's owner** (mirroring the Like branch), so the owner's `AnnounceActivityHandler` records the announcer→object edge + calls `RefreshObjectCountsAsync` (materializing `…/ns#sharedCount` on the next read). **Remaining (UI/doc-serializer facet, S3, low):** the object-detail **Boost button still renders "0"** + the note doc's embedded `shares.totalItems` stays 0 (the client button reads the embedded collection count, not the denormalized `sharedCount`) — the same UI facet as [S37](s37-remote-like-stored-but-likedcount-not-materialized.md)'s Like button. **Status: FIXED — wire `sharedCount` + `/shares` FIXED (immediate); Boost-button UI facet open (S3).**
...
- **Remaining (S3 UI facet):** the object-detail **Boost button** still renders "0" + the note doc's embedded `shares.totalItems` stays 0 (the client button reads the embedded collection count, not the denormalized `sharedCount`) — same facet as S37's Like button.

**Verdict: S28 FIXED** — the remote Boost is delivered + accepted + recorded on the note's home; the `/shares` collection + denormalized `sharedCount` are correct and materialized immediately. Only the Boost-button UI facet remains (S3, client/UI).
