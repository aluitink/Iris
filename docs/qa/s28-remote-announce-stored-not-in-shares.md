...
- **Status:** **FIXED (wire + UI facet)** — the shared-inbox Announce-drop is RESOLVED **and** the Boost's denormalized `sharedCount` now materializes **immediately** (no 30 s periodic wait). The fix (`5355e968`) routes an inbound `Announce` at the shared inbox to the **announced object's owner** (mirroring the Like branch), so the owner's `AnnounceActivityHandler` records the announcer→object edge + calls `RefreshObjectCountsAsync` (materializing `…/ns#sharedCount` on the next read). **Wire fully fixed:** `sharedCount` + `/shares` endpoint + the object-detail **Shares tab / header count** all reflect a cross-instance Boost (re-verified Pass 272 on the QA stack, 2026-09-22). **The cross-instance Boost-button UI facet (Pass 272) is now FIXED** by two commits (dev1, 2026-09-22): (1) `3836bebd` — the **root cause**: `ActorSessionAccessor.IrisNamespaceBase` derived the `iris:` extension namespace from `_rewriteBase` (the canonical FQDN baked into the WASM's static `appsettings.json`, e.g. `https://iris.luit.ink`) instead of `_advertiseBase` (the effective dial base = the server's own `BaseUri`, e.g. `https://dev1-iris-a.luit.ink`). On any multi-instance / renamed deployment the static FQDN differs from the dial host, so the `EngagementBar` read `iris:sharedCount` under the **wrong** namespace, found nothing, and silently fell back to the `/likes`/`/shares` walk (0 for a cross-instance Boost the owner views), while the Shares tab (which walks the collection) correctly showed 1. The bar now derives the namespace from `_advertiseBase` (== `_rewriteBase` in the single-instance case, so unchanged there). (2) `18e803ba` — belt-and-braces: the `EngagementBar` fast path no longer gates on `!isRemote`, so a non-Iris remote note the owner views also prefers the server-rendered `iris:` counters. **Live-verified (dev1 stack, Playwright):** owner of a cross-instance-Boosted note now sees the Boost button = **1**, matching the "1 boost" header and the Shares tab. **Status: CLOSED.**
