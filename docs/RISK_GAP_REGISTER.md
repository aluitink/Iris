...
> **Cross-reference — user-journey walkthrough (Phase 11, Slice 11.2).** [PHASE_11_USER_JOURNEYS.md](PHASE_11_USER_JOURNEYS.md) re-derives these capability gaps **from the user's point of view** (each capability walked end-to-end as an app would drive it) and confirms G-1…G-6 (as J-18/J-10/J-11/J-15/J-4/J-8 respectively). It also surfaces a **new usability-friction register (J-1…J-22)** the capability register does not cover — most notably **J-6 (client "post" API)**, and (now resolved) **J-9 (client "follow" API — Slice 11.4)**, **J-6's client half (client "post" API — Slice 11.5)**, and **J-21 (client discovery service not exposed in the bundle — Slice 11.3)**: the *client* write side is now mostly reachable, even though the *read* side was already solid. (J-21 is closed — `IrisClientBundle` exposes `Discovery`/`ResolveActorAsync`; J-9 is closed — `IActivityPubClient.FollowAsync(actorId, targetId)` builds + delivers a signed `Follow` to `targetId.InboxOf()`; J-6's client half is closed — `IActivityPubClient.PostNoteAsync(actorId, content, to)` builds a `Create` (embedded `Note`, deterministic id) + delivers it to the author's own inbox. The remaining write-path work is J-6's **server half** — record the post in the author's outbox (J-8) + outbound `Create` to followers (J-18) — so a local post is surfaced and federates.) Phase 12 should close that first (record in outbox → outbound `Create` → followed feed).
