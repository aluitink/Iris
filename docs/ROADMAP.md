...
- [ ] **22.6 — Implementation sweep + manual test pass.** Implement the remaining items and end the
  ...
  **In progress:** Actor-detail **Inbox** (change
  [191](changes/191-22.6-inbox-pagedcollection-consolidation.md)) and Community **following/followers**
  (change [192](changes/192-22.6-community-following-followers-pagedcollection.md)) consolidated onto the
  shared `PagedCollection` via the robust field-based `RenderFragment<T>` `ItemTemplate` pattern (inline
  lambda attributes are unreliable). Remaining: mutes/blocks/flags cards (actor + community) and the full
  manual pass (incl. the external-FQDN reverse-proxy route).
