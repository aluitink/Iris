...
- [ ] **Server caching**: remote actor cache, remote key cache, collection page cache, WebFinger cache. `?refresh=true` bypass on all cached endpoints. `Cache-Control` headers on responses. TTLs configurable (`ServerCachePolicies` options class exists but is not yet wired to a cache layer).
- [x] **Integration tests** (live `TestServer`): public actor doc, authenticated actor doc (with PEM `privateKey`), wrong-password → public doc, WebFinger resolution + unknown-handle 404, NodeInfo + discovery root, and the `Iris-Version` meta header. 10 tests. `remaining:` cache hit/miss/refresh tests (tracked with the server-caching bullet above).
