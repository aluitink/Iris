...
- [x] Phase 18: client/server hardening. **18.1 done** (2026-08-31): client `RetryHandler` honors the `Retry-After` HTTP-date form (RFC 9110 §10.2.1), not just the delta-seconds form — the server (Phase 17.3) may send either form on 429/503 responses; a past date or absent header falls back to the exponential backoff; no new NuGet (change 133). **18.2 done** (2026-08-31): end-to-end integration test proving the server's 429 inbound rate-limit (Phase 17.4) → client `RetryHandler` retry loop (Phase 18.1) works together over a genuine HTTP stack — the `FakeActivityPubServer` gains a rate-limit mode (`rateLimitAfter` + `rateLimitResumeAfter`) that 429s requests after the first N (with `Retry-After: 1`), and the client's `RetryHandler` honors the header and retries until the server resumes; 1 new test (change 134). **18.3 done** (2026-08-31): server-side 429 with an HTTP-date `Retry-After` (RFC 9110 §10.2.1) — the `IInboundRateLimiter` gains a `GetRetryAfter` method that returns the time when the peer's window resets; the server's 429 response now sends an HTTP-date `Retry-After` (precise, not a fixed 60-second delta); 5 new unit tests + 1 updated integration test (change 135).
