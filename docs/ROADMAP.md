...
- [x] Phase 18: client/server hardening. **18.1 done** (2026-08-31): client `RetryHandler` honors the `Retry-After` HTTP-date form (RFC 9110 §10.2.1), not just the delta-seconds form — the server (Phase 17.3) may send either form on 429/503 responses; a past date or absent header falls back to the exponential backoff; no new NuGet (change 133).
