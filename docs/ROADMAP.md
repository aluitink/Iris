...
- [ ] **Code consolidation**: find duplicated logic across `Iris.Core`/`Iris.Client`/`Iris.Server` (e.g. parallel cache engines, repeated IRI/JSON helpers, near-identical handler stacks) and consolidate into shared abstractions. *(Slice 1.1: consolidated the triple-duplicated `IObjectOrLink`→`Iri` resolution into `IriExtensions.ResolveObjectIri`. Slice 1.2: moved `CollectionPage` to `Iris.Core` + added `CollectionPageFactory.FromOrderedCollectionPage` + `IriExtensions.ResolveCollectionIri` — eliminated the duplicated page-flatten + `TryGetIri` in both `ActivityPubClient` and `IrisRemoteCollectionFetcher`. Remaining: parallel cache engines, near-identical handler stacks.)*
