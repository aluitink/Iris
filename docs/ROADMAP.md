...
- [ ] **Test audit**: identify redundant, duplicate, or low-value tests (tests that assert the same behavior as a broader integration test, over-mocked unit tests that add no signal, dead fixtures). Remove or merge them.
- [ ] **Code consolidation**: find duplicated logic across `Iris.Core`/`Iris.Client`/`Iris.Server` (e.g. parallel cache engines, repeated IRI/JSON helpers, near-identical handler stacks) and consolidate into shared abstractions.
- [ ] **Dead-code sweep**: remove unused types, parameters, and configuration surface that accumulated across phases.
