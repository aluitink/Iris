...
- [ ] **Implement high-priority gaps**: close the spec-mandated and interop-critical items; defer the rest to Phase 14+. *(Slice 12.2 closed **F-01 `sharedInbox`** (Wave 1) — served on `endpoints` when configured and resolved as a delivery target from a remote actor's document. Slice 12.3 closed **F-02 `Update`** + **F-03 `Delete`** + **F-10 `Tombstone`** (Wave 1) — the `Create` handler stores the embedded object; a `UpdateActivityHandler` refreshes a stored object in place; a `DeleteActivityHandler` tombstones it (the IRI still resolves, serving the AS2.0 `Tombstone`); and `GET /ap/v1/o/{**path}` serves an object (or its tombstone) by IRI. Slice 12.4 closed **F-05 EdDSA/Ed25519** (Wave 1) — the signing/verification pipeline is unified around a common `ISigningKey` interface implemented by both `KeyPair` (RSA/EC, BCL) and `Ed25519Key` (Ed25519, BouncyCastle — the BCL has no Ed25519 type on this runtime); Ed25519 keys are accepted inbound (JWK `OKP` / Ed25519 PEM, classified + reconstructed by the inbound key resolver) and signable outbound. Slice 12.5 closed **F-08 `Move`** (Wave 1 — the last Wave 1 gap) — a `MoveActivityHandler` re-points the local follow edges (person followers via `IFollowStore`, community follows via `ICommunityStore`) when an actor migrates to a new IRI, and invalidates the moving actor's outbound key/actor-document caches (F-25); Wave 1 is now fully closed. Slice 12.6 added the conformance test suite (regression-protection). Slice 12.7 closed **F-04 `Like`** (Wave 2 — the first Wave 2 gap) — a `LikeActivityHandler` records the like edge for a local liker (so the actor's `liked` collection can be served) and records a `Like` delivered to a local community's inbox in each local member's outbox (the community feed); the `liked` collection endpoint is served + advertised on the actor document. Slice 12.8 closed **F-14 followed feed / home timeline** (Wave 2 — the highest user-value feature) — a new `IFollowFeedService` / `FeedService` merges an actor's local follows' outboxes (local store) with remote follows' outboxes (fetched over the wire via `IActorDocumentFetcher` + the shared `IActivityPubClient.GetCollectionAsync`, walking `first`/`next` across `FeedOptions.PagesPerActor` pages), de-duplicated + capped; served at `GET /ap/v1/u/{handle}/feed` (not through the local collection-page cache), advertised as a `feed` extension, and read by `client.GetFollowFeedAsync`; pull-on-read (no background poller). Remaining: Wave 2/3/4 gaps.)*
- [x] **Conformance test suite**: add integration tests that assert spec-required behaviors (wire format, headers, status codes, pagination semantics) so conformance is regression-protected. *(Slice 12.6: `ConformanceSuiteTests` (WebFinger `application/jrd+json` per RFC 8615, NodeInfo 2.0, the actor document's `@context`/`endpoints`, `endpoints.sharedInbox`) + `OutboundSignatureConformanceTests` (the C-03 `ServerToServer` signature base covers `digest`+`content-type`, round-trips through the verifier); plus the RFC 8615 fix — WebFinger is now served as `application/jrd+json`. 9 tests.)*
