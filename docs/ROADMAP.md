  ...
  [194](changes/194-22.6-actor-moderation-collections-pagedcollection.md)) are all consolidated onto the
   ...
   verification pass (change [206](changes/206-21.5.1-21.5.2-21.6.1-21.6.2-21.6.3-instance-webfinger-nav-error-raw-verification.md))
    ...
    - [x] **22.6.2 — Person settings write path: `SetManuallyApprovesFollowersAsync`.** *(change 221)*
     ...
     - [x] **22.6.2 — Typed readers for the settings-gate state.** *(change 223)*
       `IrisDocumentExtensions` gains `GetManuallyApprovesFollowers()` (person) /
...
- [ ] **19.5.2 — Membership management (live remainder).** The `Add`/`Remove` mechanism + the
  ...
  (change 216). The **AP-native settings change** (set/clear `manuallyApprovesMembers` via `Add`/`Remove`
  of the community document to the outbox) is done (change 217): the community's "Settings" card shows
  the gating state with toggle buttons; the flag is auditable in the outbox. Still open: the two-instance
  wire drive of a remote actor's Join → Accept (live-interop).
