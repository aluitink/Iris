...
- [ ] **Code consolidation**: find duplicated logic across `Iris.Core`/`Iris.Client`/`Iris.Server` (e.g. parallel cache engines, repeated IRI/JSON helpers, near-identical handler stacks) and consolidate into shared abstractions. *(Slice 1.1: consolidated the triple-duplicated `IObjectOrLink`→`Iri` resolution into `IriExtensions.ResolveObjectIri`. Remaining: parallel cache engines, repeated IRI/JSON helpers.)*
