...
- [ ] **Test audit**: identify redundant, duplicate, or low-value tests (tests that assert the same behavior as a broader integration test, over-mocked unit tests that add no signal, dead fixtures). Remove or merge them. *(Slice 1.1: audit complete — suite well-organized; removed the dead `LazyActorDocumentFetcher` fixture. Remaining: consolidate the ~9 copy-pasted per-test federation helpers into `Iris.Testing` + decide the trivial-smoke-test/harness bridge.)*
- [ ] **Code consolidation**: find duplicated logic across `Iris.Core`/`Iris.Client`/`Iris.Server` (e.g. parallel cache engines, repeated IRI/JSON helpers, near-identical handler stacks) and consolidate into shared abstractions. *(Slice 1.1: consolidated the triple-duplicated `IObjectOrLink`→`Iri` resolution into `IriExtensions.ResolveObjectIri`. Slice 1.2: moved `CollectionPage` to `Iris.Core` + added `CollectionPageFactory.FromOrderedCollectionPage` + `IriExtensions.ResolveCollectionIri` — eliminated the duplicated page-flatten + `TryGetIri` in both `ActivityPubClient` and `IrisRemoteCollectionFetcher`. Slice 1.3: consolidated the parallel `CachingClientCache<T>`/`CachingServerCache<T>` into a single `CachingReadThrough<T>` in `Iris.Core` — migrated all 10 typed façades. Remaining: near-identical handler stacks.)*
...
- [ ] **API surface pass**: review public API for consistency (naming, parameter conventions like `bypassCache`/`forceRefresh`, disposal contracts) ahead of the 1.0 stabilization.
...
- [ ] **User-journey walkthroughs**: for each capability (auth → key → sign → post → follow → community feed → proxy fallback → cross-instance delivery), trace the full path a real user/app would take and note every gap, dead-end, or confusing step.
- [ ] **Gap register**: catalog implementation gaps found (missing endpoints, unhandled activity types, incomplete error paths, missing config) with severity and a fix plan.
