...
- [x] Close **J-6 (client half) — the client had no "post a note" API** (Slice 11.5): posting required hand-building a `Create` with an embedded `Note` and knowing a delivery target. `IActivityPubClient.PostNoteAsync(Iri actorId, string content, IEnumerable<Iri>? to = null, ct)` builds the `Create` (embedded `Note` with content, deterministic ids from a content hash so a retried post dedupes, note attributed to the author, optional audience `to`) and delivers it to `actorId.InboxOf()` (the "local post" path) through the signed pipeline. The caller supplies only the content. Proven by 3 unit tests (Create shape + embedded-note serialization, deterministic-id dedupe, audience `to`, delivery-status passthrough) + 1 e2e test (`PostNoteIntegrationTests`): a local actor authenticates and posts over a live in-process server; the signed `Create` is accepted (signature validated; single-instance self-fetch of the author's actor doc via a deferred `LazyHandler`) (490→491). **Remaining (server half):** record the note in the author's outbox (J-8, **Slice 11.6** — done) + outbound `Create` to followers (J-18, Phase 12), so a local member's post is surfaced and federates.
- [x] Close **J-8 — the server did not surface a local post in the author's outbox** (Slice 11.6, J-6's server half, "recorded" portion): an inbound `Create` delivered to a local person's inbox was stored but never surfaced — the author's outbox never gained the post, so a local member's note was invisible in their own feed. A dedicated `CreateActivityHandler : ActivityHandlerBase<Create>` (registered in the server DI) now records the `Create` in the recipient's outbox when the recipient is a local **person** (the author's own post, J-8), and in the community's local members' outboxes when the recipient is a local **community** (the "followed content" half, delegating to the shared `CommunityContentRecorder`). Because the `InboxProcessor` prefers the most specific handler, this `Create` handler intercepts every inbound `Create` before the catch-all `CommunityInboxActivityHandler` (which now serves `Like`/`Announce` to a community's inbox); the member-recording loop is extracted into `CommunityContentRecorder` so the two community content paths do not diverge. Proven by 8 unit tests (`CreateActivityHandlerTests`: local-person outbox recording, newest-first ordering, remote-person no-op, community member-recording, remote-member skip, unknown-recipient no-op, null guards) + 1 e2e test (`PostNoteSurfacesInOutboxIntegrationTests`): a local actor posts via the client's `PostNoteAsync` over a live in-process server, then reads the author's outbox collection endpoint over HTTP and asserts the post is surfaced (491→500). **Remaining (J-18, Phase 12):** outbound `Create` to the author's remote followers, so the post federates.
