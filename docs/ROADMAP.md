...
- [ ] **19.8.1 — Click-through audit (every collection).** From each collection surface (actor
  ...
  not work.
   - **Audited live (161m, alice@iris-dev1, Playwright):** every *local* collection→view transition
     ...
     (community→ObjectPage renders content + Like/Boost). Gap (b) is now closed (change 171): the
     Instance page shows a **recent-instances** list (the session's `RecentInstances`) with one-click
     switching, the current instance marked "(current)". Still open: (a) **remote (cross-instance)
     object reads CORS-fail in the browser** — the client dials the remote IRI directly (e.g.
