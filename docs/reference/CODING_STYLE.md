...
> Part of the [Iris plan](../../PLAN.md). See also [Architecture](ARCHITECTURE.md), [Projects](PROJECTS.md), [Testing](TESTING.md), [Phase Ledger](../ROADMAP.md).
>
...
- **No new NuGet packages** without a note in [PLAN.md](../../PLAN.md)'s Active Slice (or the change doc) and a justification. The ActivityStreams package is the only non-BCL dependency in `Iris.Core`.
