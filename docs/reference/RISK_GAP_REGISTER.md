...
| **G-3** | **No outbound group-follow.** The server never follows a remote `Group` (no outbound group-follow path); Iris communities can't initiate following a remote community. | High | **Open (Phase 12).** *Verified:* a local community can **receive** follows (inbound `FollowActivityHandler` records the community's follow/follower sets + auto-schedules an `Accept`), but a local community can **not initiate** a `Follow` of a remote `Group` — the only outbound `Follow` construction is the client's `FollowAsync` (a local **person**), and there is no `POST /c/{name}/outbox` write surface for a community actor. | Matrix **G2** (out): "we did not initiate a follow of the remote community." | Fix = an outbound community-follow path (a community outbox write surface that constructs + delivers a signed `Follow` from the community actor). Unlocks receiving a followed community's content. |
