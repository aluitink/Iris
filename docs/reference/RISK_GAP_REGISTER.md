...
|---|---|---|---|---|---|
| **G-1** | **No outbound `Create`.** The server writes posts to local outboxes but never sends a signed `Create` activity to followers' inboxes. Remote followers never receive our posts. | **Blocker** | **MITIGATED (Phase 11, Slice 11.7; residual closed Phase 12, change 086).** Inbound `Create` → `CreateActivityHandler` records it in the author's outbox and **server-delivers a signed `Create` to every remote, non-blocked follower's inbox** (plus relay fan-out, F-06) — a local post now federates. The **outbox-publish** path (`POST /u/{handle}/outbox`, the client's `PostNoteAsync` write surface) now also fans out to **every** remote, non-blocked follower (`RecordCreateLocalAsync` returns `IEnumerable<Iri>`, mirroring `CreateActivityHandler`'s loop). 3 integration tests cover two-remote-follower fan-out, no-remote-followers local-only, and blocked-follower skip. | Matrix **C1** (out): "remote follower's inbox did NOT receive a signed `Create`." | Closed — both the inbox path and the outbox-publish path now fan out to the full remote-follower set. |
...
| **G-6** | **Person-inbox `Create`/`Like` uninterpreted.** Posts to a local *person* inbox are stored but not surfaced in a personal feed (only community inboxes interpret them via the catch-all). | Medium | **RESOLVED (Slice 11.6, J-8).** A dedicated `CreateActivityHandler` records an inbound `Create` in the recipient's outbox: for a local **person** it records in the person's outbox (the author's own post is now surfaced in their feed); for a local **community** it records in the members' outboxes (delegating to the shared `CommunityContentRecorder`). `dotnet test` 491→500. | Predicted | Matrix **C2** (in): "a `Create` to a person inbox is stored but not in a personal feed." | Closed — a person-inbox `Create` handler records into the person's outbox (the community member-recording loop is shared via `CommunityContentRecorder`). Resolved Decision #43. |
 ...
 > **G-1 was the headline — now fully mitigated.** Iris is no longer a *receiving-only* instance: a local
   > post is now federated — both the inbox path (`CreateActivityHandler`) and the outbox-publish path
   > (`OutboxPublishHandler`) server-deliver a signed `Create` to **every** remote, non-blocked follower's
   > inbox (the core "post and have it federate" use case works end-to-end). G-3 (outbound community-follow) is now the largest
