...
| Color contrast / accessibility audit | — | ✅ | ☐ | **DONE (Phase 111):** All text/background pairs verified against WCAG AA (4.5:1). Fixed 2 failures (`.directory-scope-btn--active`, `.filter-tab[aria-pressed]` were 2.8:1 → now 5.97:1). All other pairs: 5.0–15.2:1. |
 | Visual design system (spacing, type, icons) | — | — | ✅ | **DONE (Phase 113.1–113.3):** design-token system in `app.css` — semantic color tokens (`--danger*`/`--warning`/`--success`/`--btn-fg`/`--surface-inset`/`--border-strong`), a spacing scale (`--space-1…7` + off-ramp `-25/35/45/55/60/65/90/125`), and a type scale (`--font-size-xs…4xl`). All raw rem spacing values migrated onto tokens (zero raw rem spacing remains) so a theme/spacing change is a single value swap. |
