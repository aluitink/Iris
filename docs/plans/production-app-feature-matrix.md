...
| Mentions | ✅ | ✅ | ✅ | `@handle` in compose textarea; autocomplete popover on `@`/`#`; regex detection + `Mention`/`Hashtag` tags on post. **Phase 156 (Slice A):** the client now composes Mastodon-faithful `content` HTML — `MentionLinkify.Linkify` wraps each `@mention`/`#hashtag` token (first occurrence, word-boundary-guarded) in `<a class="mention" href="actorIri">`/`<a class="hashtag" href="searchUrl">` (Markdown-rendered first, existing `<a>` elements protected), so the tokens render as tappable accent-colored links in the note body (CSS `.object-content a.mention`/`a.hashtag`), in addition to the structured `tag` array. `BuildLinkedBody` runs only when the post has mentions/hashtags (posts with neither are unchanged). Live-verified 2026-09-17 (fresh WASM after `--no-cache` rebuild): note + reply with `@alice`/`@bob` + `#phase156`/`#replytag` → stored `content` carries the `<a>` links, `tag` carries the mention IRI + `Hashtag` object, and the feed renders both as tappable links (`#5b8cff`); 0 console errors. **Phase 156 (Slice B):** display-side linkify — a note whose `content` is plain text / Markdown (not pre-rendered HTML) but which carries those tokens in its `tag` array now renders them as tappable links too (`MentionLinkify.LinkifyPlain` matches body tokens against the declared tags; `ObjectView` routes all 5 content render sites through a shared `RenderObjectContent`, emitting pre-rendered HTML verbatim so it is never re-linkified). Live-verified 2026-09-17: a plain-text note with a `Mention`+`Hashtag` tag array renders both as tappable accent links; the pre-rendered Slice-A note still renders exactly 1 mention + 1 hashtag link (no double-linkify); 0 console errors. |
...
| Boost (announce) | ✅ | ✅ | ✅ | `EngagementBar` boost button — `AnnounceAsync`/`UnannounceAsync`, `iris:sharedCount`/`iris:isShared`/`iris:announceActivityIri` seed, live-verified (24/24 buttons render; server computes sharedCount=1 + minted announce IRI for a boosted note). |
 ...
 | Boost/like card of a reply shows the original post | ✅ | ✅ | ✅ | **Phase 152:** when a boost (Announce) or like (Like) card's target is itself a reply (`inReplyTo` set), `ObjectView` now renders the ORIGINAL post it answers to as the card body — keeping the "Boosted by" / "Liked" indicator — plus a "replying to …" context hint, instead of rendering the reply. New `_boostedParentObject`/`_likedParentObject` (best-effort `GetContentObjectAsync(parentIri)` fetch in `OnInitializedAsync` when `GetParentIri()` is non-null); `BoostedRenderTarget`/`LikedRenderTarget` (= parent ?? the boosted/liked object) drive all body props. A fetch failure is non-fatal — falls back to rendering the boosted/liked reply itself. New CSS `.object-boost-reply-hint`/`.object-like-reply-hint`. Live-verified 2026-09-17 (fresh WASM load after a `--no-cache` rebuild): a Liked reply card shows "Liked … replying to alice" with alice's original post as the body; a Boosted reply card shows "Boosted by Gargron … replying to everton137" with everton137's original post as the body; 0 console errors. All suites green. |
   | Whole content card is the link + actor banner strip | ✅ | ✅ | ✅ | **Phase 153:** (1) **Whole-card is the link** — content cards (Create + direct-object branches in `ObjectView`) render a stretched-link overlay `<a class="object-card-link">` (absolute, `inset:0`, `z-index:0`) as the first child of `.object-item`, so the ENTIRE card navigates to the object detail instead of only the body text being a link; the body content is now a plain `<div>` (no longer `.object-post-link`) and the card gets `object-item--clickable` (`position:relative; cursor:pointer`) so hover shows the link cursor. Interactive children (moderation `.object-header`, `.engagement-bar`/`.lemmy-vote-bar`, `.object-sensitive`, `.object-poll`, `.object-recipient`, `.object-parent`/`-context`, `.object-iri`, `.object-title`, `.object-media`) are raised to `z-index:1` so they stay clickable above the overlay. (2) **Actor-header banner strip** — `.object-header-actor` (avatar + handle) sits on a thin tinted gradient band for "color and splash"/personalization; the tint is a deterministic hue derived from the card's primary author IRI via FNV-1a (`CardHue` helper, 0–359) set as an inline `--card-hue` custom property, so each author's cards carry a consistent personal color; the band is a darker, low-saturation gradient (`hsl(hue 45% 30% / .55 → .12)`) so the handle stays legible. New `CardLinkIri`/`CardHueStyle`/`CardHueIri` props + `CardHue` static helper. CSS kept identical in both `app.css`. Live-verified 2026-09-17 (fresh WASM load after a `--no-cache` rebuild): andrew's post cards show `object-item--clickable` + `--card-hue:19` + `.object-card-link` overlay, body content is a plain `<div>` (not a link); clicking the card body navigates to `/object?iri=…`; the Like button (z-index 1) stays clickable on top of the overlay without triggering card navigation; 0 new console errors. Boost/Like (Announce/Like) cards intentionally left as-is this slice (their content link is the boosted/liked target preview). Web tests 106/106. |
   | Actor page outbox cards: no per-card moderation (common card controls) | ✅ | ✅ | ✅ | **Phase 155:** the actor detail page's Posts (outbox) cards no longer render the per-card Block/Mute/Report buttons — moderation is offered once, at the actor level, in the page header. New `ObjectView.ShowModeration` parameter (default `true`) gates the two `CardModerationButtons` blocks; the actor page's outbox supplies an explicit `PagedCollection.ItemTemplate` (`OutboxItemTemplate`) rendering the common `<ObjectView>` card with `ShowModeration="false"` (mirroring the Profile `PostTemplate` pattern). The outbox cards keep every other common card control (actor bar, content, whole-card link overlay, Like/Boost/Reply engagement bar) so they look identical to the main feed / profile outbox cards. Live-verified 2026-09-17 (fresh WASM after a `--no-cache` rebuild): alice's actor page renders 12 outbox cards with 0 per-card `.card-moderate-btn` (was 12×3) while the header still shows the actor-level Block/Mute/Report controls; all 12 cards retain the engagement bar + whole-card link; the home feed renders identically to before; 0 console errors. Web tests 106/106. |
 
 ## Notifications

| Feature | A | C | D | Notes |
|---|---|---|---|---|
| Unified notification list (inbox projection) | ✅ | ✅ | ✅ | `Notifications.razor` page renders a card stream (Phase 154) with `NotificationRow` items (avatar, verb, relative time, target link). **Phase 154:** a notification whose target is a content `Note`/`Article` now renders the target via the shared `<ObjectView>` card (full header/engagement bar, like the feeds) instead of the compact preview box; non-content targets (Follow/Accept/Reject/remote notes without the object inline) keep the compact row + "View note/post" link. Live-verified: the "replied" notification renders the full Gargron object card. |
...
| New/unread highlight | ✅ | ✅ | ✅ | **Phase 154:** `GET /local/v1/notifications` also returns the account's `readAt` cursor (`NotificationsReadAt`); the client flags each card `notification-card--new` (left accent bar + tinted surface + a "New" badge) when `published > readAt` and not yet viewed this session. "Mark all as read" bumps the local cursor to now → clears every highlight + the nav badge; clicking a notification's View/content link marks it viewed so its highlight clears. Live-verified: back-dating the cursor highlighted all 7 cards; "Mark all as read" cleared all highlights + the badge. |
...
| Filter by type (follows/likes/replies/mentions) | ✅ | ✅ | ✅ | **Phase 154:** the filter pills are replaced with the shared `.tab-bar`/`.tab`/`.tab--active` tab pattern (proper `role=tablist`/`tab`/`tabpanel` ARIA) matching the other pages; the 6 filters (All/Follows/Likes/Boosts/Replies/Mentions) → server `?type=` filter are unchanged. Live-verified: "Likes" → 8 liked cards, "Boosts" → 20 boosted cards. |
