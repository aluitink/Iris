...
| 13.2 | **`ld+json` accept behavior documentation** — confirm and document that Iris accepts `application/ld+json` on inbound (Decision #4) and produces `application/activity+json` on outbound. Add a regression test proving the accept behavior. | Pending |
| 13.3 | **Mastodon `Question`/poll inbound handling** — verify that a `Question` activity (Mastodon poll) is correctly deserialized, stored, and served by Iris (as an opaque object with the `Question` type). Add integration tests. | Pending |
...
966 tests, 0 failures, 0 warnings (13.1 done, +5 tests).
