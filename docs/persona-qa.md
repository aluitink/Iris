...
4. Verdict + evidence (steps, observed, expected, build id) go in a commit on the qa branch: `qa: S## — PASS/FAIL (<one line>)`. Evidence stays in the commit, PLAN gets the verdict only.
...
4. No bug found after the flow -> note it in HIST (`clean: <flow>`), do not add PLAN noise.
