...
- **J-8 (Medium) — Person-inbox `Create` is stored but never surfaced.** A `Create` delivered to a local *person's* inbox is stored by `InboxProcessor` but no handler records it into the person's outbox (only the community-inbox catch-all, `CommunityInboxActivityHandler`, does that). This is RISK_GAP_REGISTER **G-6**. A user posting to a person sees nothing in that person's feed.

**Coverage.** 🟡 `DeliveryIntegrationTests` (server schedules + delivers a follow's `Accept`); ⬜ no end-to-end *client* post→outbox test exists (there is no client post API to test).
...
- **J-10 (High) — The server always accepts; `Reject` is never sent.** `FollowIris.BuildReject` exists but has no callers (RISK_GAP_REGISTER **G-2**). A user cannot be rejected, and there is no moderation/`manuallyApprovesFollowers` surface. (Confirmed.)
...
|----|-----|----------|-----------|---------------------|
| **J-6** 🔶 | Client "post a note" API now exists — `client.PostNoteAsync(actorId, content, to)` (Slice 11.5): builds a `Create` (embedded `Note`, deterministic id, attributed to author) and delivers it to the author's own inbox (the "local post" path). **Remaining (server half):** a server path that records the note into the author's outbox (J-8) and schedules outbound delivery to followers (J-18), so a local post is surfaced and federates. | **Blocker** | C. Post | Client half done. Server half → Phase 12 (J-8 + J-18). |
...
| **J-8** | Person-inbox `Create` stored but never surfaced in a personal feed. (= G-6) | **Medium** | C. Post | A person-inbox `Create` handler recording into the person's outbox. |
| **J-10** | Follows are always accepted; `Reject` never sent; no `manuallyApprovesFollowers`. (= G-2) | **High** | D. Follow | Wire the `Reject` scheduling path + a `manuallyApprovesFollowers` surface. |
...
**Headline for Phase 12.** The *read* side of the platform (auth, signed fetch, community feed, discovery) is solid, and the *client* write side is now mostly reachable: **J-21** (discovery exposure, Slice 11.3), **J-9** (client follow, Slice 11.4 — `client.FollowAsync`), and **J-6's client half** (client post, Slice 11.5 — `client.PostNoteAsync`) are resolved. What remains for the write path is the **server half of J-6**: the server currently stores an inbound `Create` but does not record it in the author's outbox (**J-8**) nor deliver it to followers (**J-18**) — so a local post is accepted but not yet surfaced or federated. Phase 12 should close that first (record post in outbox → outbound `Create` to followers → followed feed), then the secondary gaps.
