...
**Headline for Phase 12.** The single largest user-facing dead-end is **J-6 (no post)** — it blocks "post and have it federate" and, together with **J-18**, means the *write* side of the platform (post, follow-a-community) is not fully reachable through the client as a user would drive it, even though the *read* side (auth, signed fetch, community feed, discovery) is solid. **J-9 (client follow) is now resolved in Slice 11.4** (`client.FollowAsync`), and the handle→IRI step that gated follow/fetch — **J-21** — is resolved in Slice 11.3 (`IrisClientBundle` exposes `Discovery`/`ResolveActorAsync`), so follow can build on both. Phase 12 should close the remaining write path first (post → outbound `Create` → followed feed), then the secondary gaps.
