...
**Gaps / friction.**
- **J-6 (Blocker) — There is no "post a note" API.** `DeliverAsync` requires the caller to (a) *construct* the `Create`/`Note` activity by hand and (b) *know the target inbox IRI(s)*. There is no `client.PostAsync(note)`, no "deliver to all my followers", and no server endpoint that accepts a post from the actor's own client. The sample **never posts** (`SampleBlazorClient/Program.cs` only logs in and reads the community feed). This is the journey dead-end: **the user cannot post through the client as it stands.** It is the client-side mirror of RISK_GAP_REGISTER **G-1** (no outbound `Create`) — G-1 is stated server-side (the server never *sends* a `Create` to followers); J-6 makes explicit that the *client* has no post path at all, so even a local post-to-outbox is not wired. **Fix plan (Phase 12):** a `client.PostNoteAsync(content)` (or `DeliverCreateAsync`) that builds the `Create`, and a server path that records it in the actor's outbox and schedules delivery to followers.
...
**Journey.** A user wants to follow another actor. The *only* supported path is federation: the follower's instance delivers a signed `Follow` to the target's inbox. Concretely in tests, `client.DeliverAsync(bobInboxIri, follow)` where `follow` is a hand-built `Follow` activity (`tests/Iris.Server.Tests/FederationSignatureIntegrationTests.cs:96`). The server's `FollowActivityHandler` (`src/Iris.Server/FollowActivityHandler.cs:66-121`) records the edge and schedules an `Accept` back to the follower.

**Gaps / friction.**
- **J-9 (High) — No client "follow" API.** Like posting, following requires hand-building a `Follow` activity and knowing the target inbox IRI. There is no `client.FollowAsync(targetActorIri)`. The user journey "I want to follow @bob@host" has no first-class path. **Fix plan:** a `client.FollowAsync(Iri target)` that resolves the target's inbox (`target.InboxOf()`) and delivers a signed `Follow`.
