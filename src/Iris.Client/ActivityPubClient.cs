...
public sealed class ActivityPubClient : IActivityPubClient, IDisposable
{
    ...
    public Task<DeliveryResult> AcceptFollowAsync(Iri actorId, Iri followIri, ProxyCredentials credentials, CancellationToken ct = default)
        => LocalFollowDecisionAsync(actorId, followIri, accept: true, credentials, ct);

    /// <inheritdoc/>
    public Task<DeliveryResult> RejectFollowAsync(Iri actorId, Iri followIri, CancellationToken ct = default)
        => LocalFollowDecisionAsync(actorId, followIri, accept: false, credentials: null, ct);
    ...
    public Task<DeliveryResult> RejectFollowAsync(Iri actorId, Iri followIri, ProxyCredentials credentials, CancellationToken ct = default)
        => LocalFollowDecisionAsync(actorId, followIri, accept: false, credentials, ct);

    /// <summary>
    /// Sends a local, Basic-authenticated follow decision (an Accept or a Reject of an inbound follow) to
    /// the acting actor's own instance — the <em>followed</em> side's decision, so it is a local decision
    /// (not a signed inbox delivery; the instance builds and delivers the Accept/Reject).
    /// </summary>
    /// <param name="actorId">The IRI of the actor being followed (the one deciding).</param>
    /// <param name="followIri">The absolute IRI of the inbound <see cref="KristofferStrube.ActivityStreams.Follow"/>.</param>
    /// <param name="accept"><see langword="true"/> to accept (<c>{actorId}/follows/{followIri}/accept</c>);
    /// <see langword="false"/> to reject (<c>{actorId}/follows/{followIri}</c>). Both are body-less: the
    /// follow is identified by its absolute IRI in the path and the instance fetches it from its local
    /// activity store.</param>
    ...
    private async Task<DeliveryResult> LocalFollowDecisionAsync(
        Iri actorId,
        Iri followIri,
        bool accept,
    ...
    {
        // Resolve the local-auth handler (the same seam as a mute / relay subscription): the configured
        // default (shared, not disposed) or a request-scoped handler for explicit credentials (disposed).
        ...
        else
        {
            throw new InvalidOperationException(
                "A follow decision requires LocalCredentials (set ActivityPubClientOptions.LocalCredentials) or explicit credentials.");
        }

        // The follow being decided on is identified by its absolute IRI (in the path) — an optional trailing
        // "/accept" selects acceptance. Both decisions are body-less: the instance fetches the follow from
        // its local activity store (an inbound follow is always stored there on receipt).
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post,
            accept
                ? $"{actorId.Value.TrimEnd('/')}/follows/{followIri.Value.TrimStart('/')}/accept"
                : $"{actorId.Value.TrimEnd('/')}/follows/{followIri.Value.TrimStart('/')}");
