...
public sealed class RelayFanOutIntegrationTests : IDisposable
{
    ...
    /// handler (chicken-and-egg: the server's own fetcher must reach the in-process server, which does
    /// not exist yet while the server is being constructed). Wraps the inner handler in an
    /// <see cref="HttpClient"/> (whose <c>SendAsync</c> is public) and clones the request (the in-process
    /// transport does not clone between sends).
    /// </summary>
